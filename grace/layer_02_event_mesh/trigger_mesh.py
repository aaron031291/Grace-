...
class RoutingMetrics:
    ...
    def __post_init__(self):
        if self.priority_counts is None:
            self.priority_counts = {"critical": 0, "high": 0, "normal": 0, "low": 0}
...
class TriggerMesh:
    ...
    async def route_event(
        ...
        if not matching_rules:
            logger.warning(
                f"No routing rules found for event {event.get('type', 'unknown')}"
            )
            return event_id
        ...
        if (
            priority in [RoutingPriority.CRITICAL, RoutingPriority.HIGH]
            and self.immutable_logs
        ):
    ...
    async def _process_priority_event(
        self, priority_event: PriorityEvent, worker_id: str
    ):
        ...
        try:
            ...
            if (
                self.immutable_logs and queue_latency_ms > 10
            ):  # Log if > 10ms queue time
                await self.immutable_logs.log_system_performance(
            ...
            if len(self.latency_history[event_type]) > 1000:  # Keep last 1000 samples
                self.latency_history[event_type] = self.latency_history[event_type][
                    -1000:
                ]
            ...
            logger.debug(
                f"Processed {priority_event.event_id} in {total_latency_ms:.2f}ms"
            )
    ...
    def _transform_event(
        self, event: Dict[str, Any], transformations: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _calculate_success_rate(self) -> float:
        ...
        total = (
            self.routing_metrics.successful_deliveries
            + self.routing_metrics.failed_deliveries
        )
        if total == 0:
            return 1.0
    ...
    def _calculate_avg_latency(self) -> float:
        ...
        if self.routing_metrics.successful_deliveries == 0:
            return 0.0
        return (
            self.routing_metrics.total_latency_ms
            / self.routing_metrics.successful_deliveries
        )
