...
try:
    ...
    logger.warning("Scikit-learn not available. Clustering/DimRed adapters will use mock implementations.")
...
class KMeansAdapter(BaseModelAdapter):
    ...
    def get_cluster_centers(self):
        ...
        if self.fitted and SKLEARN_AVAILABLE and hasattr(self.model, 'cluster_centers_'):
...
class AgglomerativeClusteringAdapter(BaseModelAdapter):
    ...
    def __init__(self, **kwargs):
        ...
        if SKLEARN_AVAILABLE:
            ...
            self.model = MockClusteringModel("agglomerative", kwargs.get("n_clusters", 8))
    ...
    def predict(self, X, **kwargs):
        ...
        if hasattr(self, 'labels_'):
            ...
            logger.warning("Agglomerative clustering doesn't support prediction on new data")
...
class PCAAdapter(BaseModelAdapter):
    ...
    def fit(self, X, y=None, **kwargs):
        ...
        if SKLEARN_AVAILABLE:
            self.metadata["explained_variance_ratio"] = self.model.explained_variance_ratio_.tolist()
            self.metadata["explained_variance"] = self.model.explained_variance_.tolist()
            self.metadata["n_components"] = self.model.n_components_
            self.metadata["cumulative_variance"] = np.cumsum(self.model.explained_variance_ratio_).tolist()
    ...
    def inverse_transform(self, X_transformed, **kwargs):
        ...
        if SKLEARN_AVAILABLE:
            ...
            original_shape = (X_transformed.shape[0], len(self.feature_names) if self.feature_names else X_transformed.shape[1] * 2)
    ...
    def get_explained_variance_ratio(self):
        ...
        if self.fitted and SKLEARN_AVAILABLE and hasattr(self.model, 'explained_variance_ratio_'):
...
class MockDimRedModel:
    ...
    def fit(self, X, y=None, **kwargs):
        ...
        logger.info(f"Mock {self.model_type} dimensionality reduction model fitted (no-op)")
        return self
