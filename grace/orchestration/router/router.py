...
class Router:
    ...
    def add_route(self, route: RouteConfig):
        ...
        logger.debug(
            f"Added route: {route.source} -> {route.target} ({route.event_pattern})"
        )
    ...
    async def _process_message(self, message: RoutingMessage):
        ...
        try:
            ...
            logger.debug(
                f"Successfully routed {message.event_type} to {len(message.targets)} targets"
            )
            ...
            if message.can_retry():
                logger.warning(
                    f"Retrying message {message.correlation_id} (attempt {message.attempts})"
                )
            ...
            if target in self.target_handlers:
                ...
                if self.event_publisher:
                    await self.event_publisher(
                        f"ROUTE_TO_{target.upper()}", message.to_dict()
                    )
                else:
    ...
    def _update_route_metrics(
        self, message: RoutingMessage, latency_ms: float, success: bool
    ):
