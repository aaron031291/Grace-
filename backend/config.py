...
class Settings(BaseSettings):
    ...
    secret_key: str = Field(
        default_factory=lambda: os.getenv("SECRET_KEY", ""), env="SECRET_KEY"
    )

    # Database
    database_url: str = Field(
        default="postgresql://grace_user:grace_pass@localhost:5432/grace_governance",
        env="DATABASE_URL",
    )
    ...
    storage_access_key: str = Field(
        default_factory=lambda: os.getenv("STORAGE_ACCESS_KEY", ""),
        env="STORAGE_ACCESS_KEY",
    )
    storage_secret_key: str = Field(
        default_factory=lambda: os.getenv("STORAGE_SECRET_KEY", ""),
    ...
    allowed_file_types: List[str] = Field(
        default=["pdf", "docx", "txt", "html", "md", "json"], env="ALLOWED_FILE_TYPES"
    )

    # User quotas
    default_storage_quota: int = Field(
        default=1024 * 1024 * 1024, env="DEFAULT_STORAGE_QUOTA"
    )  # 1GB
    default_memory_fragments_quota: int = Field(
        default=10000, env="DEFAULT_MEMORY_FRAGMENTS_QUOTA"
    )
    ...
    # Policy engine
    policy_engine_type: str = Field(
        default="yaml", env="POLICY_ENGINE_TYPE"
    )  # yaml or opa
