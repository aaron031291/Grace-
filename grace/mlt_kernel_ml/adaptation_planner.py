...
from .contracts import (
    ...
    RecommendationType, generate_plan_id
)
...
class AdaptationPlanner:
    ...
    async def create_adaptation_plan(self, insights: List[Insight]) -> Optional[AdaptationPlan]:
        ...
        try:
            ...
            logger.info(f"Created adaptation plan {plan.plan_id} with {len(actions)} actions")
            return plan
    ...
    async def _plan_retrain(self, insight: Insight) -> tuple[List[Action], Dict[str, str], Dict[str, Any]]:
    ...
    async def _plan_reweight(self, insight: Insight) -> tuple[List[Action], Dict[str, str], Dict[str, Any]]:
        ...
        if weights:
            actions.append(Action(
                type=ActionType.REWEIGHT_SPECIALISTS,
                weights=weights
            ))
    ...
    async def _plan_recalibrate(self, insight: Insight) -> tuple[List[Action], Dict[str, str], Dict[str, Any]]:
    ...
    async def _plan_hpo(self, insight: Insight) -> tuple[List[Action], Dict[str, str], Dict[str, Any]]:
    ...
    async def _plan_policy_tune(self, insight: Insight) -> tuple[List[Action], Dict[str, str], Dict[str, Any]]:
        ...
        if insight.type == InsightType.GOVERNANCE_ALIGNMENT:
            ...
            expected_effects = {"governance_compliance": "+0.05", "approval_rate": "+0.1"}
    ...
    async def _plan_segment_route(self, insight: Insight) -> tuple[List[Action], Dict[str, str], Dict[str, Any]]:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for action_type in ActionType:
            count = sum(1 for plan in self.plans for action in plan.actions if action.type == action_type)
        ...
        return {
            ...
            "avg_actions_per_plan": sum(len(plan.actions) for plan in self.plans) / len(self.plans) if self.plans else 0
        }