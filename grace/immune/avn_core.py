    ...
    def calculate_health_score(self) -> float:
        ...
        for metric_name, metric_history in self.metrics.items():
            ...
            if latest_metric.is_healthy():
                ...
                if (
                    latest_metric.threshold_min is not None
                    and latest_metric.value < latest_metric.threshold_min
                ):
                    deviation = (
                        latest_metric.threshold_min - latest_metric.value
                    ) / latest_metric.threshold_min
                    health_factors.append(max(0.0, 1.0 - deviation))
                elif (
                    latest_metric.threshold_max is not None
                    and latest_metric.value > latest_metric.threshold_max
                ):
                    deviation = (
                        latest_metric.value - latest_metric.threshold_max
                    ) / latest_metric.threshold_max
                    health_factors.append(max(0.0, 1.0 - deviation))
                else:
...
class EnhancedAVNCore:
    ...
    async def _collect_system_metrics(self):
        ...
        try:
            ...
            await self.report_metric(
                "system", "cpu_usage", cpu_percent / 100.0, {"max": 0.9}
            )
            await self.report_metric(
                "system", "memory_usage", memory_percent, {"max": 0.9}
            )
            await self.report_metric(
                "system", "disk_usage", disk_percent, {"max": 0.95}
            )
    ...
    async def _update_component_health_scores(self):
        ...
        for component_id, component in self.component_health.items():
            ...
            await self.report_metric(
                component_id, "health_score", health_score, {"min": 0.7}
            )
    ...
    async def _check_metric_anomaly(self, metric: HealthMetric):
        ...
        if not metric.is_healthy():
            ...
            if metric.threshold_min is not None and metric.value < metric.threshold_min:
                deviation_ratio = (
                    metric.threshold_min - metric.value
                ...
                metric.threshold_max is not None and metric.value > metric.threshold_max
            ):
                deviation_ratio = (
                    metric.value - metric.threshold_max
        ...
        for component_id, component in self.component_health.items():
            ...
            for metric_name in ["decision_latency", "response_time", "throughput"]:
                ...
                if trend is not None:
                    if (
                        metric_name in ["decision_latency", "response_time"]
                        and trend > 0.1
                    ):
                        # Latency increasing
                        await self._raise_alert(
    ...
    async def _detect_trust_anomalies(self):
        ...
        for component_id, component in self.component_health.items():
            ...
            if trust_metric and trust_metric.value < 0.6:
                ...
                trust_trend = component.get_metric_trend(
                    "trust_score", window_minutes=30
                )
    ...
    async def _raise_alert(
        ...
        for alert in self.active_alerts.values():
            if (
                ...
                and (datetime.now() - alert.timestamp).total_seconds() < 300
            ):  # 5 minutes
        ...
        resolution_actions = self._get_resolution_actions(
            anomaly_type, component_id, metrics
        )
        ...
        logger.warning(
            f"Raised {severity.value} alert for {component_id}: {description}"
        )
    ...
    def _get_resolution_actions(
        self, anomaly_type: AnomalyType, component_id: str, metrics: Dict[str, float]
    ) -> List[str]:
        """Get recommended resolution actions for an anomaly."""
        actions = []

        if anomaly_type == AnomalyType.PERFORMANCE_DEGRADATION:
            actions.extend(["restart_component", "scale_resources", "reduce_load"])
        elif anomaly_type == AnomalyType.COMPONENT_FAILURE:
            actions.extend(
                ["restart_component", "failover_to_backup", "check_dependencies"]
            )
                ...
                try:
                    ...
                    if success:
                        await self.resolve_alert(
                            alert.alert_id, f"Auto-healed via {action}"
                        )
                        break
    ...
    def get_system_health_summary(self) -> Dict[str, Any]:
        ...
        health_scores = [
            comp.calculate_health_score() for comp in self.component_health.values()
        ]
