...
class LinuxAdapter(ProcessAdapter, FSAdapter, NetAdapter, PkgAdapter, SnapshotAdapter):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        logger.info(f"Linux adapter initialized with {self.package_manager} package manager")
    ...
    async def get_process_info(self, pid: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = await asyncio.create_subprocess_exec(
                "ps", "-p", str(pid), "-o", "pid,ppid,user,cpu,mem,cmd",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
    ...
    def _apply_sandbox(self, command: List[str], sandbox: str, constraints: Dict[str, Any]) -> List[str]:
