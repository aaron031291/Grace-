...
def test_intelligence_service():
    ...
    try:
        ...
        task_request = {
            "task": "classification",
            "input": {
                "X": {"feature1": 0.5, "feature2": 1.2, "feature3": -0.3},
                "modality": "tabular",
            },
            "context": {
        ...
        print(
            f"✓ Meta Ensembler: Prediction = {ensemble_result.get('prediction', 'N/A')}"
        )

        # Test inference engine
        inference_result = service.inference_engine.execute(plan, task_request)
        print(
            f"✓ Inference Engine: Status = {inference_result.get('status', 'completed')}"
        )
...
def test_api_schemas():
    ...
    sample_result = {
        "req_id": "req_20240928_143022_1234",
        "outputs": {"y_hat": "approved", "confidence": 0.85},
