...
class EnhancedGovernanceLiaison:
        ...
        try:
            ...
            for specialist in self.specialists:
                try:
                    ...
                    if result:
                        logger.info(
                            f"Initialized specialist: {specialist.specialist_id}"
                        )
                    else:
                        logger.warning(
                            f"Failed to initialize specialist: {specialist.specialist_id}"
                        )
                except Exception as e:
                    logger.error(
                        f"Error initializing specialist {specialist.specialist_id}: {e}"
                    )
            ...
            logger.info(
                f"Initialized {working_specialists}/{len(self.specialists)} specialists"
            )
    ...
    async def process_governance_request(
        ...
        try:
            ...
            specialist_results = await self._gather_specialist_predictions(
                request_data, context
            )

            # Phase 2: Build Enhanced Consensus
            consensus_result = await self._build_enhanced_consensus(
                specialist_results, context
            )
    ...
    async def _gather_specialist_predictions(
        self, request_data: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for specialist in self.specialists:
            ...
            try:
                ...
                specialist_data = self._prepare_specialist_data(
                    request_data, specialist
                )

                # Get prediction with uncertainty
                prediction = await specialist.predict_with_uncertainty(
                    specialist_data, context
                )
                ...
                logger.error(
                    f"Specialist {specialist.specialist_id} prediction failed: {e}"
                )
                predictions.append(
                    {
                        "specialist_id": specialist.specialist_id,
    ...
    def _prepare_specialist_data(
        self, request_data: Dict[str, Any], specialist: EnhancedMLSpecialist
    ) -> Dict[str, Any]:
    ...
    async def _build_enhanced_consensus(
        self,
        ...
        try:
            ...
            consensus_hallucination_risk = np.average(
                hallucination_risks, weights=weights
            )
            consensus_cross_domain_score = np.average(
                cross_domain_scores, weights=weights
            )

            # Check consensus quality
            confidence_agreement = 1.0 - np.std(
                confidences
            )  # High agreement if low std
            ...
            consensus_reached = (
                consensus_confidence >= self.config["consensus"]["confidence_threshold"]
                and consensus_uncertainty
                <= self.config["consensus"]["uncertainty_threshold"]
                and consensus_hallucination_risk
                <= self.config["consensus"]["hallucination_threshold"]
                and overall_agreement
                >= self.config["consensus"]["min_specialist_agreement"]
            )
            ...
            if consensus_reached:
                consensus_result["reasoning"] = (
                    f"Strong consensus reached with {consensus_confidence:.2f} confidence and {overall_agreement:.2f} agreement"
                )
            else:
                reasons = []
                if (
                    consensus_confidence
                    < self.config["consensus"]["confidence_threshold"]
                ):
                ...
                if (
                    consensus_hallucination_risk
                    > self.config["consensus"]["hallucination_threshold"]
                ):
                    reasons.append(
                        f"high hallucination risk ({consensus_hallucination_risk:.2f})"
                    )
                if (
                    overall_agreement
                    < self.config["consensus"]["min_specialist_agreement"]
                ...
                consensus_result["reasoning"] = (
                    f"Consensus not reached: {', '.join(reasons)}"
                )
    ...
    async def _perform_cross_domain_validation(
        self,
        ...
        try:
            ...
            for validator in self.validators:
                try:
                    ...
                    if (
                        ...
                        < self.config["validation"]["min_validation_score"]
                    ):
                    ...
                    if (
                        ...
                        < self.config["validation"]["min_compliance_score"]
                    ):
                        validation_results["summary"]["critical_issues"].append(
                            f"{validation.domain} compliance score too low: {validation.compliance_score:.2f}"
                        )

                    validation_results["summary"]["red_flags"].extend(
                        validation.red_flags
                    )
                    validation_results["summary"]["recommendations"].extend(
            ...
            if validation_scores:
                validation_results["summary"]["avg_validation_score"] = np.mean(
                    validation_scores
                )
            if compliance_scores:
                validation_results["summary"]["avg_compliance_score"] = np.mean(
                    compliance_scores
                )

            validation_results["summary"]["validation_passed"] = (
                validation_results["summary"]["avg_validation_score"]
                >= self.config["validation"]["min_validation_score"]
                and validation_results["summary"]["avg_compliance_score"]
                >= self.config["validation"]["min_compliance_score"]
                and (
                    not self.config["validation"]["red_flag_block"]
                    or not validation_results["summary"]["red_flags"]
                )
            )
    ...
    async def _assess_hallucination_risk(
        ...
        try:
            ...
            if not self.config["hallucination_mitigation"]["enabled"]:
                hallucination_assessment["overall_risk"] = consensus_result.get(
                    "consensus_hallucination_risk", 0.5
                )
            ...
            if risk_factors:
                hallucination_assessment["overall_risk"] = np.mean(
                    [rf["risk"] for rf in risk_factors]
                )
                hallucination_assessment["risk_factors"] = risk_factors
            else:
                hallucination_assessment["overall_risk"] = consensus_result.get(
                    "consensus_hallucination_risk", 0.0
                )
            ...
            if (
                hallucination_assessment["overall_risk"]
                > self.config["hallucination_mitigation"]["detection_threshold"]
            ):
                strategies = self.config["hallucination_mitigation"][
                    "mitigation_strategies"
                ]
    ...
    async def _make_governance_decision(
        self,
        request_data: Dict[str, Any],
        specialist_results: List[Dict[str, Any]],
        consensus_result: Dict[str, Any],
        ...
        try:
            ...
            # Extract key metrics
            consensus_confidence = hallucination_assessment.get(
                "confidence_after_mitigation", 0.0
            )
            consensus_reached = consensus_result.get("consensus_reached", False)
            validation_passed = validation_results["summary"].get(
            ...
            weighted_confidence = (
                ...
                + consensus_result.get("consensus_cross_domain_score", 0.5)
                * governance_weights["constitutional_weight"]
                + validation_results["summary"]["avg_validation_score"]
                * governance_weights["validation_weight"]
            )
            ...
            decision["recommendations"].extend(
                validation_results["summary"].get("recommendations", [])
            )
            ...
            if (
                decision["confidence"]
                < self.config["governance"]["escalation_threshold"]
            ):
                decision["escalation_required"] = True
                decision["recommendations"].append(
                    "Human oversight recommended due to low confidence"
                )
    ...
    async def update_specialist_performance(
        self, request_id: str, actual_outcome: Any
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for result in specialist_results:
                if result.get("prediction"):
                    ...
                    if specialist:
                        specialist.update_performance(
                            actual_outcome, prediction, confidence
                        )
