...
class SagaEngine:
    ...
    async def start(self):
        ...
        if self.event_bus:
            ...
            await self.event_bus.subscribe(
                "saga_step_completed", self._handle_step_completion
            )
            await self.event_bus.subscribe(
                "saga_step_failed", self._handle_step_failure
            )
    ...
    async def stop(self):
        ...
        for saga_exec in self.active_sagas.values():
            if saga_exec.status == SagaStatus.IN_PROGRESS:
                logger.warning(
                    f"Force compensating saga {saga_exec.saga_def.saga_id} due to shutdown"
                )
    ...
    async def execute_saga(self, saga_def: SagaDefinition) -> str:
        ...
        if len(self.active_sagas) >= self.max_concurrent_sagas:
            raise RuntimeError(
                f"Maximum concurrent sagas ({self.max_concurrent_sagas}) exceeded"
            )
    ...
    async def _execute_step(self, saga_exec: SagaExecution, step: SagaStep) -> bool:
        ...
        logger.info(
            f"Executing step {step.step_id} ({step.name}) in saga {saga_exec.saga_def.saga_id}"
        )
    ...
    async def _compensate_saga(self, saga_exec: SagaExecution):
        ...
        for step_id in completed_step_ids:
            step = next(
                (s for s in saga_exec.saga_def.steps if s.step_id == step_id), None
            )
    ...
    async def _compensate_step(self, saga_exec: SagaExecution, step: SagaStep):
        ...
        logger.info(
            f"Compensating step {step.step_id} in saga {saga_exec.saga_def.saga_id}"
        )

        try:
            if asyncio.iscoroutinefunction(step.compensation):
                await step.compensation(
                    step.parameters, saga_exec.saga_def.context, step.result
                )
            else:
                step.compensation(
                    step.parameters, saga_exec.saga_def.context, step.result
        ...
        self.saga_history.append(
            {
                ...
                "started_at": saga_exec.started_at.isoformat()
                if saga_exec.started_at
                else None,
                "completed_at": saga_exec.completed_at.isoformat()
                if saga_exec.completed_at
                else None,
                "compensation_completed_at": saga_exec.compensation_completed_at.isoformat()
                if saga_exec.compensation_completed_at
                else None,
    ...
    async def _log_saga_event(
        ...
        try:
            await self.event_bus.publish(
                event_type, event_data, correlation_id=saga_exec.saga_def.correlation_id
            )
        except Exception as e:
    ...
    async def _cleanup_old_sagas(self):
        ...
        for saga_id, saga_exec in self.completed_sagas.items():
            completed_time = (
                saga_exec.completed_at or saga_exec.compensation_completed_at
            )
