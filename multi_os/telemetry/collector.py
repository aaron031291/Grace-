...
class TelemetryCollector:
    ...
    def get_metrics_summary(
        ...
        summary = {
            ...
            "metrics": {},
            "kpis": self.kpis.copy(),
            "collection_period": {"start": None, "end": datetime.utcnow().isoformat()},
        }
    ...
    def get_traces(
        ...
        sorted_traces = sorted(
            self.traces.values(), key=lambda t: t.get("started_at", ""), reverse=True
        )
    ...
    def _process_event_metrics(
        self, event_name: str, payload: Dict[str, Any], host_id: Optional[str]
    ) -> None:
        ...
        if event_name == "MOS_TASK_COMPLETED":
            ...
            self.record_metric(
                "task_success", success_value, {"host_id": host_id}, timestamp
            )
        ...
        # Host health metrics
            ...
            for metric, value in health_data.items():
                if isinstance(value, (int, float)) and metric != "host_id":
                    self.record_metric(
                        f"host_{metric}", value, {"host_id": host_id}, timestamp
                    )

        # Sandbox violation tracking
        elif (
            "sandbox" in event_name.lower()
            and "violation" in payload.get("type", "").lower()
        ):
    ...
    def _aggregate_metric(
        self, metric_name: str, time_window: Optional[int]
    ) -> Dict[str, Any]:
    ...
    def _filter_by_time_window(
        self, measurements: deque, time_window: Optional[int]
    ) -> List[Dict]:
