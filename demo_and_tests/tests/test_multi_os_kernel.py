...
async def test_multi_os_service():
    ...
    try:
        ...
        python_task = {
            ...
            "io": {"stdin": None, "files_in": [], "files_out": ["output.txt"]},
            "constraints": {
                "os": ["linux", "macos"],
                "arch": ["x86_64", "arm64"],
                "gpu_required": False,
        ...
        print(
            f"   ✓ Task placement result: {placement['success']} on {placement.get('host_id', 'none')}"
        )
        ...
        print(
            f"   ✓ GPU task placement: {gpu_placement['success']} on {gpu_placement.get('host_id', 'none')}"
        )
        ...
        service.telemetry.record_metric(
            "task_duration_ms", 1500, {"host_id": linux_host_id}
        )
        service.telemetry.record_metric(
            "cpu_utilization", 0.65, {"host_id": linux_host_id}
        )

        # Record logs
        service.telemetry.record_log(
            "INFO", "Multi-OS test execution started", linux_host_id, "task-python-001"
        )
        service.telemetry.record_log(
            "DEBUG", "Task placement completed", linux_host_id, "task-python-001"
        )

        # Record events
        service.telemetry.record_event(
            "MOS_TASK_COMPLETED",
            {"task_id": "task-python-001", "status": "success", "duration_ms": 1500},
            linux_host_id,
        )

        metrics_summary = service.telemetry.get_metrics_summary()
        print(
            f"   ✓ Metrics collected: {metrics_summary['total_metrics']} metric types"
        )
        ...
        if snapshots:
            ...
            rollback_result = await service.snapshot_manager.rollback(
                snapshot_id, dry_run=True
            )
            print(
                f"   ✓ Rollback dry run successful: {rollback_result.get('dry_run', False)}"
            )
        ...
        await service.mesh_bridge.publish_task_completed(
            ...
            0,
            ["output.txt"],
            "/logs/task-python-001",
            1500,
            linux_host_id,
        )
        await service.mesh_bridge.publish_host_health(
            linux_host_id,
            "online",
        ...
        print(
            f"   ✓ Event routing active: {routing_stats['total_events_published']} events"
        )
        ...
        print(
            f"   ✓ Placement success rate: {service_stats['placement'].get('success_rate', 0):.2%}"
        )
        print(
            f"   ✓ Telemetry points: {service_stats['telemetry']['collection_stats']['total_metrics']}"
        )
        print(
            f"   ✓ Snapshots available: {service_stats['snapshots']['total_snapshots']}"
        )
        ...
        print("=" * 60)
        print(
            f"✅ Service Initialized: Multi-OS Service with {len(service.adapters)} OS adapters"
        )
        print(
            f"✅ Host Management: {service_stats['hosts']['total_hosts']} hosts registered across {len(service_stats['hosts']['by_os'])} OS types"
        )
        print(
            f"✅ Task Scheduling: Intelligent placement with {len(service.scheduler.placement_weights)} optimization factors"
        )
        print(
            f"✅ Telemetry System: {len(service.telemetry.kpis)} KPIs tracked with {service_stats['telemetry']['collection_stats']['total_metrics']} data points"
        )
        print(
            f"✅ Snapshot Management: {service_stats['snapshots']['total_snapshots']} snapshots with rollback capability"
        )
        print(
            f"✅ Event Mesh: {routing_stats['total_events_published']} events published with {len(routing_stats['events_by_type'])} event types"
        )
        ...
        print(
            f"✅ API Contract: OpenAPI 3.0 specification with {len([key for key in dir(app) if 'route' in key.lower()])} endpoints"
        )
