...
class JobQueue:
    ...
    async def enqueue(
        ...
        if idempotency_key in self.idempotency_cache:
            ...
            logger.info(f"Job with idempotency key {idempotency_key} already exists: {existing_job_id}")
    ...
    async def _process_jobs(self):
        ...
        try:
            while True:
                ...
                completed_tasks = [job_id for job_id, task in self.running_jobs.items() 
                                 if task.done()]
...
async def document_ingest_handler(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = await interface.grace_intelligence.ingest_batch_document(file_path, file_type)
