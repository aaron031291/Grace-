...
class EventBatch:
    ...
    def is_ready(self, config: FlowControlConfig) -> bool:
        ...
        return (
            len(self.events) >= config.max_batch_size
            or (datetime.utcnow() - self.created_at).total_seconds() * 1000
    ...
    def update_metrics(self, processing_time_ms: float, success: bool = True):
        ...
        if total_events > 0:
            self.average_processing_time_ms = (
                self.total_processing_time_ms / total_events
            )
...
class TokenBucket:
    ...
    def consume(self, tokens: int = 1) -> bool:
        ...
        with self._lock:
            ...
            self.tokens = min(
                self.burst_capacity, self.tokens + elapsed * self.rate_limit
            )
...
class PriorityEventQueue:
    ...
    def __init__(self, config: FlowControlConfig):
        ...
        self._stats = {"enqueued": 0, "dequeued": 0, "dropped": 0, "batched": 0}
...
class FlowControlManager:
    ...
    def __init__(self, config: FlowControlConfig = None, event_bus=None):
        ...
        self.token_bucket = TokenBucket(
            self.config.max_events_per_second, self.config.burst_capacity
        )
    ...
    async def stop(self):
        ...
        await asyncio.gather(
            self.batch_processor_task, self.monitoring_task, return_exceptions=True
        )

        logger.info("Flow Control Manager stopped")

    async def submit_event(
        self, event: Any, priority: int = 3, consumer_id: str = None
    ) -> bool:
        """
        ...
        if not self.token_bucket.consume():
            ...
            if wait_time > 0.1:  # Don't wait more than 100ms
                ...
                logger.debug("Rate limit exceeded, dropping event")
                return False
            else:
    ...
    async def _process_event_batch(self, batch_events: List[Tuple[Any, int]]):
        ...
        for event, priority in batch_events:
            ...
            if target_consumers:
                ...
                target = min(
                    target_consumers, key=lambda c: self.consumers[c].events_processed
                )
    ...
    async def _process_consumer_batch(
        self, consumer_id: str, events: List[Tuple[Any, int]]
    ):
        ...
        try:
            ...
            if (
                self.circuit_breaker_failures[consumer_id]
        ...
        for consumer_id, metrics in self.consumers.items():
            ...
            if metrics.average_processing_time_ms > slow_threshold:
                if consumer_id not in self.slow_consumers:
                    ...
                    logger.warning(
                        f"Consumer {consumer_id} marked as slow (avg: {metrics.average_processing_time_ms:.2f}ms)"
                    )
    ...
    async def _update_flow_control_status(self):
        ...
        if new_status != self.current_status:
            ...
            logger.info(
                f"Flow control status changed: {self.current_status.value} -> {new_status.value}"
            )
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        is_healthy = (
            ...
            in [FlowControlStatus.NORMAL, FlowControlStatus.THROTTLED]
            and queue_size < self.config.high_water_mark
            and (total_consumers == 0 or healthy_consumers / total_consumers >= 0.8)
        )
