...
class TaskRouter:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.specialist_mapping = {
            ...
            "clustering": [
                "kmeans_specialist",
                "hierarchical_specialist",
                "density_specialist",
                "spectral_specialist",
                "mixture_model_specialist",
            ],
            "nlp": [
                "transformer_nlp",
                "bert_specialist",
                ...
                "named_entity_specialist",
            ],
            "vision": [
                "cnn_vision",
                "resnet_specialist",
                "vit_specialist",
                "object_detection_specialist",
            ...
            "rl": [
                "dqn_specialist",
                "policy_gradient_specialist",
                "actor_critic_specialist",
                "multi_agent_specialist",
                "model_based_rl",
            ],
        }
    ...
    def _detect_task(self, task_req: Dict[str, Any]) -> str:
        ...
        if task_scores:
            ...
            if best_task[1] > 0:
                logger.info(
                    f"Auto-detected task: {best_task[0]} (score: {best_task[1]})"
                )
                return best_task[0]
    ...
    def _screen_policies(
        self, task_req: Dict[str, Any], task_type: str
    ) -> Dict[str, Any]:
        ...
        if isinstance(input_data.get("X"), str):
            ...
            for pattern in pii_patterns:
                if re.search(pattern, content):
                    return {"allowed": False, "reason": "PII detected in input data"}

        # Check latency budget
        latency_budget = context.get("latency_budget_ms")
        if latency_budget and latency_budget < 100:
            if task_type in ["vision", "nlp"] and "transformer" in str(
                models_allowlist
            ):
                return {
                    "allowed": False,
    ...
    def _select_specialists(
        self, task_type: str, task_req: Dict[str, Any]
    ) -> List[str]:
        ...
        for specialist in available_specialists:
            # Apply denylist
            if models_denylist and any(
                blocked in specialist for blocked in models_denylist
            ):
                continue

            # Apply allowlist if specified
            if models_allowlist and not any(
                allowed in specialist for allowed in models_allowlist
            ):
                continue
    ...
    def _select_models(
        self, specialists: List[str], task_req: Dict[str, Any]
    ) -> List[str]:
    ...
    def _optimize_route(
        self, specialists: List[str], models: List[str], task_req: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        # Shadow deployment
        allow_shadow = self.config.get("allow_shadow", True) and context.get(
            "canary_allowed", True
        )
