...
from .detectors.slis import evaluate_sli
...
class ResiliencePolicyRequest(BaseModel):
    ...
    circuit_breaker: Optional[Dict[str, Any]] = Field(None, description="Circuit breaker config")
    rate_limit: Optional[Dict[str, Any]] = Field(None, description="Rate limiting config")
    bulkhead: Optional[Dict[str, Any]] = Field(None, description="Bulkhead config")
    degradation_modes: Optional[List[Dict[str, Any]]] = Field(None, description="Degradation modes")
...
class ChaosExperimentRequest(BaseModel):
    ...
    blast_radius_pct: float = Field(..., ge=0, le=100, description="Blast radius percentage")
...
class ResilienceService:
    ...
    def _register_routes(self):
            ...
            try:
                ...
                if policy.circuit_breaker:
                    ...
                    self.circuit_breakers[cb_key] = CircuitBreaker(
                        failure_threshold_pct=policy.circuit_breaker.get("failure_rate_threshold_pct", 50),
                        volume_threshold=policy.circuit_breaker.get("request_volume_threshold", 20),
                        sleep_window_ms=policy.circuit_breaker.get("sleep_window_ms", 5000),
                        half_open_max_calls=policy.circuit_breaker.get("half_open_max_calls", 5)
                    )
        ...
        async def open_incident(incident: IncidentRequest):
            try:
                ...
                await self.mesh_bridge.publish_event("RES_INCIDENT_OPENED", {
        ...
        async def apply_action(incident_id: str, action_request: ActionRequest):
            try:
                ...
                # Emit action event
                await self.mesh_bridge.publish_event("RES_ACTION_APPLIED", {
        ...
        async def set_circuit_state(service_id: str, dep: str, state_request: CircuitStateRequest):
            try:
                ...
                await self.mesh_bridge.publish_event("RES_CIRCUIT_STATE", {
                    "service_id": service_id,
                    "dependency": dep,
                    "state": breaker.state()
                })
        ...
        async def enter_degradation(service_id: str, request: DegradationRequest):
            try:
                ...
                await self.mesh_bridge.publish_event("RES_DEGRADATION_ENTERED", {
                    "service_id": service_id,
        ...
        async def exit_degradation(service_id: str, request: DegradationRequest):
            try:
                ...
                # Emit degradation event
                await self.mesh_bridge.publish_event("RES_DEGRADATION_EXITED", {
                    "service_id": service_id,
                    "mode_id": request.mode_id
                })
        ...
        async def start_chaos_experiment(experiment: ChaosExperimentRequest):
            try:
                ...
                await self.mesh_bridge.publish_event("RES_CHAOS_STARTED", {
        ...
        async def export_snapshot(request: SnapshotExportRequest):
            try:
                ...
                await self.mesh_bridge.publish_event("RES_SNAPSHOT_CREATED", {
    ...
    async def _execute_healing_action(self, incident_id: str, action: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if action == "restart":
                ...
                mode_id = params.get("mode_id", "cached_only") if params else "cached_only"
                await enter_mode(service_id, mode_id)
                result = {"status": "success", "notes": f"Entered degradation mode: {mode_id}"}
            else:
    ...
    def export_snapshot(self) -> dict:
        ...
        snapshot = {
            ...
            "services": {
                ...
                for service_id in set(list(self.slo_policies.keys()) + list(self.resilience_policies.keys()))
            },
    ...
    async def start(self):
        ...
        await self.snapshot_manager.start() if hasattr(self.snapshot_manager, 'start') else None
