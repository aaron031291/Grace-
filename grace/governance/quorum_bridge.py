...
class QuorumBridge:
    ...
    def consensus(self, feed_ids: List[str], context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if self.intelligence_kernel:
            try:
                ...
                logger.warning(f"Intelligence kernel consensus failed: {e}, falling back to mock")
                pass
        
        # Mock consensus for development
        return self._mock_consensus(feed_ids, context)
    
    def _mock_consensus(self, feed_ids: List[str], context: Optional[Dict] = None) -> Dict[str, Any]:
