...
class RBACCheck:
    ...
    def check_permission(rbac: RBACContext, required_permission: str) -> bool:
        ...
        governance_permissions = {'governance.approve', 'governance.review', 'governance.override'}
...
class GovernanceAPIService:
    ...
    def _register_routes(self):
        ...
        @self.app.get("/api/governance/v1/status/{request_id}", response_model=GovernanceStatus)
    ...
    async def _handle_queue_request(self, governance_request: GovernanceRequest, request: Request) -> Dict[str, Any]:
    ...
    async def _handle_approval(self, decision: GovernanceDecision, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._can_approve_request(request_record["action_type"], rbac_context):
                raise HTTPException(status_code=403, detail="Insufficient permissions to approve this request")
            ...
            if request_record["approvals_received"] >= request_record["approvals_required"]:
                ...
                remaining = request_record["approvals_required"] - request_record["approvals_received"]
    ...
    async def _handle_rejection(self, decision: GovernanceDecision, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check approver permissions
            if not self._can_approve_request(request_record["action_type"], rbac_context):
                raise HTTPException(status_code=403, detail="Insufficient permissions to reject this request")
    ...
    async def _handle_get_status(self, request_id: str, request: Request) -> GovernanceStatus:
        """Get request status."""
        try:
    ...
    async def _handle_list_pending(self, request: Request, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for request_id, record in self.pending_requests.items():
                ...
                if rbac_context and self._can_view_request(record["action_type"], rbac_context):
                    pending.append({
                        "request_id": request_id,
    ...
    async def _handle_override(self, request_id: str, reason: str, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            if not RBACCheck.check_permission(rbac_context, "governance.override"):
                raise HTTPException(status_code=403, detail="Override permission required")
    ...
    def _can_submit_request(self, action_type: str, rbac_context: RBACContext) -> bool:
        ...
        return any(RBACCheck.check_permission(rbac_context, perm) for perm in required_permissions)
    ...
    def _can_approve_request(self, action_type: str, rbac_context: RBACContext) -> bool:
        ...
        has_permission = any(RBACCheck.check_permission(rbac_context, perm) for perm in required_permissions)
    ...
    def _can_view_request(self, action_type: str, rbac_context: RBACContext) -> bool:
        ...
        return (
            self._can_approve_request(action_type, rbac_context) or
            RBACCheck.check_permission(rbac_context, "governance.view") or
            "governance_viewer" in rbac_context.roles
        )
    ...
    def _calculate_average_approval_time(self) -> float:
        ...
        for record in self.request_history.values():
            if record["status"] == "approved":
                approval_time = (record["updated_at"] - record["created_at"]).total_seconds()
                approval_times.append(approval_time)
        
        return (sum(approval_times) / len(approval_times) / 3600) if approval_times else 0
    
    def _get_requests_by_type(self) -> Dict[str, int]:
        """Get request counts by type."""
        type_counts = {}
        
        for record in list(self.pending_requests.values()) + list(self.request_history.values()):
            action_type = record["action_type"]
            type_counts[action_type] = type_counts.get(action_type, 0) + 1
...
if not FASTAPI_AVAILABLE:
    logger.warning("FastAPI not available. Governance API running in compatibility mode.")
