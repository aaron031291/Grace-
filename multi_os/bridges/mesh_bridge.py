...
class MeshBridge:
    ...
    def _default_routing_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "MOS_EXPERIENCE": {"targets": ["mlt"], "priority": "low", "persist": True},
        }
    ...
    async def publish_event(
        ...
        try:
            if GME_AVAILABLE:
                ...
                logger.info(
                    f"Published GME event {event_name} with msg_id {envelope.msg_id}"
                )
    ...
    async def publish_task_submitted(self, task: Dict[str, Any], host_id: str) -> str:
        ...
        return await self.publish_event(
            "MOS_TASK_SUBMITTED", {"task": task, "host_id": host_id}, host_id=host_id
        )
    ...
    async def publish_host_health(
        self, host_id: str, status: str, metrics: Dict[str, Any]
    ) -> str:
    ...
    async def publish_agent_rollout(
        self, from_version: str, to_version: str, mode: str, progress: int
    ) -> str:
    ...
    async def publish_snapshot_created(
        self, snapshot_id: str, scope: str, uri: str, host_id: Optional[str] = None
    ) -> str:
    ...
    async def publish_rollback_requested(self, target: str, to_snapshot: str) -> str:
        ...
        return await self.publish_event(
            "ROLLBACK_REQUESTED", {"target": target, "to_snapshot": to_snapshot}
        )
    ...
    async def publish_experience(self, experience: Dict[str, Any]) -> str:
        ...
        return await self.publish_event(
            "MOS_EXPERIENCE",
            {"schema_version": "1.0.0", "experience": experience},
