...
async def test_orb_interface():
    ...
    message_id = await orb.send_chat_message(session_id, "Hello Grace, can you analyze EUR/USD market data?")
    ...
    # Test panel creation
    trading_panel = await orb.create_panel(session_id, PanelType.TRADING, "EUR/USD Analysis")
    print(f"✅ Created trading panel: {trading_panel}")
    
    analytics_panel = await orb.create_panel(session_id, PanelType.ANALYTICS, "Market Analytics")
    ...
    try:
        ...
        fragment_id = await orb.upload_document("user123", temp_file, "txt", {"tags": ["test", "demo"]})
...
async def test_grace_intelligence():
    ...
    from grace.intelligence.grace_intelligence import GraceIntelligence, ReasoningContext
    ...
    for query in test_queries:
        ...
        try:
            ...
            print(f"🏛️ Constitutional compliance: {result.verification.constitutional_compliance}")
            
            if result.ui_instructions:
                print(f"🖼️ UI instructions: {len(result.ui_instructions.get('panels', []))} panels to create")
            
            if result.knowledge_updates:
                print(f"🧠 Knowledge updates: {len(result.knowledge_updates)} fragments")
