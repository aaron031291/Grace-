...
class TestGraceAPIIntegration(unittest.TestCase):
    ...
    def test_notification_priority_enum_parsing(self):
        ...
        self.assertEqual(
            enum_from_str(NotificationPriority, "high"), NotificationPriority.HIGH
        )
        self.assertEqual(
            enum_from_str(NotificationPriority, "LOW"), NotificationPriority.LOW
        )

        # Test with default
        result = enum_from_str(
            NotificationPriority, "invalid", default=NotificationPriority.MEDIUM
        )
    ...
    def test_error_response_structure(self):
        ...
        error = create_error_response(
            "TEST_ERROR", "Test message", "Additional details"
        )
    ...
    def test_websocket_message_validation(self):
        ...
        content_size_check = validate_request_size(
            small_msg.get("content", ""), max_size_mb=1
        )
        ...
        content_size_check = validate_request_size(
            large_msg.get("content", ""), max_size_mb=1
        )
    ...
    def test_api_error_consistency(self):
        ...
        errors = [
            create_error_response(
                "INVALID_PANEL_TYPE", "Invalid panel type", "panel_type: unknown"
            ),
            create_error_response(
                "SESSION_NOT_FOUND", "Session not found", "session_id: test123"
            ),
            create_error_response(
                "CONTENT_TOO_LARGE", "Content too large", "size: 5MB"
            ),
