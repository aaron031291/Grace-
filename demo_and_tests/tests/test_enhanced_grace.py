...
async def test_vector_database_integration():
    ...
    try:
        from grace.memory.vector_db import VectorMemoryCore, MilvusVectorDB, EmbeddingService
        ...
        test_content = "This is a test governance decision about AI ethics and transparency."
        ...
        print(f"✅ Vector database stats: {stats['search_count']} searches, {stats['insert_count']} inserts")
...
async def test_quantum_safe_storage():
    ...
    try:
        from grace.memory.quantum_safe_storage import QuantumSafeStorageLayer, QuantumSafeKeyManager
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if rotation_result["status"] == "success":
                print(f"✅ Storage keys rotated: {rotation_result['keys_rotated']} keys")
            ...
            print(f"✅ Quantum-safe storage stats: {stats['write_count']} writes, {stats['read_count']} reads")
...
async def test_enhanced_memory_bridge():
    ...
    try:
        ...
        config = {
            ...
            "quantum_storage": {"enabled": True, "storage_path": "/tmp/test_quantum_storage"},
            "routing": {
        ...
        search_result = await bridge.search_enhanced(
            query="governance decision approval",
            search_type="hybrid",
            top_k=5
        )
        
        if search_result["status"] == "success":
            print(f"✅ Enhanced search completed: {search_result['total_results']} results")
        else:
...
async def test_enhanced_ml_specialists():
    ...
    try:
        ...
        gnn_data = {
            ...
            "relationships": [
                {"source": "ethics_board", "target": "ai_system", "type": "oversight", "strength": 0.8},
                {"source": "technical_team", "target": "ai_system", "type": "development", "strength": 0.9}
            ]
        }
        
        gnn_prediction = await gnn_specialist.predict_with_uncertainty(gnn_data)
        if gnn_prediction.confidence > 0.0:
            print(f"✅ GNN specialist prediction: {gnn_prediction.confidence:.2f} confidence")
        else:
        ...
        multimodal_prediction = await multimodal_specialist.predict_with_uncertainty(multimodal_data)
        if multimodal_prediction.confidence > 0.0:
            print(f"✅ Multimodal specialist prediction: {multimodal_prediction.confidence:.2f} confidence")
        else:
        ...
        uncertainty_prediction = await uncertainty_specialist.predict_with_uncertainty(uncertainty_data)
        if uncertainty_prediction.confidence > 0.0:
            print(f"✅ Uncertainty specialist prediction: {uncertainty_prediction.uncertainty:.2f} uncertainty")
        else:
...
async def test_cross_domain_validators():
    ...
    try:
        from grace.mldl.specialists.enhanced_specialists import (
            CrossDomainValidator, SpecialistPrediction, create_cross_domain_validators
        )
        ...
        mock_predictions = [
            SpecialistPrediction(
                ...
                evidence=[{"type": "technical_analysis", "quality": 0.9, "consistency": 0.8}],
            ...
            SpecialistPrediction(
                ...
                evidence=[{"type": "ethical_analysis", "quality": 0.8, "consistency": 0.9}],
        ...
        for validator in validators:
            try:
                ...
                print(f"✅ {validator.domain} validation: {validation.validation_score:.2f} score")
                
                if validation.red_flags:
                    print(f"🚩 {validator.domain} red flags: {', '.join(validation.red_flags)}")
        ...
        for validator in validators:
            ...
            print(f"✅ {validator.domain} validator stats: {stats['total_validations']} validations")
...
async def test_enhanced_governance_liaison():
    ...
    try:
        ...
        test_request = {
            ...
            "relationships": [
                {"source": "ai_system", "target": "users", "type": "serves", "strength": 0.9}
            ],
            "content": {
        ...
        if governance_result["status"] == "success":
            ...
            print(f"   - Specialist predictions: {governance_result['specialist_predictions']}")
            print(f"   - Consensus reached: {governance_result['consensus']['consensus_reached']}")
            print(f"   - Validation passed: {governance_result['validation']['validation_passed']}")
            print(f"   - Hallucination risk: {governance_result['hallucination_risk']:.2f}")
            print(f"   - Decision: {governance_result['governance_decision']['decision_type']}")
        else:
        ...
        if governance_result["status"] == "success":
            ...
            if update_result["status"] == "success":
                print(f"✅ Specialist performance updated: {len(update_result['updates'])} specialists")
        ...
        print(f"   - Working specialists: {len([s for s in stats['specialists'].values() if s['initialized']])}")
...
async def test_integration():
    ...
    try:
        ...
        memory_bridge = EnhancedMemoryBridge(config={
            "vector_db": {"enabled": False},
            "quantum_storage": {"enabled": True, "storage_path": "/tmp/integration_test_storage"}
        })
        ...
        governance_result = await governance_liaison.process_governance_request(governance_request)
        ...
        if governance_result["status"] == "success":
            ...
            store_result = await memory_bridge.store_enhanced(
                ...
                constitutional_score=governance_result["consensus"]["consensus_cross_domain_score"],
                sensitivity_score=0.7
            )
            
            if store_result["status"] == "success":
                print(f"✅ Integration test completed successfully")
                print(f"   - Governance decision: {governance_result['governance_decision']['decision_type']}")
                print(f"   - Decision stored: {store_result['content_id']}")
                
                # Test retrieval
                recall_result = await memory_bridge.recall_enhanced(store_result['content_id'])
                if recall_result.get("found"):
