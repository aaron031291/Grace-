...
class ConsensusProposal(BaseModel):
    ...
    title: str = Field(..., min_length=1, max_length=200, description="Brief title of the proposal")
    description: str = Field(..., min_length=10, description="Detailed description of what's being proposed")
    proposer_id: str = Field(..., description="ID of the member who proposed this")
    proposal_type: str = Field(..., description="Type of proposal (policy_change, constitutional_amendment, etc.)")
    
    # Voting configuration
    quorum_type: QuorumType = Field(default=QuorumType.SIMPLE_MAJORITY, description="Required quorum type")
    min_participation: float = Field(default=0.5, ge=0.1, le=1.0, description="Minimum participation rate")
    voting_deadline: datetime = Field(..., description="When voting closes")
    
    # Content and context
    rationale: str = Field(..., min_length=10, description="Justification for the proposal")
    impact_assessment: Optional[str] = Field(None, description="Assessment of potential impacts")
    alternatives_considered: Optional[List[str]] = Field(default_factory=list, description="Alternative approaches considered")
    ...
    constitutional_review_required: bool = Field(default=True, description="Whether constitutional review is required")
    transparency_level: str = Field(default="democratic_oversight", description="Audit transparency level")
...
class ConsensusResult(BaseModel):
    ...
    def participation_rate_valid(cls, v, values):
        ...
        if abs(v - expected_rate) > 0.01:  # Allow small floating point differences
            raise ValueError(f'Participation rate {v} does not match calculated rate {expected_rate}')
        return v
...
class QuorumConsensusEngine:
    ...
    async def submit_proposal(self, proposal: ConsensusProposal) -> str:
        ...
        logger.info(f"Submitted proposal for consensus: {proposal.title} ({proposal.proposal_id})")
    ...
    async def cast_vote(self, proposal_id: str, member_id: str, vote_type: VoteType,
                       reasoning: Optional[str] = None, confidence: float = 1.0) -> bool:
        ...
        existing_votes = [v for v in self.votes[proposal_id] if v.member_id == member_id]
        if existing_votes:
    ...
    async def calculate_result(self, proposal_id: str) -> ConsensusResult:
        ...
        approval_percentage = weighted_counts[VoteType.APPROVE] / total_weighted if total_weighted > 0 else 0
        rejection_percentage = weighted_counts[VoteType.REJECT] / total_weighted if total_weighted > 0 else 0
        ...
        if quorum_met:
            if proposal.quorum_type == QuorumType.SIMPLE_MAJORITY:
                ...
                consensus_achieved = approval_percentage == 1.0 and vote_counts[VoteType.ABSTAIN] == 0
    ...
    async def _check_early_closure(self, proposal_id: str) -> bool:
        ...
        if len(votes) > 0:
            ...
            if (all_approve or all_reject) and len(votes) >= len(active_members) * proposal.min_participation:
    ...
    async def _notify_members_of_proposal(self, proposal: ConsensusProposal) -> None:
        ...
        logger.info(f"Notifying {len(self.members)} members of new proposal: {proposal.title}")
    ...
    def get_consensus_statistics(self) -> Dict[str, Any]:
        ...
        approved = len([r for r in self.results.values() if r.status == ConsensusStatus.APPROVED])
        rejected = len([r for r in self.results.values() if r.status == ConsensusStatus.REJECTED])
        
        avg_participation = sum(r.participation_rate for r in self.results.values()) / total_proposals if total_proposals > 0 else 0
