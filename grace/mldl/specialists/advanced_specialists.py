...
class SpecialistPerformanceMetrics:
    ...
    def update_performance(self, accuracy: float, calibration: float, response_time: float):
        """Update performance metrics with decay."""
        # Apply temporal decay to historical data
        decay_factor = self.trust_decay_factor
        self.accuracy_history = [score * decay_factor for score in self.accuracy_history[-50:]]
        self.calibration_history = [score * decay_factor for score in self.calibration_history[-50:]]
        self.response_time_history = [time * 1.05 for time in self.response_time_history[-50:]]
    ...
    def get_current_trust_score(self) -> float:
        ...
        base_trust = np.mean(self.accuracy_history[-5:]) if self.accuracy_history else 0.5
        calibration_bonus = np.mean(self.calibration_history[-5:]) * 0.2 if self.calibration_history else 0.0
...
class AdvancedMLSpecialist(ABC):
    ...
    def __init__(self, specialist_id: str, domain: str, capabilities: List[SpecialistCapability]):
    ...
    @abstractmethod
    async def predict_with_reasoning(self, data: Dict[str, Any], 
                                   context: Optional[Dict[str, Any]] = None) -> AdvancedPrediction:
        """Make prediction with full reasoning chain."""
        pass
    
    async def assess_uncertainty(self, data: Dict[str, Any], 
                               prediction: Any) -> Tuple[float, List[str]]:
    ...
    def _check_domain_alignment(self, data: Dict[str, Any]) -> float:
        ...
        matches = sum(1 for keyword in domain_keywords if keyword.lower() in combined_text)
    ...
    async def _check_temporal_stability(self, data: Dict[str, Any]) -> float:
        ...
        recent_accuracy = self.performance_metrics.accuracy_history[-5:] if self.performance_metrics.accuracy_history else [0.5]
...
class ConstitutionalReasoningSpecialist(AdvancedMLSpecialist):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.domain_knowledge = {
                "keywords": ["ethics", "rights", "fairness", "transparency", "accountability", 
                           "privacy", "dignity", "harm", "benefit", "autonomy", "consent"],
                "risk_indicators": ["bias", "discrimination", "manipulation", "deception", 
                                  "privacy_violation", "harm", "unfairness"],
                "compliance_factors": ["documentation", "consent", "transparency", "oversight"]
            }
            ...
            logger.error(f"Failed to initialize Constitutional Reasoning Specialist: {e}")
            return False
    ...
    async def predict_with_reasoning(self, data: Dict[str, Any], 
        ...
        for principle, config in self.constitutional_principles.items():
            ...
            if config["mandatory"] and score < self.constitutional_threshold:
                risk_factors.append(f"Mandatory principle '{principle}' below threshold: {score:.3f}")
        ...
        total_weight = sum(config["weight"] for config in self.constitutional_principles.values())
        ...
        final_decision = "compliant" if (passes_threshold and mandatory_pass) else "non_compliant"
        
        # Assess uncertainty
        uncertainty, additional_risks = await self.assess_uncertainty(data, final_decision)
    ...
    async def _assess_principle_compliance(self, principle: str, data: Dict[str, Any], 
        ...
        if principle == "human_dignity":
            ...
            dignity_indicators = ["respect", "dignity", "rights", "person", "individual"]
            dignity_violations = ["dehumanize", "objectify", "degrade", "humiliate"]
            
            positive_score = sum(0.2 for indicator in dignity_indicators if indicator in content)
            negative_score = sum(0.3 for violation in dignity_violations if violation in content)
            ...
            positive_score = sum(0.15 for indicator in fairness_indicators if indicator in content)
            negative_score = sum(0.25 for violation in bias_indicators if violation in content)
            base_score = 0.75 + positive_score - negative_score
            
        elif principle == "transparency":
            # Check for transparency and explainability
            transparency_indicators = ["transparent", "explain", "clear", "open", "visible"]
            opacity_indicators = ["hidden", "secret", "opaque", "mysterious"]
            
            positive_score = sum(0.15 for indicator in transparency_indicators if indicator in content)
            negative_score = sum(0.2 for violation in opacity_indicators if violation in content)
            base_score = 0.8 + positive_score - negative_score
            
        elif principle == "accountability":
            # Check for accountability measures
            accountability_indicators = ["responsible", "accountable", "oversight", "monitoring"]
            
            positive_score = sum(0.2 for indicator in accountability_indicators if indicator in content)
    ...
    def _calculate_evidence_strength(self, data: Dict[str, Any], 
                                   context: Optional[Dict[str, Any]]) -> float:
