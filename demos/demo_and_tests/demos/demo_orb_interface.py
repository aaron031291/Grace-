...
class OrbDemo:
        ...
        for message in chat_messages:
            try:
                ...
                response = requests.get(
                    f"{self.base_url}/api/orb/v1/chat/{self.session_id}/history?limit=2"
                )
    ...
    def demo_panels(self):
        ...
        for panel_type in panel_types:
            try:
                response = requests.post(
                    f"{self.base_url}/api/orb/v1/panels/create",
                    json={
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/orb/v1/panels/{self.session_id}"
            )
    ...
    def demo_ide(self):
        ...
        try:
            # Open IDE panel
            response = requests.post(
                f"{self.base_url}/api/orb/v1/ide/panels/{self.session_id}"
            )
            result = response.json()
            ...
            # Create a new flow
            response = requests.post(
                f"{self.base_url}/api/orb/v1/ide/flows",
                json={
            ...
            for i, block_type in enumerate(block_types):
                if block_type in blocks["blocks"]:
                    response = requests.post(
                        f"{self.base_url}/api/orb/v1/ide/flows/blocks",
                        json={
                            "flow_id": flow_id,
            ...
            print(
                f"📋 Flow '{flow_details['name']}' has {len(flow_details['blocks'])} blocks"
            )
...
"""
            ...
            with open(temp_file, "rb") as f:
                ...
                response = requests.post(
                    f"{self.base_url}/api/orb/v1/memory/upload", files=files, data=data
                )
                result = response.json()
            ...
            for query in search_queries:
                response = requests.post(
                    f"{self.base_url}/api/orb/v1/memory/search",
    ...
    def demo_governance(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/orb/v1/governance/tasks",
                json={
            ...
            # Get tasks for user
            response = requests.get(
                f"{self.base_url}/api/orb/v1/governance/tasks/{self.user_id}"
            )
            tasks = response.json()
    ...
    def demo_notifications(self):
        ...
        try:
            ...
            for notif_data in notifications:
                ...
                response = requests.post(
                    f"{self.base_url}/api/orb/v1/notifications", json=notif_data
                )
                result = response.json()
                created_notifications.append(result["notification_id"])
                print(
                    f"✅ Created {notif_data['priority']} notification: {result['notification_id']}"
                )

            # Get user notifications
            response = requests.get(
                f"{self.base_url}/api/orb/v1/notifications/{self.user_id}"
            )
    ...
    def demo_stats(self):
        ...
        try:
            ...
            print(
                f"   Intelligence: {stats['intelligence']['domain_pods']} domain pods"
            )
