...
class GraceIntegratedService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"Starting event bus with transport: {self.event_config.transport_type}"
            )
            self.event_bus = GraceEventBus(
                transport_config={
                    "type": self.event_config.transport_type,
                    "config": self.event_config.transport_config,
                }
            )
            ...
            self.governance_api = GovernanceAPIService(
                event_bus=self.event_bus, immutable_logger=self.audit_logger
            )
        ...
        async def handle_memory_event(gme: GraceMessageEnvelope):
            ...
            if event_type == EventTypes.MEMORY_WRITE_COMPLETED:
                ...
                if content_key and "critical" in payload.get("tags", []):
                    ...
                    logger.info(
                        f"Auto-created snapshot: {snapshot_result.get('snapshot_id')}"
                    )
    ...
    def _register_demo_routes(self):
        ...
        async def root():
            ...
            return {
                ...
                "transport": self.event_config.transport_type
        ...
        async def health():
            ...
            try:
                ...
                if self.snapshot_manager:
                    health_data["components"]["snapshot_manager"] = {
                        "status": "healthy"
                    }
        ...
        async def demo_workflow():
            ...
            try:
                ...
                if self.governance_api:
                    ...
                    logger.info(
                        f"Would create governance request for workflow {workflow_id}"
                    )
        ...
        async def list_demo_snapshots():
            ...
            if not self.snapshot_manager:
                raise HTTPException(
                    status_code=503, detail="Snapshot manager not available"
                )
