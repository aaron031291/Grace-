...
class IngressService:
    ...
    def _setup_routes(self):
        ...
        @self.app.post(
            "/api/ingress/v1/sources", response_model=SourceRegistrationResponse
        )
        async def register_source(request: SourceRegistrationRequest):
            ...
            try:
                ...
                # Build configuration
                config = {"source_id": source_id, **request.dict()}
        ...
        async def capture_data(
            request: CaptureRequest, background_tasks: BackgroundTasks
        ):
        ...
        @self.app.post(
            "/api/ingress/v1/snapshot/export", response_model=SnapshotResponse
        )
        async def export_snapshot():
            ...
            try:
                ...
                return SnapshotResponse(snapshot_id=snapshot.snapshot_id, uri=uri)
        ...
        async def get_metrics(
            source_id: Optional[str] = None, since: Optional[str] = None
        ):
            """Get ingress metrics."""
            try:
                # Mock metrics data
                ...
                return MetricsResponse(timeseries=timeseries, summary=summary)
