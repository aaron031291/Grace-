...
class EventMeshConfig:
    ...
    def from_env(cls) -> "EventMeshConfig":
        ...
        return cls(
            ...
            enable_deduplication=os.getenv("GRACE_EVENT_DEDUP", "true").lower()
            == "true",
            ...
            enable_tracing=os.getenv("GRACE_TRACING_ENABLED", "false").lower()
            == "true",
        ...
        if transport_type == "kafka":
            return {
                ...
                "connection_timeout": float(
                    os.getenv("KAFKA_CONNECTION_TIMEOUT", "30.0")
                ),
                "retry_attempts": int(os.getenv("KAFKA_RETRY_ATTEMPTS", "3")),
                "heartbeat_interval": float(
                    os.getenv("KAFKA_HEARTBEAT_INTERVAL", "30.0")
                ),
            }
            ...
            return {
                ...
                "connection_timeout": float(
                    os.getenv("NATS_CONNECTION_TIMEOUT", "30.0")
                ),
                "retry_attempts": int(os.getenv("NATS_RETRY_ATTEMPTS", "3")),
                "heartbeat_interval": float(
                    os.getenv("NATS_HEARTBEAT_INTERVAL", "30.0")
                ),
            }
            ...
            return {
                ...
                "connection_timeout": float(
                    os.getenv("REDIS_CONNECTION_TIMEOUT", "30.0")
                ),
                "retry_attempts": int(os.getenv("REDIS_RETRY_ATTEMPTS", "3")),
                "heartbeat_interval": float(
                    os.getenv("REDIS_HEARTBEAT_INTERVAL", "30.0")
                ),
            }
            ...
            return {
                ...
                "connection_timeout": float(
                    os.getenv("EVENT_CONNECTION_TIMEOUT", "30.0")
                ),
                "retry_attempts": int(os.getenv("EVENT_RETRY_ATTEMPTS", "3")),
                "heartbeat_interval": float(
                    os.getenv("EVENT_HEARTBEAT_INTERVAL", "30.0")
                ),
            }
