...
class MemoryBridge:
        ...
        try:
            if self.mtl_kernel and hasattr(self.mtl_kernel, "librarian"):
                ...
                results = self.mtl_kernel.librarian.search_and_rank(
                    query=query, limit=filters.get("limit", 20) if filters else 20
                )

                # Apply governance filtering based on user access
                filtered_results = await self._apply_governance_filters(
                    results, user_id, filters
                )
    ...
    async def _apply_governance_filters(
        self, results: List, user_id: str, filters: Optional[Dict]
    ) -> List:
    ...
    async def distill_content(
        self, results: List[Any], context: str, user_id: str
    ) -> Optional[str]:
    ...
    def get_search_history(self, user_id: str, limit: int = 10) -> List[Dict]:
        ...
        user_searches = [s for s in self.search_history if s.get("user_id") == user_id]
