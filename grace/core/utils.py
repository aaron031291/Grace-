...
def enum_from_str(enum_class: Type[EnumT], value: str, *, default: Optional[EnumT] = None) -> EnumT:
    """
    ...
    if not isinstance(value, str):
        if default is not None:
            return default
        raise ValueError(f"Expected string value for enum parsing, got {type(value)}: {value}")
...
def create_error_response(code: str, message: str, detail: Optional[str] = None, 
...
def validate_request_size(content: Any, max_size_mb: int = 10) -> Optional[Dict[str, Any]]:
    """
