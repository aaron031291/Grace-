...
class QdrantVectorStore(VectorStore):
    ...
    def __init__(
        self, url: str = "http://localhost:6333", collection_name: str = "grace_memory"
    ):
        self.url = url
    ...
    async def create_collection(
        ...
        try:
            ...
            logger.info(
                f"Created collection {collection_name} with dimension {vector_dimension}"
            )
            return True
...
class VectorSearchService:
    ...
    async def index_content(
        self, content_id: str, vector: List[float], metadata: Dict[str, Any]
    ) -> bool:
        """Index content with vector and metadata."""
        vector_data = {"id": content_id, "vector": vector, "metadata": metadata}
    ...
    async def search_content(
        ...
        logger.info(
            f"Search returned {len(filtered_results)} results (trust >= {trust_threshold})"
        )
