...
class MemorySearchRequest(BaseModel if FASTAPI_AVAILABLE else object):
    ...
    min_constitutional_score: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum constitutional score"
    )
...
class GraceMemoryAPI:
    ...
    def __init__(
        ...
        self.fusion = fusion_memory or FusionMemory(
            storage_path="/tmp/grace_fusion_memory", enable_compression=True
        )

        self.librarian = librarian or EnhancedLibrarian(
            lightning_memory=self.lightning, fusion_memory=self.fusion
        )
        ...
        if FASTAPI_AVAILABLE:
            ...
            async def http_exception_handler(request: Request, exc: HTTPException):
                return await GraceHTTPExceptionHandler.handle_http_exception(
                    request, exc
                )
    ...
    def _register_routes(self):
        ...
        async def write_memory(request: MemoryWriteRequest):
            ...
            try:
                ...
                if (
                    ...
                    cache_key = (
                        request.source_id
                        or f"direct_{utc_timestamp().replace(':', '').replace('-', '').replace('.', '')[:16]}"
                    )
                ...
                logger.error(
                    f"Memory write failed: {e}", extra={"request_id": get_request_id()}
                )
                return JSONResponse(
                    status_code=500,
                    content=create_error_response(
                        "MEMORY_WRITE_FAILED", "Failed to write to memory", str(e)
                    ),
                )
        ...
        async def read_memory(request: MemoryReadRequest):
            ...
            try:
                ...
                content = self.lightning.get(
                    request.key, verify_integrity=request.verify_integrity
                )
                ...
                return MemoryResponse(
                    success=False, message=f"Content not found for key: {request.key}"
                )
        ...
        async def get_memory_stats():
            ...
            try:
                ...
                return MemoryResponse(
                    ...
                    data={
                        ...
                        "overall_health": {
                            ...
                            "total_memory_entries": lightning_stats["entries"]
                            + fusion_stats.get("active_entries", 0),
                        },
                    },
                )
                ...
                return JSONResponse(
                    status_code=500,
                    content=create_error_response(
                        "MEMORY_STATS_FAILED", "Failed to get memory statistics", str(e)
                    ),
                )

        @self.app.post(
            "/api/memory/v1/document/{source_id}", response_model=MemoryResponse
        )
        async def get_document_info(source_id: str):
            ...
            try:
                ...
                if doc_info:
                    ...
                    return MemoryResponse(
                        success=False, message=f"Document not found: {source_id}"
                    )
        ...
        async def verify_integrity():
            ...
            try:
                ...
                overall_healthy = (
                    fusion_integrity.get("corrupted_entries", 0) == 0
                    and cache_health["healthy"]
                )
    ...
    async def search_content(
        self, query: str, limit: int = 10, min_constitutional_score: float = 0.7
    ) -> List[Dict[str, Any]]:
        """Search content (standalone method)."""
        return self.librarian.search_and_rank(
            query=query, limit=limit, min_constitutional_score=min_constitutional_score
        )
