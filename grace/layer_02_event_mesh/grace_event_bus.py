...
from .transports import EventTransport, create_transport
...
class GraceEventBus:
    ...
    async def _message_processor(self, worker_name: str):
        ...
        while self.running:
            try:
                ...
                gme = await asyncio.wait_for(self.message_queue.get(), timeout=1.0)
    ...
    async def _schedule_retry(self, gme: GraceMessageEnvelope):
        ...
        logger.info(
            f"Scheduling retry {gme.retry_count} for message {gme.msg_id} in {delay:.2f}s"
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "subscribers": {
                pattern: len(handlers) for pattern, handlers in self.subscribers.items()
            },
            "registered_schemas": len(self.schema_registry),
            "dlq_stats": self.dlq.get_stats(),
            "deduplication_cache_size": len(self.seen_messages),
            "transport": asyncio.create_task(self.transport.health_check())
    ...
    def get_health(self) -> Dict[str, Any]:
        ...
        is_healthy = (
            self.running
            and queue_size < self.backpressure_config.high_water_mark
            and len(self.processing_workers) > 0
        )
