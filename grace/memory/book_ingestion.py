...
class BookIngestionService:
    ...
    async def ingest_book(
        ...
        try:
            logger.info(
                f"Starting book ingestion: {title} (estimated {estimated_pages} pages)"
            )
            ...
            for i, chapter in enumerate(chapters):
                ...
                if i % 5 == 0:  # Log progress every 5 chapters
                    logger.info(
                        f"Book ingestion progress: {progress.chapters_processed}/{progress.total_chapters} chapters"
                    )
            ...
            logger.info(
                f"Book ingestion completed: {title} ({len(chapters)} chapters, {len(insights)} insights)"
            )
            return result
    ...
    async def _detect_chapters(self, content: str, title: str) -> List[BookChapter]:
        ...
        if not chapter_starts:
            logger.info(
                "No chapters found with strict patterns, trying permissive patterns"
            )
        ...
        for i, (start_line, chapter_title) in enumerate(chapter_starts):
            ...
            if len(chapter_content.split()) < self.config["min_chapter_words"]:
                logger.info(
                    f"Skipping short chapter: {chapter_title} ({len(chapter_content.split())} words)"
                )
    ...
    async def _process_chapter(
        self, chapter: BookChapter, book_title: str
    ) -> List[TextChunk]:
    ...
    async def _extract_chapter_insights(
        self, chapter: BookChapter
    ) -> List[BookInsight]:
    ...
    async def _store_book_insights(
        self, insights: List[BookInsight], job_id: str, title: str
    ):
    ...
    async def _store_chapter_summaries(
        self, chapters: List[BookChapter], job_id: str, title: str
    ):
    ...
    async def search_book_content(
        self, job_id: str, query: str, limit: int = 10
    ) -> List[Dict[str, Any]]:
