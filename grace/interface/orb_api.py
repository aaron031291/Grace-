...
async def create_panel(request: PanelCreateRequest):
    """Create a new panel."""
    try:
        # Centralized enum parsing with clear error handling
...
async def upload_document(file: UploadFile = File(...), user_id: str = "default"):
    ...
    try:
        ...
        # Create temporary file with safe suffix if present
        suffix = ""
        if "." in safe_filename:
    ...
    return {
        "blocks": {
            block_id: {
                ...
                if hasattr(block_info["block_type"], "value")
                else str(block_info["block_type"]),
...
async def websocket_endpoint(
    ...
    logger.info(
        f"WebSocket connected for user {user_id}, session {session_id}, tenant {tenant_id}",
    ...
    try:
        while True:
            ...
            if message_type == "chat_message":
                ...
                try:
                    # Validate message content size (smaller for WS)
