...
class InMemoryTransport(EventTransport):
    ...
    async def disconnect(self):
        ...
        if self.subscription_tasks:
            await asyncio.gather(
                *self.subscription_tasks.values(), return_exceptions=True
            )
    ...
    async def _consume_messages(
        self, queue: asyncio.Queue, handler: Callable, subscription_id: str
    ):
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.error(
                        f"Handler error in subscription {subscription_id}: {e}"
                    )
...
class KafkaTransport(EventTransport):
    ...
    async def subscribe(self, topic_pattern: str, handler: Callable) -> str:
        ...
        try:
            ...
            logger.info(
                f"Subscribed to Kafka pattern {topic_pattern} (ID: {subscription_id})"
            )
            return subscription_id
    ...
    async def unsubscribe(self, topic_pattern: str, subscription_id: str):
        ...
        logger.info(
            f"Unsubscribed from Kafka pattern {topic_pattern} (ID: {subscription_id})"
        )

    async def _consume_kafka_messages(
        self, consumer, handler: Callable, subscription_id: str
    ):
        ...
        try:
                ...
                for topic_partition, messages in message_batch.items():
                    for message in messages:
                        try:
                            ...
                            logger.error(
                                f"Handler error in Kafka subscription {subscription_id}: {e}"
                            )
            ...
            logger.error(
                f"Kafka consumer error for subscription {subscription_id}: {e}"
            )
