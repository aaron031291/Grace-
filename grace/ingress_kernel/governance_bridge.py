...
class GovernanceRequest:
    ...
    def __init__(
        self, request_type: str, data: Dict[str, Any], context: Optional[Dict] = None
    ):
...
class IngressGovernanceBridge:
    ...
    async def validate_source_registration(
    ...
    async def validate_record_processing(self, record: NormRecord) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"approved": True, "reason": "All policies satisfied"}

        except Exception as e:
            logger.error(f"Record validation failed: {e}")
            return {"approved": False, "reason": f"Validation error: {e}"}

    async def request_policy_update(
        self, policy_type: str, current_value: Any, proposed_value: Any
    ) -> Dict[str, Any]:
        """
        ...
        try:
            request = GovernanceRequest(
                ...
                context={"source": "ingress_kernel", "priority": "normal"},
            )
            ...
            return {"approved": False, "reason": f"Request error: {e}"}
    ...
    async def _requires_approval(self, source_config: SourceConfig) -> bool:
        ...
        if (
            source_config.pii_policy != "block"
            and "external" in source_config.uri.lower()
        ):
            return True
    ...
    async def _auto_approve_source(self, source_config: SourceConfig) -> Dict[str, Any]:
        ...
        return {
            ...
            "conditions": ["Monitor for first 30 days", "Review if error rate > 5%"],
        }
    ...
    async def _check_governance_compliance(self, record: NormRecord) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "passed": True,
                "details": f"Governance compliance: {governance_label}",
            }
    ...
    async def _refresh_policy_cache(self):
        ...
        try:
            ...
            expired_keys = [
                k for k, expiry in self.cache_expiry.items() if current_time > expiry
            ]
