...
TEST_CASES = {
    ...
    "edge_cases": [
        {"text": "", "expected_intent": "unknown", "expected_sentiment": "neutral"},
        {
...
class EliteNLPTester:
    ...
    async def test_elite_nlp_specialist(self):
        ...
        try:
            ...
            logger.info(
                f"      - Sentiment: {analysis.sentiment.get('label')} (confidence: {analysis.sentiment.get('confidence', 0):.2f})"
            )
    ...
    async def test_enhanced_w5h_indexer(self):
        ...
        try:
            ...
            logger.info(
                f"      - Intent: {intent_analysis.get('primary_intent')} (confidence: {intent_analysis.get('confidence', 0):.2f})"
            )
    ...
    async def test_toxicity_detection(self):
        ...
        try:
            ...
            logger.info(
                f"      - Toxic text score: {toxic_result['toxicity_score']:.2f}"
            )
    ...
    async def test_conversation_management(self):
        ...
        try:
            ...
            logger.info(
                f"      - Turn progression: {context1.turn_number} -> {context2.turn_number}"
            )
                ...
                try:
                    ...
                    if "expected_sentiment" in test_case:
                        ...
                        if actual_sentiment != test_case["expected_sentiment"]:
                            logger.warning(
                                f"      Sentiment mismatch: expected {test_case['expected_sentiment']}, got {actual_sentiment}"
                            )

                    if "expected_toxicity" in test_case:
                        toxicity_result = await self.elite_nlp.detect_toxicity(text)
                        if (
                            toxicity_result["is_toxic"]
                            != test_case["expected_toxicity"]
                        ):
                            logger.warning(
                                f"      Toxicity mismatch: expected {test_case['expected_toxicity']}, got {toxicity_result['is_toxic']}"
                            )
