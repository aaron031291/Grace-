...
from grace.core.models import User, Session, Memory, MemoryEmbedding, SystemOperation, BackgroundTask
...
class BaseRepository(ABC, Generic[T]):
    ...
    async def list(self, 
                   offset: int = 0, 
                   limit: int = 100,
                   filters: Optional[Dict[str, Any]] = None) -> Sequence[T]:
...
class SessionRepository(SQLAlchemyRepository[Session]):
    ...
    async def get_user_sessions(self, user_id: str, active_only: bool = False) -> Sequence[Session]:
    ...
    async def deactivate_session(self, token_id: str) -> bool:
        ...
        stmt = update(Session).where(Session.token_id == token_id).values(
            is_active=False
        )
        result = await self.session.execute(stmt)
        return result.rowcount > 0
...
class MemoryRepository(SQLAlchemyRepository[Memory]):
    ...
    async def get_by_tags(self, user_id: str, tags: List[str], limit: int = 50) -> Sequence[Memory]:
...
class MemoryEmbeddingRepository(SQLAlchemyRepository[MemoryEmbedding]):
    ...
    async def get_by_model(self, embedding_model: str, limit: int = 1000) -> Sequence[MemoryEmbedding]:
...
class SystemOperationRepository(SQLAlchemyRepository[SystemOperation]):
    ...
    async def log_operation(self, 
                           ...
                           user_agent: Optional[str] = None) -> SystemOperation:
    ...
    async def get_user_operations(self, 
                                 ...
                                 limit: int = 100) -> Sequence[SystemOperation]:
...
class BackgroundTaskRepository(SQLAlchemyRepository[BackgroundTask]):
    ...
    async def get_pending_tasks(self, task_type: Optional[str] = None, limit: int = 10) -> Sequence[BackgroundTask]:
    ...
    async def update_task_status(self, 
                                ...
                                error_message: Optional[str] = None) -> Optional[BackgroundTask]:
