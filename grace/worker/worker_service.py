...
TASKS_PROCESSED = Counter(
    "grace_worker_tasks_total", "Total tasks processed", ["queue", "status"]
)
TASK_DURATION = Histogram(
    "grace_worker_task_duration_seconds", "Task processing duration", ["queue"]
)
...
class GraceWorker:
    ...
    def __init__(self):
        ...
        self.queue_semaphores = {
            q: asyncio.Semaphore(getattr(self.settings, f"queue_{q}_concurrency", 5))
        ...
        while self.running:
            try:
                ...
                result = await self.redis_client.blpop(
                    f"grace:queue:{queue_name}", timeout=5
                )
                if result is None:
                ...
                logger.error(
                    f"Error in queue processor for {queue_name}: {e}", exc_info=True
                )
    ...
    async def _process_task(self, queue_name: str, task_data: str):
        ...
        try:
            ...
            if task_id:
                ...
                if self.redis_client:
                    ...
                    if retries >= MAX_RETRIES:
                        ...
                        logger.error(
                            f"Poison-pill detected for task {task_id} (retries={retries}), quarantined."
                        )
                        TASKS_PROCESSED.labels(
                            queue=queue_name, status="quarantined"
                        ).inc()
                        return
            ...
            if self.redis_client:
                ...
                if exists:
                    logger.info(
                        f"Duplicate task detected (fingerprint: {fingerprint}), skipping."
                    )
            ...
            logger.info(
                f"Processing task {task_id} of type {task_type} from queue {queue_name}"
            )
    ...
    async def _process_embeddings_task(self, task: dict):
        ...
        try:
            ...
            if self.redis_client and "task_id" in task:
                result_key = f"grace:task_result:{task['task_id']}"
                result = {"status": "success", "embeddings": embedding_results}
                await self.redis_client.setex(
                    result_key, 3600, str(result)
                )  # 1 hour TTL
