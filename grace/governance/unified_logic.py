...
class UnifiedLogic:
    ...
    async def synthesize_decision(
        ...
        topic: str,
        inputs: Dict[str, Any],
        mldl_consensus_id: Optional[str] = None,
    ) -> UnifiedDecision:
        """
        ...
        try:
            ...
            decision = UnifiedDecision(
                ...
                inputs={
                    ...
                    "component_signals": [
                        signal.to_dict() for signal in arbitrated_signals
                    ],
            ...
            logger.info(
                f"Synthesized decision {decision_id}: {recommendation} (conf: {confidence:.3f})"
            )
            return decision
    ...
    async def _extract_component_signals(
        ...
        if "trust_scores" in inputs:
            ...
            signals.append(
                ComponentSignal(
                    component="trust_core", signal="average_trust", weight=avg_trust
                )
            )
    ...
    def _convert_mldl_output_to_signal(
        self, specialist_name: str, output: Dict[str, Any]
    ) -> Optional[ComponentSignal]:
        ...
        if "classification" in specialist_name:
            ...
            signal_type = (
                "anomaly_detected" if output.get("is_anomaly", False) else "normal"
            )
        elif "fairness" in specialist_name:
            signal_type = (
                "fairness_compliant" if confidence > 0.7 else "fairness_concern"
            )
    ...
    async def _apply_weights(
        self, signals: List[ComponentSignal]
    ) -> List[ComponentSignal]:
    ...
    async def _arbitrate_conflicts(
        ...
        for component, component_signals in component_groups.items():
            if len(component_signals) == 1:
                ...
                consolidated_signal = ComponentSignal(
                    component=component, signal="consolidated", weight=avg_weight
                )
        ...
        if high_confidence_signals and low_confidence_signals:
            logger.warning(
                f"Detected conflicting signals: "
    ...
    async def _calculate_recommendation(self, signals: List[ComponentSignal]) -> str:
        ...
        for signal in signals:
            if (
                signal.component in critical_components
                and signal.weight < self.decision_thresholds["min_trust_score"]
            ):
                return "reject"
    ...
    async def _generate_rationale(
        self, topic: str, signals: List[ComponentSignal], recommendation: str
    ) -> str:
        """Generate human-readable rationale for the decision."""
        rationale_parts = [f"Decision on '{topic}': {recommendation.upper()}"]
        ...
        if recommendation == "approve":
            rationale_parts.append(
                "All critical compliance checks passed with sufficient confidence."
            )
        elif recommendation == "reject":
            rationale_parts.append(
                "Critical compliance failures or insufficient confidence detected."
            )
        else:  # review
            rationale_parts.append(
                "Mixed signals require human review for final determination."
            )
    ...
    async def _calculate_confidence(self, signals: List[ComponentSignal]) -> float:
        ...
        weighted_confidence = sum(w * c for w, c in zip(weights, confidences)) / sum(
            weights
        )
        return min(max(weighted_confidence, 0.0), 1.0)

    async def _calculate_trust_score(
        self, signals: List[ComponentSignal], inputs: Dict[str, Any]
    ) -> float:
    ...
    def _create_error_decision(
        self, decision_id: str, topic: str, inputs: Dict[str, Any], error_msg: str
    ) -> UnifiedDecision:
    ...
    async def _record_synthesis_experience(
        self,
        input_count: int,
        signal_count: int,
        recommendation: str,
        ...
        experience = Experience(
            ...
            context={
                "inputs": input_count,
                "conflicts": max(
                    0, input_count - signal_count
                ),  # Simplified conflict count
                "processing_time": processing_time,
            },
            outcome={
                "final_decision": recommendation,
                "delta_vs_thresholds": confidence
                - self.decision_thresholds["min_confidence_approve"],
            },
    ...
    async def update_weights(self, weight_updates: Dict[str, float]):
        ...
        for component, new_weight in weight_updates.items():
            if component in self.specialist_weights:
                ...
                logger.info(
                    f"Updated {component} weight: {old_weight:.3f} -> {new_weight:.3f}"
                )
    ...
    async def update_thresholds(self, threshold_updates: Dict[str, float]):
        ...
        for threshold_name, new_value in threshold_updates.items():
            if threshold_name in self.decision_thresholds:
                ...
                logger.info(
                    f"Updated {threshold_name}: {old_value:.3f} -> {new_value:.3f}"
                )
