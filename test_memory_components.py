...
try:
    ...
    def test_lightning_memory():
        ...
        # Test with metadata
        lightning.put("complex_key", {"data": "complex", "number": 42}, tags=["test", "complex"])
        ...
        print(f"✅ Lightning Memory: {stats['entries']} entries, {stats['hit_rate']} hit rate")
        return True
    ...
    def test_fusion_memory():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            entry_id = fusion.write("test_key", {"content": "test data", "timestamp": "2023-01-01"})
            ...
            fusion.write("search_test", {"title": "Test Document", "content": "This is searchable content"}, tags=["test", "document"])
            ...
            print(f"✅ Fusion Memory: {stats['total_entries']} total, {stats['active_entries']} active")
            return True
    ...
    def test_constitutional_filter():
        ...
        assert good_result["constitutional_score"] > 0.5, "Good content should have high score"
        
        # Test problematic content
        bad_content = "This content describes how to harm people through illegal activities."
        ...
        print(f"✅ Constitutional Filter: Good={good_result['constitutional_score']:.2f}, Bad={bad_result['constitutional_score']:.2f}")
        return True
    ...
    def test_enhanced_librarian():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test search functionality
            search_results = librarian.search_and_rank("artificial intelligence", limit=5)
            assert len(search_results) > 0, "Search should return results"
            
            # Verify constitutional scoring
            for result in search_results:
                assert result["constitutional_score"] >= 0.7, "Results should meet constitutional threshold"
            ...
            assert doc_info["total_chunks"] == result["chunks_processed"], "Chunk counts should match"
            ...
            print(f"✅ Enhanced Librarian: {result['chunks_processed']} chunks, {len(search_results)} search results")
            return True
    ...
    async def test_memory_api():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert write_result["status"] == "success", f"API write failed: {write_result}"
            ...
            print(f"✅ Memory API: Write successful, {len(search_results)} search results")
            return True
    ...
    def test_memory_integration():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            documents = [
                ("Grace kernel overview", "The Grace kernel provides autonomous governance for AI systems with constitutional constraints."),
                ("Memory architecture", "The memory system uses Lightning for caching and Fusion for long-term storage."),
                ("Constitutional AI", "Constitutional AI ensures that AI systems operate within defined ethical boundaries.")
            ]
            ...
            assert len(search_results) >= 2, "Should find results across multiple documents"
            
            # Verify cache/storage integration
            lightning_stats = lightning.get_stats()
            fusion_stats = fusion.get_stats()
            
            assert lightning_stats["entries"] > 0, "Lightning should have cached entries"
            assert fusion_stats["total_entries"] > 0, "Fusion should have stored entries"
            ...
            print(f"✅ Memory Integration: {len(documents)} docs, {len(search_results)} results, cache/storage working")
            return True
