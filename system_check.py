...
class GraceSystemHealthChecker:
    ...
    async def _check_core_layers(self):
        ...
        for layer_name, module_path, class_name in layers_to_check:
            status = await self._check_component_import(
                layer_name, module_path, class_name
            )
    ...
    async def _check_governance_components(self):
        ...
        components_to_check = [
            (
                "GovernanceKernel",
                "grace.governance.grace_governance_kernel",
                "GraceGovernanceKernel",
            ),
            (
                "VerificationEngine",
                "grace.governance.verification_engine",
                "VerificationEngine",
            ),
            ...
            (
                "GovernanceAPI",
                "grace.governance.governance_api",
        ...
        for comp_name, module_path, class_name in components_to_check:
            status = await self._check_component_import(
                comp_name, module_path, class_name
            )
            self.report.components.append(status)
    ...
    async def _check_vault_systems(self):
        ...
        for vault_name, module_path, class_name in vault_systems:
            status = await self._check_component_import(
                vault_name, module_path, class_name
            )
            self.report.components.append(status)
    ...
    async def _check_telemetry_systems(self):
        ...
        telemetry_systems = [
            ...
            (
                ...
                "grace.orchestration.orchestration_service",
                "OrchestrationService",
            ),
        ]
        ...
        for telem_name, module_path, class_name in telemetry_systems:
            status = await self._check_component_import(
                telem_name, module_path, class_name
            )
            self.report.components.append(status)
    ...
    async def _test_ooda_loop(self):
        ...
        try:
            # Import scheduler
            from grace.orchestration.scheduler.scheduler import (
                Scheduler,
                LoopDefinition,
            )
            ...
            status = ComponentStatus(
                name="OODA_Loop", status="unhealthy", details={}, error=str(e)
            )
            self.report.components.append(status)
    ...
    async def _test_contradiction_detection(self) -> ComponentStatus:
        ...
        try:
            ...
            contradictions = await verification_engine._check_contradictions(
                claim1, [claim2]
            )
    ...
    async def _test_decision_narration(self) -> ComponentStatus:
        ...
        try:
            ...
            decision = await unified_logic.synthesize_decision(
                topic="system_health_check", inputs=test_inputs
            )

            if (
                decision
                and hasattr(decision, "recommendation")
            ...
            return ComponentStatus(
                name="Decision_Narration", status="unhealthy", details={}, error=str(e)
            )
    ...
    async def _test_trust_scoring(self) -> ComponentStatus:
        ...
        try:
            ...
            return ComponentStatus(
                name="Trust_Scoring", status="unhealthy", details={}, error=str(e)
            )

    async def _check_component_import(
        self, component_name: str, module_path: str, class_name: str
    ) -> ComponentStatus:
...
def print_health_report(report: SystemHealthReport):
    ...
    for component in sorted(report.components, key=lambda x: x.name):
        ...
        response_time = (
            f" ({component.response_time:.3f}s)" if component.response_time else ""
        )
        print(
            f"{symbol} {component.name:<20} {component.status.upper():<10}{response_time}"
        )
        if component.error:
...
async def main():
    ...
    try:
        ...
        # Save report to file
        report_file = (
            f"system_health_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
