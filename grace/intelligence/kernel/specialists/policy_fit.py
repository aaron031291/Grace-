...
class PolicyFitSpecialist(BaseSpecialist):
    ...
    def _specialist_analysis(
        ...
        reasoning = (
            f"Policy fit score: {avg_policy_fit:.2f}. "
    ...
    def _analyze_policy_fit(
        self, item: QuorumFeedItem, context: Optional[Dict]
    ) -> tuple:
        ...
        negative_score = sum(
            1 for indicator in self.negative_indicators if indicator in content
        )
        ...
        if context:
            ...
            if required_policies:
                policy_mentions = sum(
                    1 for policy in required_policies if policy.lower() in content
                )
