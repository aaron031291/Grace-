...
class GraceIntegrationTester:
    ...
    async def run_comprehensive_tests(self) -> IntegrationTestReport:
        ...
        report = IntegrationTestReport(
            ...
            and health_report.overall_status in ["healthy", "degraded"],
    ...
    async def _test_ooda_loop_full_cycle(self):
        ...
        await self._run_test(
            "OODA Decision Integration", self._test_ooda_decision_integration
        )
    ...
    async def _run_test(self, test_name: str, test_func):
        ...
        try:
            ...
            result = TestResult(
                name=test_name, passed=True, duration=duration, details=details or {}
            )
    ...
    async def _test_memory_core_ops(self) -> Dict[str, Any]:
        ...
        assert any(exp["id"] == "test_exp_1" for exp in experiences), (
            "Test experience not found"
        )
    ...
    async def _test_governance_startup(self) -> Dict[str, Any]:
        ...
        assert status["status"] in ["running", "initialized"], (
    ...
    async def _test_claim_verification(self) -> Dict[str, Any]:
        ...
        test_claim = Claim(
            ...
            logical_chain=[
                LogicStep(
                    step="Integration test execution validates system functionality"
                )
            ],
        )
        ...
        assert hasattr(verification_result, "confidence"), (
            "Missing confidence in verification result"
        )
    ...
    async def _test_decision_synthesis(self) -> Dict[str, Any]:
        ...
        decision = await unified_logic.synthesize_decision(
            topic="integration_test", inputs=test_inputs
        )
    ...
    async def _test_ooda_complete_cycle(self) -> Dict[str, Any]:
        ...
        assert len(result["stages"]) == 4, (
            f"Expected 4 stages, got {len(result['stages'])}"
        )
        ...
        for expected_stage in expected_stages:
            assert expected_stage in stage_names, (
                f"Missing OODA stage: {expected_stage}"
            )
    ...
    async def _test_ooda_event_processing(self) -> Dict[str, Any]:
        ...
        processing_result = {
            ...
            "orient": {
                "severity_assessed": test_event["severity"],
                "component_identified": test_event["component"],
            },
    ...
    async def _test_ooda_decision_integration(self) -> Dict[str, Any]:
        ...
        decision = await unified_logic.synthesize_decision(
            topic="ooda_integration_test", inputs=ooda_inputs
        )

        return {
            "decision_made": decision.decision if decision else "no_decision",
            "decision_confidence": getattr(decision, "confidence", 0.0)
            if decision
            else 0.0,
    ...
    async def _test_mtl_memory_cycle(self) -> Dict[str, Any]:
        ...
        test_entry = MemoryEntry(
            content="Integration test memory entry", content_type="text/plain"
        )
    ...
    async def _test_trust_attestation(self) -> Dict[str, Any]:
        ...
        attestation_id = trust_service.attest(
            memory_id, attestation_delta, "integration_test"
        )
    ...
    async def _test_audit_integrity(self) -> Dict[str, Any]:
        ...
        for action in test_actions:
            log_id = await immutable_logs.log_governance_action(
                action["action"], action["data"], "integration_test"
            )
    ...
    async def _test_immutable_logging(self) -> Dict[str, Any]:
        ...
        log_id = await logger.log_governance_action(
            "test_event", test_log, "integration"
        )
...
def print_test_report(report: IntegrationTestReport):
    ...
    print(
        f"📊 Tests: {report.passed_tests}/{report.total_tests} passed ({report.failed_tests} failed)"
    )
