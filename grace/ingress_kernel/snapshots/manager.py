...
class SnapshotManager:
    ...
    async def create_snapshot(
        self, state: Optional[Dict[str, Any]] = None, manual: bool = False
    ) -> IngressSnapshot:
        """
    ...
    async def should_rollback(
        ...
        try:
            ...
            historical_errors = sum(
                h["metrics"].get("error_rate", 0) for h in self.performance_history[:-5]
            ...
            if recent_errors > historical_errors * error_threshold_multiplier:
                return (
                    True,
                    f"Error rate spike: {recent_errors:.2%} vs {historical_errors:.2%}",
                )

            if recent_latency > historical_latency * latency_threshold_multiplier:
                return (
                    True,
                    f"Latency spike: {recent_latency:.0f}ms vs {historical_latency:.0f}ms",
                )
    ...
    async def _load_snapshot_from_disk(
        self, snapshot_id: str
    ) -> Optional[IngressSnapshot]:
            ...
            if isinstance(data.get("watermarks"), dict):
                for key, value in data["watermarks"].items():
                    if isinstance(value, str):
                        data["watermarks"][key] = datetime.fromisoformat(
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "auto_snapshot_interval_hours": self.auto_snapshot_interval.total_seconds()
            / 3600,
