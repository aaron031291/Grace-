...
class ConstitutionalValidator:
    ...
    async def validate_against_constitution(
        ...
        result = ConstitutionalValidationResult(
            ...
            validation_id=validation_id,
            audit_trail=[
                f"Validated {len(violations)} principles",
    ...
    async def _check_transparency_compliance(
        ...
        transparency_level = (
            context.get("transparency_level", "internal") if context else "internal"
        )
    ...
    async def _check_fairness_compliance(
        self, action: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> Optional[ConstitutionalViolation]:
    ...
    async def _check_accountability_compliance(
        self, action: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> Optional[ConstitutionalViolation]:
        ...
        if context and context.get("action_type") == "autonomous":
            if (
                not context.get("has_human_oversight")
                and action.get("risk_level") == "high"
            ):
    ...
    async def _check_consistency_compliance(
        self, action: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> Optional[ConstitutionalViolation]:
        ...
        if action.get("contradicts_previous_decision"):
            if not action.get("contradiction_justification"):
                issues.append(
                    "Action contradicts previous decision without justification"
                )
    ...
    async def _check_harm_prevention_compliance(
        self, action: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> Optional[ConstitutionalViolation]:
        ...
        if risk_level in ["high", "critical"]:
            if not action.get("reversible", False) and not action.get(
                "harm_safeguards"
            ):
    ...
    async def _log_violations(
        self, result: ConstitutionalValidationResult, action: Dict[str, Any]
    ):
