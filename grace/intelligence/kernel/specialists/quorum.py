...
class QuorumSpecialist(BaseSpecialist):
    ...
    def facilitate_consensus(
        self, specialist_analyses: List[Dict], feed_items: List[QuorumFeedItem]
    ) -> QuorumResult:
    ...
    def _analyze_specialist_consensus(self, analyses: List[Dict]) -> Dict:
        ...
        if len(confidences) > 1:
            ...
            agreement_level = max(
                0.0, 1.0 - confidence_stdev
            )  # Lower stdev = higher agreement
        else:
            agreement_level = 1.0
        ...
        if len(high_confidence_analyses) >= len(analyses) * 0.6:
            ...
            dissenting_views = self._identify_dissenting_views(
                analyses, high_confidence_analyses
            )
        else:
            # No clear consensus
    ...
    def _build_consensus_opinion(self, high_confidence_analyses: List[Dict]) -> str:
        ...
        positive_count = sum(
            1 for keyword in positive_keywords if keyword in all_opinions.lower()
        )
        negative_count = sum(
            1 for keyword in negative_keywords if keyword in all_opinions.lower()
        )
    ...
    def _identify_dissenting_views(
        self, all_analyses: List[Dict], consensus_analyses: List[Dict]
    ) -> List[str]:
        """Identify dissenting views from non-consensus analyses."""
        consensus_specialists = {
            analysis.get("specialist") for analysis in consensus_analyses
        }
    ...
    def _specialist_analysis(
        self, feed_items: List[QuorumFeedItem], context: Optional[Dict]
    ) -> Dict:
