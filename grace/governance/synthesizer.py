...
class Synthesizer:
    ...
    def merge(
        ...
        quorum_consensus: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        ...
        execution_approved = (
            approved
            and policy_analysis["all_passed"]
            and verification_result.get("verified", False)
        )
        ...
        if PYDANTIC_AVAILABLE:
            try:
                ...
                logger.warning(
                    f"Failed to create GovernedDecision model: {e}, returning dict"
                )
    ...
    def _analyze_policies(self, policy_results: List[Dict[str, Any]]) -> Dict:
        ...
        critical_failures = [
            ...
            and pr.get("policy_type") in ["content_filter", "access_control"]
        ]
    ...
    def _determine_approval(
        ...
        verification_result: Dict[str, Any],
        quorum_consensus: Optional[Dict[str, Any]],
    ) -> bool:
        ...
        policy_pass_rate = policy_analysis["passed_count"] / max(
            1, policy_analysis["total_count"]
        )
    ...
    def _generate_reasoning(
        ...
        if policy_analysis["has_critical_failures"]:
            ...
            pass_rate = policy_analysis["passed_count"] / max(
                1, policy_analysis["total_count"]
            )
        ...
        if quorum_consensus:
            if quorum_consensus.get("consensus_reached", False):
                ...
                parts.append(
                    f"Quorum consensus reached ({agreement_level:.1%} agreement)."
                )
            else:
    ...
    def _generate_conditions(
        self, policy_analysis: Dict, verification_result: Dict[str, Any]
    ) -> List[str]:
        ...
        if (
            policy_analysis["failed_count"] > 0
            and not policy_analysis["has_critical_failures"]
        ):
            conditions.append(
                "Monitor for compliance with failed non-critical policies"
            )
