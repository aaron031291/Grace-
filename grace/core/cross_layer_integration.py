...
class KernelHealthStatus:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.status in [IntegrationStatus.CONNECTED, IntegrationStatus.SYNCHRONIZED] and
            (datetime.now() - self.last_heartbeat).total_seconds() < 30 and
            self.response_time_ms < 1000 and
            self.cpu_usage_percent < 90
        )
...
class KernelInterface(ABC):
    ...
    def register_capability(self, capability: KernelCapability) -> None:
        ...
        logger.info(f"Registered capability {capability.capability_id} for kernel {self.kernel_id}")
    
    def register_message_handler(self, message_type: MessageType, handler: Callable) -> None:
        """Register a handler for a specific message type."""
        self.message_handlers[message_type] = handler
        logger.debug(f"Registered {message_type.value} handler for kernel {self.kernel_id}")
    
    async def send_message(self, target_kernel: str, message_type: MessageType,
    ...
    async def broadcast_message(self, message_type: MessageType, payload: Dict[str, Any]) -> None:
        """Broadcast a message to all connected kernels."""
        if not self.integration_bus:
            raise RuntimeError("Kernel not connected to integration bus")
        
        await self.integration_bus.broadcast_message(self.kernel_id, message_type, payload)
...
class DependencyResolver:
    ...
    def __init__(self):
        self.dependencies: Dict[str, Set[str]] = {}  # kernel_id -> set of dependency kernel_ids
        self.dependents: Dict[str, Set[str]] = {}    # kernel_id -> set of dependent kernel_ids
...
class StateManager:
    ...
    def __init__(self):
        ...
        self.state_subscribers: Dict[str, Set[str]] = {}  # state_key -> set of kernel_ids
        self.sync_handlers: Dict[str, Callable] = {}
    
    async def register_state(self, kernel_id: str, state_key: str, initial_state: Any) -> None:
    ...
    async def update_state(self, kernel_id: str, state_key: str, new_state: Any,
                          notify_subscribers: bool = True) -> None:
        ...
        async with self.state_locks[state_key]:
            ...
            if notify_subscribers and state_key in self.state_subscribers:
                for subscriber_id in self.state_subscribers[state_key]:
                    if subscriber_id != kernel_id and subscriber_id in self.sync_handlers:
                        try:
                            await self.sync_handlers[subscriber_id](state_key, kernel_id, old_state, new_state)
    ...
    def subscribe_to_state(self, subscriber_id: str, state_key: str, sync_handler: Callable) -> None:
...
class CrossLayerIntegrationBus:
    ...
    async def broadcast_message(self, source_kernel: str, message_type: MessageType,
    ...
    def subscribe_to_broadcasts(self, kernel_id: str, message_type: MessageType) -> None:
        ...
        logger.debug(f"Kernel {kernel_id} subscribed to {message_type.value} broadcasts")
    ...
    def _update_latency_stats(self, latency_ms: float) -> None:
        ...
        if total_msgs == 0:
            ...
            self.message_stats["avg_latency_ms"] = alpha * latency_ms + (1 - alpha) * current_avg
                ...
                for kernel_id, kernel in self.kernels.items():
                    try:
                        ...
                        if health_status.is_healthy():
                            ...
                            logger.warning(f"Kernel {kernel_id} is unhealthy: {health_status.status.value}")
                    
                    except Exception as e:
                        logger.error(f"Health check failed for kernel {kernel_id}: {e}")
                        # Mark as failed
                        if kernel_id in self.kernel_health:
                            self.kernel_health[kernel_id].status = IntegrationStatus.FAILED
    ...
    async def _check_and_initialize(self) -> None:
        ...
        if len(kernel_ids) >= 3:  # Minimum viable set
            initialization_order = self.dependency_resolver.get_initialization_order(kernel_ids)
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_kernels": len(self.kernels),
            "healthy_kernels": sum(1 for h in self.kernel_health.values() if h.is_healthy()),
            "kernel_statuses": kernel_statuses,
            "message_stats": self.message_stats,
            "uptime_seconds": (datetime.now() - self.message_stats["start_time"]).total_seconds()
        }
