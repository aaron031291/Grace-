...
class GovernanceProposal:
    ...
    def __init__(
        self, proposal_id: str, plan: AdaptationPlan, priority: str = "normal"
    ):
    ...
    def _determine_governance_requirements(self) -> Dict[str, Any]:
        ...
        has_policy_changes = any(
            action.type == ActionType.POLICY_DELTA for action in self.plan.actions
        )
...
class MLTGovernanceBridge:
    ...
    async def submit_plan_for_approval(
        self, plan: AdaptationPlan, priority: str = "normal"
    ) -> str:
        ...
        try:
            ...
            if self.event_bus:
                ...
                await self.event_bus.publish(
                    "ADAPTATION_PLAN_PROPOSED", governance_event
                )

                logger.info(
                    f"Submitted plan {plan.plan_id} to governance as proposal {proposal_id} (memory: {memory_id})"
                )
    ...
    async def handle_governance_decision(self, decision_event: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not proposal_id or proposal_id not in self.pending_proposals:
                logger.warning(
                    f"Unknown proposal ID in governance decision: {proposal_id}"
                )
                return False
            ...
            if event_type == "GOVERNANCE_APPROVED":
                ...
                self._store_governance_decision(
                    proposal, "approved", payload.get("rationale", "")
                )
                ...
                proposal.rejection_reason = payload.get(
                ...
                logger.info(
                    f"Governance rejected proposal {proposal_id}: {proposal.rejection_reason}"
                )
                return False
    ...
    def _store_governance_decision(
        self, proposal: GovernanceProposal, decision: str, rationale: str
    ) -> str:
    ...
    async def _apply_approved_plan(
        ...
        try:
            ...
            logger.info(
                f"Applied plan {plan.plan_id} with {len(applied_actions)} actions (memory: {applied_plan_memory_id})"
            )
            return {
                "status": "applied",
    ...
    def _store_applied_plan(
        self, proposal: GovernanceProposal, applied_actions: List[Dict]
    ) -> str:
    ...
    def get_memory_audit_trail(self, proposal_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = self.mtl_kernel.recall(
                query=proposal_id, filters={"metadata.proposal_id": proposal_id}
            )
                ...
                audit_trail.append(
                    {
                        ...
                        "timestamp": entry.metadata.get("submitted_at")
                        or entry.metadata.get("decided_at")
                        or entry.metadata.get("applied_at"),
            ...
            logger.error(
                f"Failed to get memory audit trail for proposal {proposal_id}: {e}"
            )
            return []
