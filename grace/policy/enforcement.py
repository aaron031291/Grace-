    ...
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        if operation:
            ...
            if not policy_result['allowed']:
                ...
                logger.warning(f"Blocked operation due to policy violations: {violations_summary}")
...
class PolicyValidator:
    ...
    def validate_file_operation(self, file_path: str, operation_type: str,
                               user_id: str = None, user_roles: list = None,
                               user_scopes: list = None) -> Dict[str, Any]:
    ...
    def validate_code_execution(self, code_content: str,
                               user_id: str = None, user_roles: list = None,
                               user_scopes: list = None) -> Dict[str, Any]:
    ...
    def validate_ide_changes(self, changes: Dict[str, Any],
            ...
            for violation in result['violations']:
                if violation['rule_name'] == 'ide_apply_changes':
                    ...
                    if 'require_sandbox_branch' in violation['actions']:
                        sandbox_prefix = metadata.get('sandbox_branch_prefix', 'sandbox/')
                        if not branch_name or not branch_name.startswith(sandbox_prefix):
                    ...
                    if 'require_policy_pass' in violation['actions']:
                        ...
                        result['required_labels'] = metadata.get('required_labels', ['policy:pass'])
                    
                    # Check human approval requirement
                    if 'require_human_approval' in violation['actions']:
                        result['human_approval_required'] = True
                        result['required_approvals'] = metadata.get('required_approvals', 1)
