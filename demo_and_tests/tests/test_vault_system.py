...
class TestVaultSystem:
    ...
    def test_vault_specifications_completeness(self):
        ...
        assert priority_vaults == [2, 3, 6, 12, 15], (
            f"Priority vaults incorrect: {priority_vaults}"
        )
        ...
        assert set(critical_ids) == set(expected_critical), (
            f"Critical vaults: {critical_ids}"
        )
    ...
    async def test_vault_2_code_verification(self):
        ...
        assert vault_2_result.passed, (
            f"Vault 2 failed: {vault_2_result.explainable_narrative}"
        )
    ...
    async def test_vault_6_contradiction_detection(self):
        ...
        request = {"request_id": "test_vault_6", "claims": [claim1, claim2]}
    ...
    async def test_vault_12_reasoning_chains(self):
        ...
        report = await self.vault_compliance_checker.check_priority_compliance(
            good_request
        )
        vault_12_result = report.vault_results.get(12)
        assert vault_12_result is not None
        assert vault_12_result.passed, (
            f"Good reasoning should pass: {vault_12_result.explainable_narrative}"
        )
    ...
    async def test_vault_15_sandbox_requirements(self):
        ...
        vault_15_result = report.vault_results.get(15)
        assert vault_15_result is not None
        assert vault_15_result.passed, (
            f"Sandboxed code should pass: {vault_15_result.explainable_narrative}"
        )
