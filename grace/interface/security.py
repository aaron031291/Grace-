...
class TokenManager:
    ...
    def generate_token(
        self, user_id: str, session_id: str, tenant_id: Optional[str] = None
    ) -> str:
...
class FileValidator:
    ...
    def validate_file_type(
        ...
        if content_type:
            ...
            if not (
                content_type.startswith(expected_mime.split("/")[0])
                or content_type == expected_mime
            ):
                logger.warning(
                    f"MIME type mismatch for {filename}: expected {expected_mime}, got {content_type}"
                )
                return False
