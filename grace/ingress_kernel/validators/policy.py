...
class BaseValidator(ABC):
    ...
    @abstractmethod
    async def validate(
        self, record: NormRecord, source_config: SourceConfig
    ) -> ValidationResult:
...
class PIIValidator(BaseValidator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.pii_patterns = {
            ...
            "credit_card": [r"\b(?:\d{4}[-\s]?){3}\d{4}\b", r"\b\d{13,19}\b"],
    ...
    async def validate(
        ...
        try:
            ...
            if pii_flags:
                if pii_policy == "block":
                    violations.append(
                        f"PII detected with block policy: {', '.join(pii_flags)}"
                    )
                    return ValidationResult(
                        passed=False, policy_type="pii", violations=violations
                    )
    ...
    async def validate(
        ...
        try:
            ...
            if not schema:
                ...
                return ValidationResult(
                    passed=False, policy_type="schema", violations=violations
                )
            ...
            for field, value in record.body.items():
                if field in properties:
                    ...
                    field_violations = await self._validate_field(
    ...
    async def validate(
        ...
        try:
            ...
            if retention_days > max_allowed:
                violations.append(
                    f"Retention period {retention_days} exceeds maximum {max_allowed}"
                )

            # Check governance label compliance
            label = source_config.governance_label
            restricted_patterns = self.config.get(
                "restricted_patterns",
            ...
            if label == "public":
                ...
                for pattern in restricted_patterns:
                    if pattern in content_text:
                        violations.append(
                            f"Public data contains restricted content: {pattern}"
                        )
            ...
            export_control_patterns = self.config.get(
                "export_control_patterns", ["itar", "ear", "dual use", "export control"]
            )
        ...
        try:
            ...
            for validator_name, validator in validators:
                try:
                    ...
                    logger.debug(
                        f"{validator_name} validation: {'PASS' if result.passed else 'FAIL'}"
                    )

                except Exception as e:
                    logger.error(f"{validator_name} validator failed: {e}")
                    all_violations.append(
                        f"{validator_name} validation error: {str(e)}"
                    )

            # Log results
            if all_violations:
                logger.warning(
                    f"Policy violations for {record.record_id}: {all_violations}"
                )
