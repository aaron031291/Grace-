...
class NotificationService:
    ...
    async def dispatch_notification(
        self, user_id: str, notif_id: str, channels: List[str] = None
    ):
    ...
    def create_and_dispatch(
        ...
        # Schedule dispatch
        asyncio.create_task(self.dispatch_notification(user_id, notif_id, channels))
    ...
    def get_stats(self) -> Dict:
        ...
        for notification in notifications:
            level_counts[notification.level] = (
                level_counts.get(notification.level, 0) + 1
            )
        ...
        active_users = len(
            [uid for uid, nids in self.user_notifications.items() if nids]
        )
