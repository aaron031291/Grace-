...
class PolicyRule:
    ...
    def _matches_condition(
        self, condition: Dict[str, Any], operation: Dict[str, Any]
    ) -> bool:
...
class PolicyEngine:
    ...
    def load_policies(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.rules)} policy rules from {self.policy_file}"
            )
        ...
        default_policies = {
            ...
            "rules": {
                "dangerous_file_operations": {
                    ...
                    "exceptions": [{"type": "user_role", "roles": ["admin", "system"]}],
                },
                ...
                "network_access": {
                    ...
                    "exceptions": [
                        {"type": "scope_required", "scopes": ["network:access"]}
                    ],
                },
                "ide_apply_changes": {
    ...
    def evaluate_operation(self, operation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for rule in self.rules:
            if rule.matches_operation(operation):
                ...
                if highest_severity is None or self._severity_level(
                    rule.severity
                ) > self._severity_level(highest_severity):
        ...
        result = {
            ...
            or not any("block" in v["actions"] for v in violations),
        ...
        if violations:
            logger.warning(
                f"Policy violations detected for operation {operation.get('type', 'unknown')}: {len(violations)} violations"
            )
        else:
            logger.debug(
                f"Policy evaluation passed for operation {operation.get('type', 'unknown')}"
            )
