...
class OrchSnapshot:
    ...
    def __init__(
        self, snapshot_id: str, snapshot_type: SnapshotType = SnapshotType.MANUAL
    ):
...
class SnapshotManager:
    ...
    def __init__(
        self,
        storage_path: str = "/tmp/grace_orchestration_snapshots",
        scheduler=None,
        state_manager=None,
        router=None,
        watchdog=None,
        ...
        try:
            ...
            if self.event_publisher:
                await self.event_publisher(
                    "ORCH_SNAPSHOT_CREATED", {"snapshot": snapshot.to_dict()}
                )
    ...
    async def rollback(self, to_snapshot: str) -> str:
        ...
        try:
            logger.info(
                f"Starting rollback operation {operation_id} to snapshot {to_snapshot}"
            )
    ...
    async def _capture_scheduler_state(self, snapshot: OrchSnapshot):
        ...
        if self.scheduler:
            try:
                ...
                snapshot.loops = [
                    loop.to_dict()
                    for loop in scheduler_status.get("loops", {}).values()
                ]
                snapshot.active_tasks = list(
                    scheduler_status.get("active_tasks", {}).keys()
                )
    ...
    async def _validate_snapshot(self, snapshot: OrchSnapshot):
        ...
        try:
            ...
            if snapshot.hash != calculated_hash:
                snapshot.validation_errors.append(
                    "Hash mismatch - content may be corrupted"
                )
            ...
            for component_name, state in snapshot.component_states.items():
                if not isinstance(state, dict):
                    snapshot.validation_errors.append(
                        f"Invalid state format for {component_name}"
                    )
            ...
            if snapshot.validation_errors:
                ...
                logger.warning(
                    f"Snapshot {snapshot.snapshot_id} validation failed: {snapshot.validation_errors}"
                )
            else:
                snapshot.status = SnapshotStatus.COMPLETED
    ...
    async def _store_snapshot(self, snapshot: OrchSnapshot):
        ...
        try:
            ...
            logger.debug(
                f"Stored snapshot {snapshot.snapshot_id} ({snapshot.size_bytes} bytes)"
            )
    ...
    async def _cleanup_old_snapshots(self):
        ...
        sorted_snapshots = sorted(
            self.snapshots.values(), key=lambda s: s.created_at, reverse=True
        )
    ...
    async def _create_scheduled_snapshot(
        ...
        try:
            ...
            snapshot_id = await self.export_snapshot(
                description=description, snapshot_type=SnapshotType.SCHEDULED, tags=tags
            )

            config["last_created"] = datetime.now()
            logger.info(
                f"Created scheduled snapshot {snapshot_id} for schedule {schedule_name}"
            )
    ...
    def list_snapshots(
        self, snapshot_type: SnapshotType = None, tags: Set[str] = None
    ) -> List[OrchSnapshot]:
