...
class TrustDecayEngine:
    ...
    async def initialize_entity(
        ...
        logger.info(
            f"Initialized trust decay for entity {entity_id} with model {decay_model.value}"
        )

        return state

    async def update_trust_with_decay(
        self,
        entity_id: str,
        ...
        if len(state.performance_variance_history) > 100:
            state.performance_variance_history = state.performance_variance_history[
                -50:
            ]

        logger.debug(
            f"Updated trust for {entity_id}: {adjusted_trust:.3f} (performance: {performance_score:.3f})"
        )
    ...
    async def _apply_time_decay(
        self, state: TrustDecayState, current_time: datetime
    ) -> float:
    ...
    async def _calculate_decayed_trust(
        self, state: TrustDecayState, current_time: datetime
    ) -> float:
    ...
    async def _exponential_decay(
        self, state: TrustDecayState, days_elapsed: float
    ) -> float:
    ...
    async def _adaptive_decay(
        ...
        if interaction_freq > 1.0:  # More than daily interaction
            ...
            base_decayed = (
                state.current_trust - (state.current_trust - base_decayed) * 0.5
            )
        elif interaction_freq < 0.1:  # Less than weekly interaction
            # Increase decay for inactive entities
            base_decayed = (
                state.current_trust - (state.current_trust - base_decayed) * 1.5
            )
        ...
        if variance > 0.3:  # High variance
            variance_penalty = (
                variance * state.parameters.variance_penalty * days_elapsed
            )
        ...
        if success_rate > 0.8:  # High success rate
            ...
            base_decayed = (
                state.current_trust - (state.current_trust - base_decayed) * 0.8
            )
        elif success_rate < 0.4:  # Low success rate
            # Faster decay for unreliable entities
            base_decayed = (
                state.current_trust - (state.current_trust - base_decayed) * 1.3
            )

        return max(base_decayed, state.parameters.minimum_trust)

    def _get_contextual_decay_rate(
        self, state: TrustDecayState, base_rate: float
    ) -> float:
        ...
        if state.critical_errors > 0:
            critical_penalty = (
                state.critical_errors * state.parameters.critical_error_penalty
            )
    ...
    async def _calculate_interaction_frequency(self, state: TrustDecayState) -> float:
        ...
        recent_interactions = [
            dt for dt in state.interaction_frequency if dt >= cutoff_date
        ]
        ...
        time_span = (
            recent_interactions[-1] - recent_interactions[0]
    ...
    async def _apply_performance_adjustment(
        self,
        state: TrustDecayState,
        current_trust: float,
        performance_score: float,
        ...
        if performance_score > 0.8:
            ...
            adjustment = (
                (performance_score - 0.8) * state.parameters.recovery_factor * 0.1
            )
    ...
    async def get_trust_projection(
        self, entity_id: str, days_ahead: int = 30
        ...
        for day in range(1, days_ahead + 1):
            ...
            projected_trust = await self._calculate_decayed_trust(
                temp_state, future_time
            )
    ...
    def get_trust_statistics(self, entity_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "days_since_last_interaction": (
                datetime.now() - state.last_interaction
            ).days,
            "recent_trend": recent_trend,
            "interaction_frequency": asyncio.create_task(
                self._calculate_interaction_frequency(state)
            ),
        }
