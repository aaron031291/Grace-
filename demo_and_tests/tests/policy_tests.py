    ...
    def test_security_config_structure(self):
        ...
        if os.path.exists(security_path):
            ...
            for section in required_sections:
                assert section in security_config, f"Missing required section: {section}"
            
            # Check JWT validation
            auth_config = security_config.get("auth", {})
            jwt_validation = auth_config.get("token_validation", {})
            assert jwt_validation.get("verify_signature", False), "JWT signature verification should be enabled"
            assert jwt_validation.get("verify_expiry", False), "JWT expiry verification should be enabled"
...
class TestDangerousOperationsPolicy:
    ...
    def test_dangerous_operations_policy_exists(self):
        ...
        if not policy_exists:
            ...
            default_policy = {
                ...
                "dangerous_operations": {
                    ...
                    "network_access": {"allowed": True, "restrictions": ["internal_only"]}
                },
                "enforcement": "strict",
        ...
        assert os.path.exists("policies/dangerous_operations.yaml"), "Dangerous operations policy should exist"
...
def test_policy_validation_scripts_exist():
    ...
    for script in scripts:
        assert os.path.exists(script), f"Policy validation script should exist: {script}"
