...
class OrchestrationInstance:
    ...
    def can_accept_tasks(self) -> bool:
        ...
        return (
            ...
            self.metrics.active_tasks < self.max_tasks
        )
    ...
    async def transition_status(self, new_status: InstanceStatus):
        ...
        logger.info(f"Instance {self.instance_id} transitioned: {old_status.value} -> {new_status.value}")
...
class LoadBalancer:
    ...
    def __init__(self, algorithm: LoadBalanceAlgorithm = LoadBalanceAlgorithm.LEAST_CONNECTIONS):
        self.algorithm = algorithm
        self.round_robin_index = 0
    
    def select_instance(self, instances: List[OrchestrationInstance]) -> Optional[OrchestrationInstance]:
    ...
    def _round_robin_selection(self, instances: List[OrchestrationInstance]) -> OrchestrationInstance:
    ...
    def _least_connections_selection(self, instances: List[OrchestrationInstance]) -> OrchestrationInstance:
        """Select instance with least active connections."""
        return min(instances, key=lambda i: i.metrics.active_tasks)
    
    def _weighted_round_robin_selection(self, instances: List[OrchestrationInstance]) -> OrchestrationInstance:
    ...
    def _least_response_time_selection(self, instances: List[OrchestrationInstance]) -> OrchestrationInstance:
...
class ScalingManager:
    ...
    async def spawn_instance(self, loop_id: str, parent_id: str = None) -> str:
        ...
        if self.event_publisher:
            await self.event_publisher("ORCH_INSTANCE_SPAWNED", {
    ...
    async def retire_instance(self, instance_id: str) -> bool:
        ...
        while instance.metrics.active_tasks > 0 and (time.time() - start_time) < max_wait_time:
        ...
        if instance.metrics.active_tasks > 0:
            logger.warning(f"Force retiring instance {instance_id} with {instance.metrics.active_tasks} active tasks")
    ...
    async def _on_instance_status_change(self, instance: OrchestrationInstance,
                                       old_status: InstanceStatus,
                                       new_status: InstanceStatus):
        """Handle instance status changes."""
        logger.debug(f"Instance {instance.instance_id} status: {old_status.value} -> {new_status.value}")
        
        # Handle failed instances
        if new_status == InstanceStatus.FAILED:
            logger.error(f"Instance {instance.instance_id} failed, will attempt replacement")
    ...
    async def _handle_failed_instance(self, instance: OrchestrationInstance):
        ...
        policy = self.scaling_policies.get(instance.loop_id, self.default_scaling_policy)
        
        if len(loop_instances) < policy["min_instances"]:
            logger.info(f"Spawning replacement instance for failed {instance.instance_id}")
    ...
    async def _scaling_loop(self):
        ...
        try:
            while self.running:
                ...
                for loop_id in set(instance.loop_id for instance in self.instances.values()):
    ...
    async def _evaluate_scaling(self, loop_id: str):
        ...
        avg_cpu = sum(i.metrics.cpu_usage for i in active_instances) / len(active_instances)
        avg_task_load = sum(i.metrics.active_tasks for i in active_instances) / len(active_instances)
        ...
        # Scale up decision
        should_scale_up = (
            len(active_instances) < policy["max_instances"] and
            (avg_cpu > policy["scale_up_threshold"] or 
             avg_task_load > policy["target_task_load"])
        )
        
        # Scale down decision  
        should_scale_down = (
            len(active_instances) > policy["min_instances"] and
            avg_cpu < policy["scale_down_threshold"] and
            avg_task_load < policy["target_task_load"] * 0.5
        )
