...
class GraceIntegrationTestSuite:
    ...
    async def run_all_tests(self):
        ...
        if passed == total:
            logger.info("🎉 ALL TESTS PASSED - Grace infrastructure is production ready!")
            return True
        else:
    ...
    async def test_event_mesh_transports(self) -> bool:
        ...
        try:
            ...
            assert await in_memory_transport.connect(), "In-memory transport connection failed"
            ...
            assert unknown_transport.__class__.__name__ == "InMemoryTransport", "Unknown transport fallback failed"
    ...
    async def test_event_bus_features(self) -> bool:
        ...
        try:
            ...
            event_bus = GraceEventBus(
                transport_config={"type": config.transport_type, "config": config.transport_config}
            )
            ...
            assert received_events[0].headers.event_type == "TEST_EVENT", "Wrong event type received"
            ...
            assert stats["processing_stats"]["messages_deduplicated"] > 0, "Deduplication not working"
    ...
    async def test_governance_api(self) -> bool:
        ...
        try:
            ...
            assert len(governance_api.consent_requirements) > 0, "Consent requirements not loaded"
    ...
    async def test_persistent_storage(self) -> bool:
        ...
        try:
            ...
            assert retrieved_snapshot["component_type"] == "test_component", "Wrong component type retrieved"
            ...
            assert retrieved_payload["test_data"]["key"] == "value", "Payload data incorrect"
            ...
            assert any(s["snapshot_id"] == snapshot_id for s in snapshots), "Created snapshot not in list"
    ...
    async def test_memory_system(self) -> bool:
        ...
        try:
            ...
            write_result = await memory_api.write_content(
                ...
                metadata={"test_type": "integration", "timestamp": datetime.utcnow().isoformat()}
            )
    ...
    async def test_end_to_end_workflow(self) -> bool:
        ...
        try:
            ...
            audit_logger = ImmutableLogs()
            snapshot_manager = GraceSnapshotManager(db_path=str(Path(self.temp_dir) / "e2e_test.db"))
            memory_api = GraceMemoryAPI()
            ...
            assert len(workflow_events) >= 2, f"Expected at least 2 workflow events, got {len(workflow_events)}"
    ...
    async def test_configuration_management(self) -> bool:
        ...
        try:
            ...
            assert default_config.transport_type == "in-memory", "Default transport type wrong"
            ...
            assert env_config.transport_type == "kafka", "Environment transport type not set"
            assert "test1:9092" in env_config.transport_config["bootstrap_servers"], "Kafka servers not configured"
