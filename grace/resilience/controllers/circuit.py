...
class CircuitBreaker:
    ...
    def __init__(
        ...
        logger.debug(f"Circuit breaker initialized: threshold={failure_threshold_pct}%, volume={volume_threshold}")
        ...
        with self._get_lock():
            ...
            if self._state == CircuitState.HALF_OPEN:
                ...
                if self._should_open_circuit():
                    self._state = CircuitState.OPEN
                    logger.warning(f"Circuit breaker opened: {self._get_failure_rate():.1f}% failure rate")
