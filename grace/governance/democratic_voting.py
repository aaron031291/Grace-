...
class DemocraticVote:
    ...
    def get_weighted_value(self, base_weight: float = 1.0) -> float:
        ...
        return float(self.vote_value.value) * base_weight * expertise_multiplier * confidence_multiplier
...
class VotingRound:
    ...
    def calculate_weighted_outcome(self) -> Tuple[float, float]:
        ...
        total_weight = sum(
            ...
            for vote in self.votes if vote.vote_value != VoteValue.ABSTAIN
        )
...
class DemocraticVotingEngine:
    ...
    async def register_voter(self, voter_id: str, name: str, 
    ...
    async def submit_proposal(self, title: str, description: str,
        ...
        logger.info(f"Submitted proposal {proposal_id} ({category.value}) with {voting_method.value}")
        
        return proposal_id
    
    async def cast_vote(self, proposal_id: str, voter_id: str,
                       vote_value: VoteValue, reasoning: str,
        ...
        expertise_relevance = await self._calculate_expertise_relevance(voter_id, proposal)
        ...
        if not proposal.voting_rounds or proposal.current_round == 0:
            proposal.voting_rounds.append(VotingRound(
                round_number=1,
                votes=[vote],
                deliberation_notes=[],
                timestamp=datetime.now()
        ...
        logger.info(f"Vote cast by {voter_id} on proposal {proposal_id}: {vote_value.name}")
    ...
    async def _identify_eligible_voters(self, proposal: DemocraticProposal) -> Set[str]:
        ...
        if proposal.category in [ProposalCategory.CONSTITUTIONAL, ProposalCategory.EMERGENCY]:
        ...
        for voter_id, voter_profile in self.voter_registry.items():
            relevance = await self._calculate_proposal_voter_relevance(proposal, voter_profile)
    ...
    async def _calculate_expertise_relevance(self, voter_id: str, 
                                           proposal: DemocraticProposal) -> float:
    ...
    async def _calculate_proposal_voter_relevance(self, proposal: DemocraticProposal,
                                                voter_profile: Dict[str, Any]) -> float:
        ...
        category_keywords = {
            ProposalCategory.OPERATIONAL: ["operations", "management", "process", "workflow"],
            ProposalCategory.POLICY: ["policy", "governance", "strategy", "planning"],
            ProposalCategory.CONSTITUTIONAL: ["legal", "constitutional", "rights", "principles"],
    ...
    async def _evaluate_proposal_outcome(self, proposal: DemocraticProposal) -> Dict[str, Any]:
        ...
        min_participation = max(3, len(proposal.eligible_voters) * 0.5)  # 50% of eligible voters
        if len(current_round.votes) < min_participation:
            return {"can_decide": False, "reason": f"Insufficient participation ({len(current_round.votes)}/{min_participation})"}
        ...
        if voting_method == VotingMethod.SIMPLE_MAJORITY:
            ...
            result = await self._evaluate_weighted_majority(proposal, current_round)  # Default
        
        return result
    
    async def _evaluate_simple_majority(self, proposal: DemocraticProposal, 
                                      round: VotingRound) -> Dict[str, Any]:
    ...
    async def _evaluate_weighted_majority(self, proposal: DemocraticProposal,
                                        round: VotingRound) -> Dict[str, Any]:
    ...
    async def _evaluate_supermajority(self, proposal: DemocraticProposal,
                                    round: VotingRound) -> Dict[str, Any]:
        """Evaluate using supermajority rule (typically 2/3 or 3/4)."""
        approve_votes = sum(1 for vote in round.votes if vote.vote_value.value > 0)
        total_votes = len([vote for vote in round.votes if vote.vote_value != VoteValue.ABSTAIN])
        ...
        return {
            "can_decide": True,
            "outcome": "approved" if approval_ratio >= supermajority_threshold else "rejected",
            "approval_ratio": approval_ratio,
    ...
    async def _evaluate_consensus(self, proposal: DemocraticProposal,
                                round: VotingRound) -> Dict[str, Any]:
        ...
        approve_votes = distribution[VoteValue.STRONGLY_APPROVE] + distribution[VoteValue.APPROVE]
        oppose_votes = distribution[VoteValue.STRONGLY_OPPOSE] + distribution[VoteValue.OPPOSE]
    ...
    async def _evaluate_expertise_weighted(self, proposal: DemocraticProposal,
                                         round: VotingRound) -> Dict[str, Any]:
    ...
    async def _finalize_proposal(self, proposal: DemocraticProposal, 
    ...
    async def _consider_new_deliberation_round(self, proposal: DemocraticProposal) -> None:
    ...
    async def _initiate_deliberation_round(self, proposal: DemocraticProposal) -> None:
        ...
        logger.info(f"Started deliberation round {proposal.current_round} for proposal {proposal.proposal_id}")
    
    async def _notify_eligible_voters(self, proposal: DemocraticProposal) -> None:
        """Notify eligible voters about new proposal."""
        if self.event_bus:
            await self.event_bus.publish("PARLIAMENT_NEW_PROPOSAL", {
                "proposal_id": proposal.proposal_id,
    ...
    async def _notify_deliberation_round(self, proposal: DemocraticProposal) -> None:
        """Notify voters about new deliberation round."""
        if self.event_bus:
            await self.event_bus.publish("PARLIAMENT_DELIBERATION_ROUND", {
                "proposal_id": proposal.proposal_id,
                "round_number": proposal.current_round,
                "eligible_voters": list(proposal.eligible_voters)
            })
    
    async def _notify_decision(self, proposal: DemocraticProposal, decision: Dict[str, Any]) -> None:
        """Notify stakeholders about final decision."""
        if self.event_bus:
            await self.event_bus.publish("PARLIAMENT_DECISION_MADE", {
                "proposal_id": proposal.proposal_id,
                "outcome": decision["outcome"],
    ...
    def get_voting_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_rounds": sum(record["voting_rounds"] for record in self.decision_history) / total_proposals,
