...
class DeploymentManager:
    ...
    def __init__(
        self, event_bus=None, governance_bridge=None, db_path: str = ":memory:"
    ):
    ...
    async def request(
        self, model_key: str, version: str, deployment_spec: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.governance_bridge:
                ...
                if not approval_required["approved"]:
                    raise ValueError(
                        f"Governance approval required: {approval_required['reason']}"
                    )
            ...
            if not validation_result["valid"]:
                raise ValueError(
                    f"Invalid deployment request: {validation_result['reason']}"
                )
            ...
            """,
                (
                    ...
                    if not shadow
                    else DeploymentStatus.SHADOW.value,
                    json.dumps(deployment_spec),
                    canary_pct,
                    shadow,
            ...
            deployment_info = {
                ...
                "status": DeploymentStatus.SHADOW.value
                if shadow
                else DeploymentStatus.CANARY.value,
            ...
            logger.info(
                f"Deployment {deployment_id} requested for {model_key}@{version}"
            )
    ...
    async def promote(
        self, deployment_id: str, target_pct: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if deployment["status"] not in [
                ...
                raise ValueError(
                    f"Cannot promote deployment with status {deployment['status']}"
                )
    ...
    async def get_deployment(self, deployment_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if deployment_data["deployment_spec"]:
                deployment_data["deployment_spec"] = json.loads(
                    deployment_data["deployment_spec"]
                )
            if deployment_data["metadata"]:
                deployment_data["metadata"] = json.loads(deployment_data["metadata"])

            # Get canary progress
            deployment_data["canary_progress"] = await self._get_canary_progress(
                deployment_id
            )
    ...
    async def rollback(
        self, deployment_id: str, reason: str = None, triggered_by: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.event_bus:
                await self.event_bus.publish(
                    "ROLLBACK_REQUESTED", {"target": "mldl", "to_snapshot": rollback_id}
                )
    ...
    async def _validate_deployment_request(
        ...
        try:
            ...
            if not 0 <= canary_pct <= 100:
                return {
                    "valid": False,
                    "reason": f"Invalid canary percentage: {canary_pct}",
                }
            ...
            active_count = len(
                [
                    ...
                    if d["status"]
                    in [DeploymentStatus.CANARY.value, DeploymentStatus.SHADOW.value]
                ]
            )
            ...
            if active_count >= self.max_concurrent_deployments:
                return {
                    "valid": False,
                    "reason": f"Too many active deployments: {active_count}",
                }
        ...
        try:
            ...
            if deployment_spec.get("target_env") == "prod":
                ...
                if self.governance_bridge:
                    ...
                    return {
                        "approved": False,
                        "reason": "Governance approval required for production",
                    }
    ...
    async def _run_canary_deployment(self, deployment_id: str):
        ...
        try:
            ...
            canary_steps = deployment.get("deployment_spec", {}).get(
                "canary_steps", self.default_canary_steps
            )
            promotion_window = deployment.get("deployment_spec", {}).get(
                "promotion_window", self.default_promotion_window
            )
            ...
            for step, target_pct in enumerate(canary_steps):
                ...
                logger.info(
                    f"Canary deployment {deployment_id} step {step}: {target_pct}%"
                )
                ...
                if metrics_ok:
                    ...
                    # Rollback on failure
                    await self.rollback(
                        deployment_id, "Canary metrics failed", "automated"
                    )
                    return
    ...
    async def _run_shadow_deployment(self, deployment_id: str):
        ...
        try:
            ...
            shadow_duration = deployment.get("deployment_spec", {}).get(
                "shadow_duration", 3600
            )  # 1 hour
    ...
    async def _check_promotion_eligibility(self, deployment_id: str) -> Dict[str, Any]:
        ...
        if deployment["updated_at"]:
            ...
            if datetime.now() - last_update < timedelta(minutes=10):
                return {
                    "eligible": False,
                    "reason": "Insufficient time since last promotion",
                }
