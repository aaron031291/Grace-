...
from .quantum_safe_storage import QuantumSafeStorageLayer
...
class EnhancedMemoryBridge:
    ...
    def _initialize_routing_rules(self) -> Dict[str, Any]:
        ...
        return {
            "governance_decisions": {
                "primary": "quantum"
                if routing_config.get("governance_to_quantum", True)
                else "traditional",
                "secondary": "vector",
            ...
            "unstructured_text": {
                "primary": "vector"
                if routing_config.get("unstructured_to_vector", True)
                else "traditional",
                "secondary": "traditional",
        ...
        try:
            ...
            if self.config["vector_db"]["enabled"]:
                ...
                if self.vector_memory and not await self.vector_memory.initialize():
                    logger.warning(
                        "Failed to initialize vector memory, continuing without it"
                    )
            ...
            if self.config["quantum_storage"]["enabled"]:
                ...
                if not await self.quantum_storage.initialize():
                    logger.warning(
                        "Failed to initialize quantum storage, continuing without it"
                    )
    ...
    def _determine_storage_strategy(
        self, content_type: str, data: Any, sensitivity_score: float = 0.0
    ) -> Dict[str, Any]:
    ...
    async def recall_enhanced(
        ...
        try:
            ...
            if search_strategy in ["auto", "quantum"] and self.quantum_storage:
                quantum_result = await self.quantum_storage.retrieve_encrypted(
                    content_id
                )
            ...
            if (
                not results["primary"]
                and search_strategy in ["auto", "vector"]
            ...
            if not results["primary"] and search_strategy in ["auto", "traditional"]:
                memory_result = await self.traditional_memory.recall_structured_memory(
                    content_id
                )
                if memory_result:
                    results["primary"] = {
            ...
            if results["primary"]:
                ...
                return {"content_id": content_id, "found": False, "status": "not_found"}
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            health_status = {"overall_healthy": True, "backends": {}}
