    ...
    def _register_routes(self):
        ...
        async def register_dataset_endpoint(manifest: dict):
            ...
            try:
                ...
                await self.mesh_bridge.publish_event(
                    "LEARN_DATASET_REGISTERED", {"manifest": manifest}
                )
        ...
        async def create_label_task_endpoint(task: dict):
            ...
            try:
                ...
                # Emit event
                await self.mesh_bridge.publish_event(
                    "LEARN_LABEL_TASK_CREATED", {"task": task}
                )
        ...
        async def submit_label_endpoint(label: dict):
            ...
            try:
                ...
                # Emit event
                await self.mesh_bridge.publish_event(
                    "LEARN_LABEL_ACCEPTED", {"label": label}
                )
        ...
        async def apply_augmentation_endpoint(request: dict):
            ...
            try:
                result = self.apply_augmentation(
                    request["dataset_id"], request["version"], request["spec"]
                )

                # Emit event
                await self.mesh_bridge.publish_event(
                    "LEARN_AUGMENT_APPLIED",
                    {
        ...
        async def update_curriculum_endpoint(curriculum: dict):
            ...
            try:
                ...
                spec_id = curriculum.get(
                    "spec_id", f"curr_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )

                # Emit event
                await self.mesh_bridge.publish_event(
                    "LEARN_CURRICULUM_UPDATED", {"curriculum": curriculum}
                )
        ...
        async def quality_report_endpoint(
            dataset_id: str, version: Optional[str] = None
        ):
