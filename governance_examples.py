...
class GraceGovernanceEngine:
    ...
    async def evaluate_action(self, action_type: str, action_details: Dict[str, Any], 
                            user_context: Dict[str, Any]) -> GovernanceDecision:
        """
        ...
        """
        logger.info(f"Evaluating {action_type} action for user {user_context.get('user_id')}")
        
        violations = []
        
        # Layer-1 Constitutional Checks (always enforced)
        layer_1_violations = await self._check_layer_1_rules(action_type, action_details, user_context)
        violations.extend(layer_1_violations)
        
        # Layer-2 Organizational Policy Checks 
        layer_2_violations = await self._check_layer_2_policies(action_type, action_details, user_context)
        ...
        medium_violations = [v for v in violations if v.severity == "medium" and not v.can_override]
    ...
    async def _check_layer_1_rules(self, action_type: str, action_details: Dict[str, Any], 
                                  user_context: Dict[str, Any]) -> List[GovernanceViolation]:
        ...
        if await self._check_pii_exposure(action_details):
            violations.append(GovernanceViolation(
                ...
                severity="critical",
                can_override=False,
                suggested_actions=["Remove PII from data", "Apply data anonymization"]
            ))
        ...
        if not action_details.get("audit_enabled", True):
            violations.append(GovernanceViolation(
                ...
                severity="high",
                can_override=False,
                suggested_actions=["Enable audit logging", "Add decision reasoning"]
            ))
        
        return violations

    async def _check_layer_2_policies(self, action_type: str, action_details: Dict[str, Any], 
                                     user_context: Dict[str, Any]) -> List[GovernanceViolation]:
        ...
        if action_type == "trading" and "trading_limits" in self.layer_2_policies:
            ...
            if trade_amount > policy["parameters"]["max_single_trade"]:
                violations.append(GovernanceViolation(
                    ...
                    severity="medium",
                    can_override=True,
                    suggested_actions=["Reduce trade size", "Request risk manager approval"]
                ))
        ...
        if action_type == "data_access" and "data_access" in self.layer_2_policies:
            ...
            if requester_dept != data_owner_dept and policy["parameters"]["require_approval"]:
                violations.append(GovernanceViolation(
                    layer=GovernanceLayer.LAYER_2_ORGANIZATIONAL,
                    rule_id="data_access",
                    ...
                    severity="medium",
                    can_override=True,
                    suggested_actions=["Request data owner approval", "Specify access duration"]
                ))
        
        # Marketing Campaign Timing
        if action_type == "marketing_campaign" and "marketing_timing" in self.layer_2_policies:
            ...
            if scheduled_time:
                ...
                if hour < allowed_hours["start"] or hour >= allowed_hours["end"]:
                    violations.append(GovernanceViolation(
                        ...
                        severity="low",
                        can_override=True,
                        suggested_actions=["Reschedule to business hours", "Request marketing lead approval"]
                    ))
    ...
    async def _assess_harm_risk(self, action_type: str, action_details: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def request_override(self, decision: GovernanceDecision, requester_id: str, 
...
async def developer_journey_example():
    ...
    decision = await governance.evaluate_action("module_deployment", action_details, user_context)
    ...
    if decision.violations:
        ...
        for violation in decision.violations:
            print(f"   • [{violation.layer.value}] {violation.rule_name}: {violation.description}")
    ...
    if any(v.rule_id == "privacy_protection" for v in decision.violations):
        ...
        action_details["data"] = "Processing anonymized tweets with sentiment indicators"
...
async def sales_journey_example():
    ...
    # Governance evaluation
    decision = await governance.evaluate_action("marketing_campaign", action_details, user_context)
    ...
    if any(v.rule_id == "marketing_timing" for v in decision.violations):
        ...
        # Re-evaluate
        decision = await governance.evaluate_action("marketing_campaign", action_details, user_context)
        print(f"✅ After reschedule: {'APPROVED' if decision.approved else 'NEEDS_REVIEW'}")
...
async def operations_journey_example():
    ...
    # Governance evaluation
    decision = await governance.evaluate_action("data_access", action_details, user_context)
    ...
    if decision.violations:
        ...
        for violation in decision.violations:
            layer_emoji = "🏛️" if violation.layer == GovernanceLayer.LAYER_1_CONSTITUTIONAL else "🏢"
...
async def main():
    ...
    print("=" * 60)
    print("Demonstrating Layer-1 (Constitutional) and Layer-2 (Organizational) governance\n")
