...
class HealthDashboard:
            ...
            if health.status in [HealthStatus.CRITICAL, HealthStatus.UNHEALTHY]:
                ...
                if not existing_alert:
                    alert = SystemAlert(
                        ...
                        component=name,
                        message=health.error_message or f"{name} status is {health.status.value}",
    ...
    async def run_diagnostic(self, component_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def _run_component_diagnostic(self, component_name: str) -> Dict[str, Any]:
        ...
        if health.status == HealthStatus.CRITICAL:
            diagnostic["recommendations"].append("Immediate attention required - component is in critical state")
        elif health.status == HealthStatus.DEGRADED:
            diagnostic["recommendations"].append("Monitor closely - component performance is degraded")
        ...
        if len(history) > 5:
            ...
            if recent_statuses.count("degraded") >= 3:
                diagnostic["recommendations"].append("Persistent degradation detected - investigate underlying cause")
    ...
    async def _run_full_diagnostic(self) -> Dict[str, Any]:
        ...
        diagnostic = {
            ...
            "unhealthy_components": [
                ...
                if health.status in [HealthStatus.CRITICAL, HealthStatus.UNHEALTHY, HealthStatus.DEGRADED]
            ],
