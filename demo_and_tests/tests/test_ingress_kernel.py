...
async def test_ingress_kernel():
    ...
    try:
        ...
        print(
            f"   ✓ PII patterns configured: {len(kernel.policy_guard.pii_validator.pii_patterns)}"
        )
        ...
        storage_paths = [kernel.bronze_path, kernel.silver_path, kernel.gold_path]
...
async def test_bridges():
    ...
    try:
        ...
        print(
            f"   ✓ Governance bridge pending requests: {governance_stats['pending_requests']}"
        )
        print(
