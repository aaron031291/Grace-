...
class ChaosExperiment:
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"Starting chaos experiment {self.experiment_id} on {self.target}"
            )
            ...
            return {
                ...
                "outcome": "pass"
                if self.results.get("hypothesis_confirmed", False)
                else "fail",
    ...
    async def _analyze_results(self) -> Dict[str, Any]:
        ...
        latency_increase = self.metrics_after.get(
            "latency_p95", 0
        ) - self.metrics_before.get("latency_p95", 0)

        error_increase = self.metrics_after.get(
            "error_rate", 0
        ) - self.metrics_before.get("error_rate", 0)

        # Determine if system remained stable
        system_stable = (
            latency_increase < 500  # Less than 500ms increase
            and error_increase < 5.0  # Less than 5% error increase
        )
...
class ChaosRunner:
    ...
    def __init__(self, max_blast_radius_pct: float = 5.0):
        ...
        logger.debug(
            f"Chaos runner initialized with max blast radius {max_blast_radius_pct}%"
        )
    ...
    async def start_experiment(
        ...
        if blast_radius_pct > self.max_blast_radius_pct:
            raise ValueError(
                f"Blast radius {blast_radius_pct}% exceeds maximum {self.max_blast_radius_pct}%"
            )
    ...
    async def _run_experiment_async(self, experiment: ChaosExperiment):
        ...
        try:
            ...
            logger.error(
                f"Error running chaos experiment {experiment.experiment_id}: {e}"
            )
...
async def start_experiment(
    target: str, blast_radius_pct: float, duration_s: int
) -> str:
