...
class GovernanceBridge:
    ...
    def _assess_risk_level(self, plan: Dict[str, Any]) -> str:
        ...
        if (
            route.get("canary_pct", 0) > 50  # High canary percentage
            or len(route.get("models", [])) > 10  # Many models
            or policy.get("min_confidence", 1.0) < 0.5  # Very low confidence threshold
            or route.get("env") == "prod"  # Production environment
        ):
            return "high"

        # Medium risk conditions
        elif (
            route.get("canary_pct", 0) > 10
            or len(route.get("models", [])) > 3
            or route.get("shadow", False)
        ):
            return "medium"

        return "low"

    def _process_governance_decision(
        self, decision: Dict[str, Any], plan: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not approved:
                ...
                approval_result["remediation_suggestions"] = (
                    self._get_remediation_suggestions(failure_modes)
                )
    ...
    def _basic_policy_check(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            if violations
            else "Basic policy checks passed",
    ...
    def _get_remediation_suggestions(self, failure_modes: list[str]) -> list[str]:
        ...
        for failure_mode in failure_modes:
            if "confidence" in failure_mode.lower():
                suggestions.append(
                    "Consider using more confident models or ensemble methods"
                )
    ...
    def _log_approval(self, plan: Dict[str, Any], approval_result: Dict[str, Any]):
        ...
        log_entry = {
            ...
            "risk_level": self._assess_risk_level(plan),
            "governance_mode": "fallback"
            if approval_result.get("fallback_mode")
            else "full",
        }
        ...
        logger.info(
            f"Approval logged: Plan {plan.get('plan_id')} - {'APPROVED' if approval_result['approved'] else 'REJECTED'}"
        )
