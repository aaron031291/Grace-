...
class LifecycleManager:
    ...
    async def startup(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Orchestration system started successfully in {self.startup_time:.2f}s"
            )
            ...
            if self.event_publisher:
                await self.event_publisher(
                    "ORCH_SYSTEM_STOPPED",
                    {
                        "timestamp": datetime.now().isoformat(),
                        "uptime_seconds": (
                            datetime.now() - self.uptime_start
                        ).total_seconds()
    ...
    async def _check_dependencies(self, component: ManagedComponent) -> bool:
        ...
        for dep_name in component.dependencies:
            if dep_name not in self.components:
                logger.warning(
                    f"Dependency {dep_name} not found for {component.name} - will proceed anyway"
                )
                continue
            ...
            if dep_component.status != ComponentStatus.HEALTHY:
                logger.warning(
                    f"Dependency {dep_name} not healthy for {component.name} - will proceed anyway"
                )
                continue
                ...
                try:
                    ...
                    if health_result.get("status") in ["healthy", "running"]:
                        ...
                        if component.status == ComponentStatus.HEALTHY:
                            logger.warning(
                                f"Component {component.name} became unhealthy"
                            )
    ...
    async def _transition_phase(self, new_phase: LifecyclePhase):
        ...
        logger.info(
            f"Lifecycle phase transition: {old_phase.value} -> {new_phase.value}"
        )

        # Publish phase transition event
        if self.event_publisher:
            await self.event_publisher(
                "ORCH_PHASE_TRANSITION",
                {
