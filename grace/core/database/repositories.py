...
class KnowledgeEntryRepository(BaseRepository[KnowledgeEntry]):
    ...
    async def search_by_content(
        self, query: str, trust_threshold: float = 0.5, limit: int = 50
    ) -> List[KnowledgeEntry]:
        ...
        result = await self.session.execute(
            ...
            .order_by(
                KnowledgeEntry.trust_score.desc(), KnowledgeEntry.created_at.desc()
            )
            .limit(limit)
        )
        return list(result.scalars().all())

    async def get_by_trust_score(
        self, min_trust: float, limit: int = 100
    ) -> List[KnowledgeEntry]:
...
class EventLogRepository(BaseRepository[EventLog]):
    ...
    async def get_by_user_session(
        self, user_id: str, session_id: UUID, limit: int = 100
    ) -> List[EventLog]:
