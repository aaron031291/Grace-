...
class TokenBucket:
    ...
    def __init__(self, rate_per_second: int, burst_capacity: int):
        ...
        logger.debug(f"Token bucket initialized: {rate_per_second} rps, burst {burst_capacity}")
...
class RateLimiter:
    ...
    def configure(self, service_id: str, rps: int, burst: int):
        ...
        logger.info(f"Rate limiter configured for {service_id}: {rps} rps, burst {burst}")
...
class SlidingWindowRateLimiter:
    ...
    def __init__(self, window_size_ms: int = 60000):
        ...
        logger.debug(f"Sliding window rate limiter initialized: {window_size_ms}ms window")
    ...
    def configure(self, service_id: str, requests_per_window: int):
        ...
        logger.info(f"Sliding window configured for {service_id}: {requests_per_window} requests per window")
