...
from ..core.kpi_trust_monitor import KPITrustMonitor, TrustScore
...
class GraceSandbox:
    ...
    def __init__(
        ...
        self.expires_at = self.created_at + timedelta(hours=resource_quota.max_duration_hours)
    ...
    async def start_experiment(
        ...
        logger.info(f"Started experiment {experiment.experiment_id} in sandbox {self.sandbox_id}")
    ...
    async def complete_experiment(
        ...
        # Update success rate
        completed_experiments = [e for e in self.experiments if e.completed_at is not None]
        successful_experiments = [e for e in completed_experiments if e.validation_status == "completed"]
        self.metrics.success_rate = len(successful_experiments) / len(completed_experiments) if completed_experiments else 0.0
    ...
    async def validate(self) -> Dict[str, Any]:
        ...
        if (self.metrics.trust_score >= 0.6 and 
            validation_results["resource_compliance"]["compliant"] and
    ...
    def should_auto_cleanup(self) -> bool:
        ...
        if (self.state == SandboxState.FAILED and 
            datetime.now() - self.metrics.last_activity > timedelta(hours=1)):
            return True
...
class SandboxManager:
    ...
    async def create_sandbox(
        ...
        if len(self.active_sandboxes) >= self.default_quota.max_concurrent_experiments:
            raise ValueError(f"Maximum concurrent sandboxes ({self.default_quota.max_concurrent_experiments}) reached")
    ...
    async def list_sandboxes(self, state_filter: Optional[SandboxState] = None) -> List[Dict[str, Any]]:
    ...
    async def approve_merge(self, sandbox_id: str, human_approval: bool = True) -> Dict[str, Any]:
        ...
        if sandbox.state != SandboxState.READY_FOR_MERGE:
            raise ValueError(f"Sandbox {sandbox_id} not ready for merge (state: {sandbox.state})")
    ...
    async def destroy_sandbox(self, sandbox_id: str, reason: str = "manual") -> Dict[str, Any]:
        """Destroy a sandbox."""
        sandbox = self.active_sandboxes.get(sandbox_id)
        if not sandbox:
            raise ValueError(f"Sandbox {sandbox_id} not found")
        ...
        while True:
            try:
                ...
                if sandboxes_to_cleanup:
                    logger.info(f"Auto-cleaned up {len(sandboxes_to_cleanup)} sandboxes")
    ...
    async def get_sandbox_dashboard_data(self) -> Dict[str, Any]:
        ...
        if active_count > 0:
            total_trust = sum(s.metrics.trust_score for s in self.active_sandboxes.values())
    ...
    def _get_success_metrics(self) -> Dict[str, Any]:
        ...
        total_experiments = sum(len(s.experiments) for s in self.active_sandboxes.values())
        total_completed = sum(
        ...
        return {
            ...
            "completion_rate": total_completed / total_experiments if total_experiments > 0 else 0.0,
            "success_rate": total_successful / total_completed if total_completed > 0 else 0.0,
            "active_sandbox_health": sum(1 for s in self.active_sandboxes.values() if s.metrics.trust_score >= 0.6)
        }
