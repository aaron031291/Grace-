...
class ExperienceCollector:
    ...
    async def collect_experience(self, source: ExperienceSource, raw_data: Dict[str, Any]) -> Experience:
        ...
        try:
            ...
            if self.event_bus:
                await self.event_bus.publish("EXPERIENCE_INGESTED", experience.to_dict())
            
            logger.info(f"Collected experience {experience.experience_id} from {source.value}")
            return experience
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for source in ExperienceSource:
            by_source[source.value] = len([exp for exp in self.experiences if exp.source == source])
        
        return {
            "total_experiences": len(self.experiences),
            "by_source": by_source,
            "latest_timestamp": max([exp.timestamp for exp in self.experiences]) if self.experiences else None
        }