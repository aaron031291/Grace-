...
class MetaEnsembler:
    ...
    def uncertainty(self, model_outputs: List[Dict]) -> Dict[str, float]:
        ...
        try:
            ...
            predictions = [
                out.get("prediction") for out in model_outputs if "prediction" in out
            ]
            probabilities = [
            ...
            confidences = [
                out.get("confidence") for out in model_outputs if "confidence" in out
            ]
            ...
            if predictions and len(predictions) > 1:
                if all(isinstance(p, (int, float)) for p in predictions):
                    ...
                    variance = sum((x - mean_pred) ** 2 for x in pred_list) / len(
                        pred_list
                    )
            ...
            if confidences:
                ...
                std_confidence = (
                    (
                        (
                            sum((x - avg_confidence) ** 2 for x in confidences)
                            / len(confidences)
                        )
                        ** 0.5
                    )
    ...
    def _predict_stacking(self, predictions: List, probabilities: List) -> Dict:
        ...
        features = [
            float(p) if isinstance(p, (int, float)) else 0.5 for p in predictions
        ]
        ...
        if weights and len(weights) == len(features):
            ...
            ensemble_score = intercept + (
                sum(features) / len(features) if features else 0.5
            )
    ...
    def _predict_blending(self, predictions: List, probabilities: List) -> Dict:
        ...
        if all(isinstance(p, (int, float)) for p in predictions):
            ...
            if total_weight > 0:
                weighted_pred = (
                    sum(p * w for p, w in zip(predictions, weights[: len(predictions)]))
                    / total_weight
                )
            else:
                weighted_pred = sum(predictions) / len(predictions)
            return {"prediction": float(weighted_pred), "confidence": 0.75}
        else:
    ...
    def _predict_voting(self, predictions: List, probabilities: List) -> Dict:
        ...
        # Average for regression
            ...
            numeric_preds = [
                float(p) for p in predictions if isinstance(p, (int, float))
            ]
            if numeric_preds:
                ...
                return {
                    ...
                    "confidence": 0.7,
                    "std_dev": float(np.std(numeric_preds))
    ...
    def _average_probabilities(self, probabilities: List) -> Optional[List[float]]:
        ...
        try:
            ...
            if valid_probs:
                ...
                avg_probs = [
                    sum(probs[i] for probs in truncated_probs) / len(truncated_probs)
    ...
    def _estimate_calibration_error(self) -> float:
        ...
        errors = []
        for predicted_prob, actual_outcome in self.calibration_data[
            -100:
        ]:  # Last 100 samples
    ...
    def _update_performance_history(
        self, reports: List[Dict], predictions: List[float]
    ):
        ...
        history_entry = {
            ...
            "prediction_variance": (
                (
                    sum(
                        (p - sum(predictions) / len(predictions)) ** 2
