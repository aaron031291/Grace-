...
from ..contracts.dto_common import W5HIndex
...
class EnhancedW5HIndexer:
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            logger.warning(
                f"Could not initialize advanced models: {e}. Falling back to basic patterns."
            )
            self.nlp = None
    ...
    def _extract_with_advanced_nlp(
    ...
    def _extract_who_advanced(self, doc, content: str) -> List[str]:
        ...
        for token in doc:
            if token.pos_ in ["NOUN", "PROPN"] and token.dep_ in ["nsubj", "nsubjpass"]:
                ...
                if any(
                    role in token.text.lower()
                    for role in ["user", "admin", "manager", "developer", "analyst"]
                ):
    ...
    def _extract_where_advanced(self, doc, content: str) -> List[str]:
        ...
        for pattern in self.where_patterns:
            matches = re.findall(pattern, content, re.IGNORECASE)
            where_entities.extend(
                [match if isinstance(match, str) else match[0] for match in matches]
            )

        return list(
            set([entity.strip() for entity in where_entities if entity.strip()])
        )
    ...
    def _extract_why_advanced(self, doc, content: str) -> List[str]:
        ...
        if self.sentiment_analyzer:
            try:
                ...
                if confidence > 0.8:
                    why_entities.append(
                        f"motivated by {sentiment_score.lower()} sentiment"
                    )
            except Exception as e:
    ...
    def _add_semantic_understanding(
        self, index: W5HIndex, doc, content: str, context: Optional[Dict[str, Any]]
    ) -> W5HIndex:
        ...
        try:
            ...
            if sentiment.subjectivity > 0.7:
                index.why.append(
                    "highly subjective content indicates personal opinions"
                )
    ...
    def _is_temporal_entity(self, text: str) -> bool:
        ...
        return (
            any(keyword in text_lower for keyword in temporal_keywords)
            or re.search(r"\d{1,4}[/\-:]\d{1,2}", text) is not None
        )
