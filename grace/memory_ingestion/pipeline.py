...
from .text_chunking import get_text_chunker
        ...
        async with self.tracer.async_span(
            ...
            try:
                ...
                async with self.tracer.async_span(
                    ...
                    embedding_results = (
                        await self.embedding_generator.generate_embeddings(chunk_texts)
                    )

                    embedding_span.set_tag(
                        "embeddings.generated", len(embedding_results)
                    )
                    if embedding_results:
                        embedding_span.set_tag(
                            "embedding.dimension",
                            embedding_results[0].get("embedding_dimension", 0),
                        )

                # Step 4: Store in database and vector store
                async with self.tracer.async_span(
                    "memory.database_storage", parent_context=main_span.context
                ) as storage_span:
                    async with get_database() as db_session:
                        ...
                        storage_span.set_tag(
                            "knowledge_entry.id", str(knowledge_entry.id)
                        )
                        ...
                        for chunk, embedding_result in zip(chunks, embedding_results):
                            if "error" in embedding_result:
                                logger.warning(
                                    f"Skipping chunk due to embedding error: {embedding_result['error']}"
                                )
                                continue
                            ...
                            async with self.tracer.async_span(
                                ...
                                tags={
                                    ...
                                    "vector.dimension": embedding_result[
                                        "embedding_dimension"
                                    ],
                                },
                            ) as vector_span:
                ...
                logger.info(
                    f"Successfully ingested file {file_path} with {len(ingested_chunks)} chunks"
                )
            ...
            try:
                ...
                async with self.tracer.async_span(
                    ...
                    query_embedding_result = (
                        await self.embedding_generator.generate_embedding(query)
                    )
                    ...
                    async with get_database() as db_session:
                        ...
                        for result in search_results:
                            ...
                            if fragment_id:
                                ...
                                fragment = await fragment_repo.get_by_id(
                                    UUID(fragment_id)
                                )
                                if fragment:
                                    enhanced_result = {
                        ...
                        enhancement_span.set_tag(
                            "enhanced.results_count", len(enhanced_results)
                        )

                # Log final search results
                self.memory_tracer.add_search_result(
                    search_span, len(enhanced_results), search_time_ms, trust_threshold
                )
...
def get_memory_ingestion_pipeline(
