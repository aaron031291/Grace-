...
from grace.mldl import (
    MLDLService, LogisticRegressionAdapter, XGBAdapter, KMeansAdapter,
    TrainingJobRunner, ModelRegistry, DeploymentManager,
...
async def main():
    ...
    for adapter in adapters:
        ...
        if adapter.task == "clustering":
            ...
            if hasattr(adapter, 'predict_proba'):
                ...
                print(f"    Sample probabilities: {probabilities[:3, 1] if probabilities is not None else 'N/A'}")
    ...
    print(f"    ✅ Training completed: {trained_bundle['model_key']}@{trained_bundle['version']}")
    ...
    print(f"  Fairness Delta: {fairness_result.get('metrics', {}).get('overall_delta', 'N/A'):.4f}")
    ...
    print(f"  ✅ Model registered: {registration_result['model_key']}@{registration_result['version']}")
    
    # Query the registry
    registered_model = await registry.get(trained_bundle['model_key'], trained_bundle['version'])
    if registered_model:
    ...
    deployment = await deployment_manager.request(
        trained_bundle['model_key'],
        trained_bundle['version'],
        deployment_spec
    )
