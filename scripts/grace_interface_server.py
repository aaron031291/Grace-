...
from grace_orb_api_implementation import setup_orb_api_routes
...
class GraceInterfaceServer:
    ...
    def __init__(self):
        ...
        # Setup Orb API routes
        setup_orb_api_routes(self.app, self.grace_kernel)
    ...
    def _setup_routes(self):
        ...
        async def health_check():
            """Health endpoint required by Grace Build & Policy Contract."""
        ...
        async def book_progress(job_id: str):
            ...
            try:
                progress = self.grace_kernel.book_ingestion.get_ingestion_progress(
                    job_id
                )
                if not progress:
        ...
        async def book_insights(job_id: str):
            """Get book insights."""
            try:
                insights = await self.grace_kernel.book_ingestion.get_book_insights(
                    job_id
                )
                return {"job_id": job_id, "insights": insights, "total": len(insights)}
            except Exception as e:
        ...
        # Required Orb API stats endpoint
        @self.app.get("/api/orb/v1/stats")
        async def orb_stats():
            """Get Orb interface statistics - required by Grace Build & Policy Contract."""
            from datetime import datetime
            ...
            return {
                ...
                "loop_engine": {
                    "turn_latency_ms": 2500,
                    "tool_success_rate": 0.98,
                    "error_rate": 0.01,
                    "hallucination_flag_rate": 0.005,
                    "governance_block_rate": 0.02,
                },
                "memory": {
                    "fragments_count": 0,
                    "short_term_capacity": 50,
                    "long_term_fragments": 0,
        ...
        # IDE stats endpoint
        @self.app.get("/api/orb/v1/stats/ide")
        async def ide_stats():
            """Get IDE-specific statistics."""
            from datetime import datetime
        ...
        logger.info("Grace Interface Server routes setup complete")
