...
class DocQualitySpecialist(BaseSpecialist):
    ...
    def _specialist_analysis(
        ...
        return {
            ...
            "evidence": [
                item.memory_id
                for item in feed_items
                if quality_scores[feed_items.index(item)] >= 0.7
            ],
        }
    ...
    def _analyze_document_quality(self, item: QuorumFeedItem) -> tuple:
        ...
        # Structural analysis
        structure_score = self._score_patterns(
            content, self.quality_patterns["structure"]
        )

        # Language quality analysis
        language_score = self._score_patterns(
            content, self.quality_patterns["language_quality"]
        )

        # Content depth analysis
        depth_score = self._score_patterns(
            content, self.quality_patterns["content_depth"]
        )
