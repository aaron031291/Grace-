...
class TrustScorer:
    ...
    async def update_source_reputation(
        ...
        """
        try:
            current_rep = self.source_reputation.get(
                source_id, 0.7
            )  # Default reputation
            ...
            if len(self.reputation_history[source_id]) > max_history:
                self.reputation_history[source_id] = self.reputation_history[source_id][
                    -max_history:
                ]

            logger.debug(
                f"Updated reputation for {source_id}: {current_rep:.3f} -> {new_rep:.3f}"
            )
    ...
    async def _calculate_freshness_factor(self, record: NormRecord) -> float:
        ...
        try:
            ...
            freshness_factor = math.exp(
                -freshness_minutes / half_life_minutes * math.log(2)
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_reputation": (
                sum(self.source_reputation.values())
                / max(1, len(self.source_reputation))
            ),
    ...
    async def get_source_trust_report(self, source_id: str) -> Dict[str, Any]:
        ...
        recent_history = [
            h for h in history if h["timestamp"] > datetime.utcnow() - timedelta(days=7)
        ]
