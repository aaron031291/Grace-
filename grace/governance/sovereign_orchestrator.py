...
from .sandbox_manager import SandboxManager, GraceSandbox, ExperimentType, ResourceQuota, SandboxState
from .validation_harness import ValidationHarness, ValidationLevel
from .consensus_engine import CrossPersonaConsensus, PersonaRole, ConsensusResult
from .human_feedback import HumanFeedbackInterface, FeedbackType, FeedbackScore
...
class SovereignSandboxOrchestrator:
    ...
    def __init__(
        ...
        self.consensus_engine = CrossPersonaConsensus(
            mldl_quorum=mldl_quorum or MLDLQuorum(event_bus),
            trust_threshold=0.7
        )
    ...
    async def grace_experiment_cycle(
        ...
        logger.info(f"Starting Grace experiment cycle {cycle_id}: {experiment_description}")
        ...
        try:
            ...
            if validation_results["overall_status"] not in ["passed", "warning"]:
                ...
                await self.sandbox_manager.destroy_sandbox(sandbox_id, reason="validation_failed")
                return {
                    "cycle_id": cycle_id,
                    "status": "failed",
            ...
            consensus_decision = await self._make_sovereign_decision(
                ...
                context={"sandbox_id": sandbox_id, "validation_results": validation_results},
            ...
            if not consensus_result.overall_approval:
                ...
                await self._request_human_feedback_on_rejection(sandbox_id, consensus_result)
                return {
                    "cycle_id": cycle_id,
            ...
            if human_approval_required:
                review_decision = await self._make_sovereign_decision(
                    ...
                    context={"sandbox_id": sandbox_id, "consensus_result": asdict(consensus_result)},
            ...
            if 'sandbox_id' in locals():
                await self.sandbox_manager.destroy_sandbox(sandbox_id, reason="cycle_error")
    ...
    async def _make_sovereign_decision(
        ...
        if requires_human_approval is None:
            requires_human_approval = self.autonomy_level < 0.8  # High autonomy = less human approval needed
        ...
        logger.info(f"Grace made sovereign decision: {action.value} (confidence: {confidence:.2f})")
        return decision
    
    def _assess_decision_risk(self, action: SovereignAction, context: Dict[str, Any]) -> Dict[str, float]:
    ...
    async def _simulate_experiment_work(self, sandbox: GraceSandbox, experiment_id: str):
        ...
        results = {
            ...
            "improvements_identified": ["Better error handling", "Performance optimization"],
            "metrics": {
                "execution_time_improvement": 0.15,
                "memory_usage_reduction": 0.08,
                "reliability_increase": 0.12
            },
            "new_capabilities": ["Enhanced pattern recognition", "Improved decision confidence"],
            "lessons_learned": ["Iterative validation is crucial", "Multi-perspective consensus adds value"]
        }
    ...
    def _assess_impact(self, sandbox: GraceSandbox, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "benefits": sandbox.experiments[0].results.get("improvements_identified", []) if sandbox.experiments else [],
    ...
    async def _auto_approve_merge(self, sandbox_id: str, consensus_result: ConsensusResult) -> Dict[str, Any]:
    ...
    async def _process_human_feedback(self, feedback):
        ...
        learning_decision = await self._make_sovereign_decision(
            ...
            context={"feedback_id": feedback.feedback_id, "target_id": feedback.target_id},
        ...
        logger.info(f"Processed feedback {feedback.feedback_id}, autonomy now: {self.autonomy_level:.2f}")
    
    async def _learn_from_autonomous_action(self, decision: SovereignDecision, result: Dict[str, Any]):
    ...
    async def _perform_meta_learning(self):
        ...
        success_rate = len(successful_patterns) / (len(successful_patterns) + len(failed_patterns))
    ...
    async def get_orb_dashboard_data(self) -> OrbitDashboardData:
        ...
        validation_history = await self.validation_harness.get_validation_history(limit=10)
        ...
        autonomy_metrics = {
            ...
            "recent_sovereign_decisions": len([d for d in self.sovereign_decisions if 
                                              (datetime.now() - d.timestamp).days < 7]),
            "successful_autonomous_actions": len(self.governance_learning_data["successful_patterns"]),
            "learning_enabled": self.learning_enabled,
            "trust_calibration": self.trust_monitor.get_trust_score("sovereign_orchestrator").score if self.trust_monitor else 0.5
        }
        
        # System health overview
        active_sandboxes = len(await self.sandbox_manager.list_sandboxes())
        pending_validations = len([v for v in validation_history if v.get("overall_status") == "pending"])
        pending_consensus = len(consensus_history)  # Simplified
        pending_feedback = len(self.human_feedback.get_pending_feedback_requests())
        
        system_health = {
            "overall_status": "healthy" if active_sandboxes < 10 and pending_feedback < 5 else "busy",
            "active_components": 4,  # sandbox_manager, validation_harness, consensus_engine, human_feedback
            "process_queue_size": pending_validations + pending_consensus + pending_feedback,
    ...
    async def get_sovereignty_report(self) -> Dict[str, Any]:
        ...
        recent_decisions = [d for d in self.sovereign_decisions if 
                          (datetime.now() - d.timestamp).days < 30]
        
        autonomous_decisions = [d for d in recent_decisions if not d.requires_human_approval]
        
        sovereignty_metrics = {
            "autonomy_level": self.autonomy_level,
            "total_sovereign_decisions": len(self.sovereign_decisions),
            "recent_decisions_30_days": len(recent_decisions),
            "autonomous_decisions_ratio": len(autonomous_decisions) / len(recent_decisions) if recent_decisions else 0.0,
            "avg_decision_confidence": sum(d.confidence for d in recent_decisions) / len(recent_decisions) if recent_decisions else 0.0,
            "learning_data_points": len(self.governance_learning_data["successful_patterns"]) + 
        ...
        return {
            ...
            "recommendations": self._generate_sovereignty_recommendations(sovereignty_metrics)
        }
    
    def _generate_sovereignty_recommendations(self, metrics: Dict[str, Any]) -> List[str]:
        """Generate recommendations for improving Grace's sovereignty."""
        
        recommendations = []
        
        if metrics["autonomy_level"] < 0.5:
            recommendations.append("Consider increasing validation automation to build confidence")
        
        if metrics["autonomous_decisions_ratio"] < 0.3:
            recommendations.append("Gradually increase autonomous decision-making in low-risk scenarios")
        
        if metrics["avg_decision_confidence"] < 0.6:
            recommendations.append("Focus on improving decision-making calibration through more feedback")
        
        if metrics["learning_data_points"] < 50:
            recommendations.append("Collect more operational data to improve meta-learning effectiveness")
