...
class KPITrustMonitor:
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        if low_trust_components:
            ...
            health_data["alerts"].append(
                f"Low trust components: {', '.join(low_trust_components)}"
            )
            ...
            try:
                ...
                for trust_score in self.trust_scores.values():
                    if now - trust_score.last_updated > decay_threshold:
                        ...
                        logger.debug(
                            f"Applied trust decay to {trust_score.component_id}"
                        )
    ...
    async def _collect_system_metrics(self):
        ...
        try:
            ...
            await self.record_metric(
                "cpu_usage", 0.0, "system", tags={"source": "placeholder"}
            )
            await self.record_metric(
                "memory_usage", 0.0, "system", tags={"source": "placeholder"}
            )
    ...
    async def _check_metric_anomalies(self, metric: KPIMetric):
        ...
        if anomaly_detected:
            ...
            logger.warning(
                f"Anomaly detected: {metric.name}={metric.value} ({severity})"
            )
