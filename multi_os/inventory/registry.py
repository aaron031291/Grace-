...
class Registry:
    ...
    def register_host(self, host_descriptor: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if host_id in self.hosts:
                ...
                host_descriptor["first_registered"] = old_host.get(
                    "first_registered", host_descriptor["registered_at"]
                )
                host_descriptor["total_tasks"] = old_host.get("total_tasks", 0)
                host_descriptor["successful_tasks"] = old_host.get(
                    "successful_tasks", 0
                )
            ...
            logger.info(
                f"Host {host_id} registered: {host_descriptor['os']}/{host_descriptor['arch']} "
    ...
    def update_host_status(
        self, host_id: str, status: str, health_metrics: Optional[Dict] = None
    ) -> bool:
        """
        ...
        for host in self.hosts.values():
            ...
            if constraints:
                ...
                if constraints.get("gpu_required", False):
                    ...
                    if not gpu_labels or any(
                        "none" in label.lower() for label in gpu_labels
                    ):
                        continue
                ...
                if "labels" in constraints:
                    ...
                    for req_label in required_labels:
                        if not any(
                            req_label in host_label for host_label in host_labels
                        ):
                            continue
    ...
    def _validate_host_descriptor(self, host_descriptor: Dict[str, Any]) -> None:
        ...
        if host_descriptor["os"] not in valid_os:
            raise ValueError(
                f"Invalid OS: {host_descriptor['os']}. Must be one of {valid_os}"
            )
        ...
        if host_descriptor["arch"] not in valid_arch:
            raise ValueError(
                f"Invalid architecture: {host_descriptor['arch']}. Must be one of {valid_arch}"
            )
        ...
        if host_descriptor["status"] not in valid_status:
            raise ValueError(
                f"Invalid status: {host_descriptor['status']}. Must be one of {valid_status}"
            )
    ...
    def _save_hosts(self) -> None:
        ...
        try:
            data = {"hosts": self.hosts, "last_updated": datetime.utcnow().isoformat()}
