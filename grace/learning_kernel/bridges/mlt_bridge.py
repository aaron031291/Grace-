...
class MLTBridge:
    ...
    async def send_learning_experience(self, stage: str, metrics: Dict[str, Any], 
        ...
        # Validate stage
        valid_stages = ["labeling", "active_query", "weak_supervision", "augmentation", 
                       "curriculum", "version_publish", "eval"]
    ...
    async def send_active_query_experience(self, query_gain_f1: float, strategy: str,
                                         batch_size: int, coverage: float,
                                         dataset_id: Optional[str] = None) -> str:
        """Send active learning query experience to MLT."""
        metrics = {
            "query_gain_f1": query_gain_f1,
            "coverage": coverage
        }
    ...
    async def send_weak_supervision_experience(self, weak_precision: float, 
        ...
        return await self.send_learning_experience(
            stage="weak_supervision",
            metrics=metrics,
            dataset_id=dataset_id
        )
    
    async def send_augmentation_experience(self, delta_rows: int, augment_type: str,
        ...
        metrics = {
            ...
            "augmentation_type": augment_type
        }
    ...
    async def send_evaluation_experience(self, bias_delta: float, leakage_flags: int,
    ...
    async def receive_adaptation_plan(self, plan: Dict[str, Any]) -> bool:
        ...
        if learning_actions:
            print(f"[MLT->LEARNING] Received plan with {len(learning_actions)} learning actions")
