...
class MLTService:
    ...
    def _register_routes(self):
        ...
        async def get_status():
            ...
            try:
                components = {
                    "experience_collector": hasattr(
                        self.mlt_kernel, "experience_collector"
                    ),
                    "insight_generator": hasattr(self.mlt_kernel, "insight_generator"),
                    "adaptation_planner": hasattr(
                        self.mlt_kernel, "adaptation_planner"
                    ),
        ...
        async def submit_experience(request: ExperienceRequest):
            ...
            try:
                ...
                experience = (
                    await self.mlt_kernel.experience_collector.collect_experience(
                        source=source, raw_data=request.dict()
                    )
                )
                ...
                raise HTTPException(
                    status_code=400, detail=f"Failed to submit experience: {str(e)}"
                )
        ...
        async def get_plans(limit: int = 10):
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to get plans: {str(e)}"
                )
        ...
        async def control_job(request: JobControlRequest):
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to control job: {str(e)}"
                )
        ...
        async def get_dashboard():
            ...
            try:
                ...
                # Recent activity
                recent_experiences = (
                    self.mlt_kernel.experience_collector.get_recent_experiences(5)
                )
                recent_insights = self.mlt_kernel.insight_generator.get_recent_insights(
                    5
                )
        ...
        async def create_snapshot():
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to propose snapshot: {str(e)}"
                )
