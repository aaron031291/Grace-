...
class RBACEvaluator:
    ...
    def evaluate(
        self, user_roles: List[str], action: str, resource: str, ctx: Dict
    ) -> bool:
        """
        ...
        logger.info(
            f"RBAC decision: {result} for {action} on {resource} by {user_roles}"
        )
        return result
    ...
    def _resource_matches(
        self, policy_resources: List[str], requested_resource: str, ctx: Dict
    ) -> bool:
        ...
        for policy_resource in policy_resources:
            if policy_resource == "*":
                ...
                if self._resource_pattern_matches(
                    expanded_resource, requested_resource
                ):
                    return True
