...
class I18nManager:
    ...
    def __init__(self, locales_dir: Optional[str] = None):
        self.locales_dir = locales_dir or os.path.join(
    ...
    def set_locale(self, locale_code: str) -> bool:
        ...
        logger.warning(
            f"Locale {locale_code} not available, using {self.default_locale}"
        )
        return False
    ...
    def format_date(self, date: datetime, locale: Optional[str] = None) -> str:
        ...
        if locale in self.catalogs:
            format_str = (
                self.catalogs[locale].get("formats", {}).get("date", "dd/MM/yyyy")
            )

            # Convert format string from locale format to Python format
            py_format = (
                format_str.replace("dd", "%d").replace("MM", "%m").replace("yyyy", "%Y")
            )
    ...
    def format_time(self, time: datetime, locale: Optional[str] = None) -> str:
        ...
        if locale in self.catalogs:
            format_str = (
                self.catalogs[locale].get("formats", {}).get("time", "HH:mm:ss")
            )
    ...
    def format_datetime(self, dt: datetime, locale: Optional[str] = None) -> str:
        ...
        if locale in self.catalogs:
            format_str = (
                ...
                .get("formats", {})
                .get("datetime", "dd/MM/yyyy HH:mm")
            )

            # Convert format string
            py_format = (
                format_str.replace("dd", "%d")
                .replace("MM", "%m")
                .replace("yyyy", "%Y")
                .replace("HH", "%H")
                .replace("mm", "%M")
    ...
    def format_number(self, number: float, locale: Optional[str] = None) -> str:
        ...
        if locale in self.catalogs:
            ...
            if number >= 1000:
                return (
                    f"{number:,.2f}".replace(",", "TEMP")
                    .replace(".", decimal_sep)
                    .replace("TEMP", thousand_sep)
                )
            else:
