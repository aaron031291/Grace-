...
class TestGraceTracer:
    ...
    async def test_vault_compliance_initialization(self, tracer_setup):
        ...
        # Start a trace
        trace_id = await tracer.start_trace(
            component_id="vault_test_component", operation="vault_test_operation"
        )
    ...
    async def test_trace_events_with_narrative(self, tracer_setup):
        ...
        trace_id = await tracer.start_trace(
            component_id="narrative_component", operation="decision_making"
        )
    ...
    async def test_contradiction_detection(self, tracer_setup):
        ...
        trace_id = await tracer.start_trace(
            component_id="contradiction_component", operation="test_sequence"
        )
    ...
    async def test_sandbox_isolation(self, tracer_setup):
        ...
        trace_id = await tracer.start_trace(
            component_id="external_component", operation="external_api"
        )
        ...
        assert (
            status["vault_compliance"][VaultCompliance.VAULT_15_SANDBOX.value] is True
        )
    ...
    async def test_constitutional_compliance(self, tracer_setup):
        ...
        trace_id = await tracer.start_trace(
            component_id="constitutional_component", operation="policy_change"
        )
    ...
    async def test_trust_score_updates(self, tracer_setup):
        ...
        trace_id = await tracer.start_trace(
            component_id="trust_component", operation="trust_test"
        )
    ...
    async def test_system_metrics(self, tracer_setup):
        ...
        for i in range(3):
            ...
            await tracer.add_trace_event(
                trace_id=trace_id, event_type="test", operation=f"test_event_{i}"
            )
...
async def run_basic_test():
    ...
    try:
        # Create tracer
        tracer = await create_grace_tracer(
            event_bus=event_bus, memory_core=memory_core, immutable_logs=immutable_logs
        )
        ...
        # Test basic tracing
        trace_id = await tracer.start_trace(
            component_id="test_component", operation="basic_test", user_id="test_user"
        )
