...
class TaskRequest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": self.status,
            "error": self.error,
            "ts": self.completed_at.isoformat()
...
class Scheduler:
    ...
    async def dispatch_task(
        self, loop_id: str, inputs: Dict[str, Any], priority: int = 5
    ) -> str:
        ...
        try:
            ...
            if self.event_publisher:
                await self.event_publisher(
                    "ORCH_LOOP_STARTED",
                    {"loop_id": loop.loop_id, "ts": datetime.now().isoformat()},
                )
    ...
    async def _execute_task(self, task: TaskRequest):
        ...
        try:
            ...
            if task.loop_id in self.loops:
                duration = time.time() - (
                    task.started_at.timestamp() if task.started_at else time.time()
                )
    ...
    async def _handle_expired_tasks(self):
        ...
        for task_id in expired_tasks:
            ...
            logger.warning(
                f"Task {task_id} timed out after {self.task_timeout_minutes} minutes"
            )
    ...
    async def _execute_governance_adaptation_loop(
        self, loop: LoopDefinition
    ) -> Dict[str, Any]:
    ...
    async def _execute_value_generation_loop(
        self, loop: LoopDefinition
    ) -> Dict[str, Any]:
