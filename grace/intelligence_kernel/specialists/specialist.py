...
class BaseSpecialist:
    ...
    def __init__(
        self, name: str, expertise_domain: str, confidence_threshold: float = 0.5
    ):
        self.name = name
    ...
    def _preprocess_feed(
        self, feed_items: List[QuorumFeedItem]
    ) -> List[QuorumFeedItem]:
        """Common preprocessing of feed items."""
        # Filter by relevance threshold
        filtered = [item for item in feed_items if item.relevance_score >= 0.3]

        # Sort by combined relevance and trust scores
        filtered.sort(
            key=lambda x: (x.relevance_score + x.trust_score) / 2, reverse=True
        )
