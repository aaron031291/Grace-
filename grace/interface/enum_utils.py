...
def safe_enum_parse(
    ...
    for enum_item in enum_class:
        ...
        if str_value.lower().replace("_", "") == enum_item.value.lower().replace(
            "_", ""
        ):
            return enum_item

    logger.warning(
        f"Could not parse '{value}' as {enum_class.__name__}, using default: {default}"
    )
    return default


def create_enum_mapper(
    mapping_dict: dict, enum_class: Type[E], default: Optional[E] = None
):
    """
