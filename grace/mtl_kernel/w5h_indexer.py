...
class W5HIndexer:
    ...
    def enhance_entry(self, entry: MemoryEntry) -> MemoryEntry:
        ...
        if not entry.w5h_index or not any(
            [
                entry.w5h_index.who,
                entry.w5h_index.what,
                entry.w5h_index.where,
                entry.w5h_index.why,
                entry.w5h_index.how,
            ]
        ):
    ...
    def search_by_w5h(
        ...
        for entry in entries:
            ...
            if who and not any(
                who.lower() in person.lower() for person in entry.w5h_index.who
            ):
                match = False

            if what and not any(
                what.lower() in thing.lower() for thing in entry.w5h_index.what
            ):
                match = False

            if where and not any(
                where.lower() in place.lower() for place in entry.w5h_index.where
            ):
                match = False

            if why and not any(
                why.lower() in reason.lower() for reason in entry.w5h_index.why
            ):
                match = False

            if how and not any(
                how.lower() in method.lower() for method in entry.w5h_index.how
            ):
                match = False
