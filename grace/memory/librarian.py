...
from grace.utils.time import now_utc, to_utc, iso_now_utc
...
class ConstitutionalFilter:
    ...
    def evaluate_content(self, content: str) -> Dict[str, Any]:
        ...
        constitutional_score = max(
            0, 1.0 - (len(forbidden_violations) * 0.5) - (len(warnings) * 0.2)
        )
...
class EnhancedLibrarian:
    ...
    def ingest_document(
        self, content: str, source_id: str = None, metadata: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not constitutional_result["approved"]:
                ...
                return {
                    ...
                    "constitutional_score": constitutional_result[
            ...
            for chunk in chunks:
                try:
                    ...
                    chunk_constitutional = self.constitutional_filter.evaluate_content(
                        chunk.content
                    )
                    chunk.metadata.update(
                        {
                            "constitutional_score": chunk_constitutional[
                                "constitutional_score"
                            ],
            ...
            self.lightning.put(
                doc_key, doc_metadata, ttl_seconds=7200, tags=["document", source_id]
            )

            logger.info(
                f"Ingested document {source_id}: {len(processed_chunks)} chunks processed"
            )
    ...
    def search_and_rank(
        ...
        try:
            ...
            query_keywords = self.semantic_indexer.extract_keywords(
                query, max_keywords=10
            )
            ...
            for chunk_id, score in ranked_chunks[
                ...
                if chunk_data:
                    ...
                    constitutional_score = chunk_data.get("metadata", {}).get(
    ...
    def _chunk_text(self, text: str, source_id: str) -> List[TextChunk]:
        ...
        if current_chunk.strip():
            chunk = TextChunk(
                content=current_chunk.strip(), source_id=source_id, position=position
            )
            chunks.append(chunk)
    ...
    def _calculate_trust_score(
        self, chunk_data: Dict[str, Any], relevance_score: float
    ) -> float:
