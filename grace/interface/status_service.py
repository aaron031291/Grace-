...
class GraceStatusService:
    ...
    def __init__(
        self, event_bus: EventBus, orchestration_service: OrchestrationService
    ):
    ...
    def _setup_routes(self):
        ...
        async def get_status():
            ...
            try:
                ...
                if (
                    hasattr(self.orchestration_service, "running")
                    and not self.orchestration_service.running
                ):
                    overall_status = "degraded"
        ...
        async def get_detailed_status():
            ...
            try:
                ...
                if self.orchestration_service:
                    try:
                        components["orchestration"] = {
                            "running": getattr(
                                self.orchestration_service, "running", False
                            ),
                            "started_at": getattr(
                                self.orchestration_service, "started_at", None
                            ),
                        }
                        ...
                        if hasattr(self.orchestration_service, "app"):
                            ...
                            orchestration_status = (
                                await self._get_orchestration_status()
                            )
                ...
                configuration = {
                    ...
                    if self.config["database_config"]["use_postgres"]
                    else "sqlite",
                    "redis_enabled": self.config["database_config"]["use_redis_cache"],
                    "monitoring_enabled": self.config["infrastructure_config"][
                        "enable_telemetry"
                    ],
                }
        ...
        async def get_telemetry():
            ...
            try:
                ...
                eventbus_status = (
                    self.event_bus.get_system_status() if self.event_bus else {}
                )
                ...
                if "immutable_logs" in eventbus_status:
                    ...
                    metrics["log_buffer_utilization"] = logs_data.get(
                ...
                raise HTTPException(
                    status_code=500, detail=f"Telemetry error: {str(e)}"
                )
...
async def create_status_service(
    event_bus: EventBus, orchestration_service: OrchestrationService
) -> GraceStatusService:
