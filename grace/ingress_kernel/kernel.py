...
class IngressKernel:
    ...
    def __init__(
        ...
        self.config = {
            ...
            "dedupe": {"threshold": 0.87, "strategy": "hash+fuzzy"},
            "enrichment": {
                "ner_model": "ner.en@2.4.1",
                "embeddings": "text-encoder@1.1.0",
            },
            "governance": {"retention_days_default": 365, "label_default": "internal"},
        }
    ...
    async def capture(
        self, source_id: str, payload: Any, headers: Optional[Dict[str, Any]] = None
    ) -> str:
        """
    ...
    async def export_snapshot(self) -> Dict[str, Any]:
        ...
        snapshot_payload = {
            ...
            "offsets": {
                src_id: f"current_{datetime.utcnow().isoformat()}"
                for src_id in self.sources.keys()
            },
            "watermarks": {
                src_id: datetime.utcnow().isoformat() for src_id in self.sources.keys()
            },
    ...
    async def _parse_content(self, raw_event: RawEvent) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            parser = self.parser_factory.create_parser(
                source_config.parser, source_config.parser_opts
            )
            ...
            if not result.success:
                logger.error(
                    f"Parsing failed for {raw_event.event_id}: {result.errors}"
                )
                return None
    ...
    async def _normalize_content(
        self, raw_event: RawEvent, parsed_content: Dict[str, Any]
    ) -> Optional[NormRecord]:
        ...
        norm_record = NormRecord(
            ...
            lineage=LineageInfo(
                raw_event_id=raw_event.event_id, transforms=["parse", "normalize"]
            ),
        )

        # Calculate trust score using trust scorer
        trust_score = await self.trust_scorer.compute_trust_score(
            norm_record, source_config
        )
    ...
    async def _validate_record(self, record: NormRecord) -> bool:
        ...
        try:
            ...
            passed, violations = await self.policy_guard.enforce_policies(
                record, source_config
            )

            if not passed:
                logger.warning(
                    f"Record {record.record_id} failed validation: {violations}"
                )
            ...
            try:
                for source_id in self.sources.keys():
                    self.health_status[source_id].update(
                        {"last_check": datetime.utcnow(), "status": "ok"}
                    )
