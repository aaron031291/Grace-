...
class User(Base):
    ...
    memory_fragments = relationship(
        "MemoryFragment", back_populates="user", cascade="all, delete-orphan"
    )
...
class Session(Base):
    ...
    collab_sessions = relationship(
        "CollaborationSession", back_populates="session", cascade="all, delete-orphan"
    )

    __table_args__ = (Index("ix_sessions_owner_active", "owner_id", "is_active"),)
...
class Panel(Base):
    ...
    )  # chat, memory, knowledge, task, etc.
