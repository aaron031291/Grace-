...
class DegradationManager:
    ...
    def __init__(self):
        ...
        self._active_modes: Dict[str, Set[str]] = {}  # service_id -> set of active mode_ids
    ...
    def configure_modes(self, service_id: str, degradation_modes: List[Dict]):
        ...
        logger.info(f"Configured {len(degradation_modes)} degradation modes for {service_id}")
    
    async def enter_mode(self, service_id: str, mode_id: str, reason: str = "manual") -> bool:
        """
        ...
        try:
            ...
            if mode_id not in self._mode_configs[service_id]:
                logger.warning(f"Unknown degradation mode {mode_id} for service {service_id}")
                return False
            ...
            success = await self._execute_actions(service_id, mode_config.get("actions", []), "enter")
            ...
            if success:
                ...
                logger.info(f"Service {service_id} entered degradation mode {mode_id}: {reason}")
                return True
            else:
                logger.error(f"Failed to enter degradation mode {mode_id} for service {service_id}")
                return False
                
        except Exception as e:
            logger.error(f"Error entering degradation mode {mode_id} for service {service_id}: {e}")
            return False
    
    async def exit_mode(self, service_id: str, mode_id: str, reason: str = "manual") -> bool:
        """
        ...
        """
        try:
            if service_id not in self._active_modes or mode_id not in self._active_modes[service_id]:
                logger.info(f"Service {service_id} not in degradation mode {mode_id}")
                return True
            
            # Get mode configuration
            if service_id in self._mode_configs and mode_id in self._mode_configs[service_id]:
                mode_config = self._mode_configs[service_id][mode_id]
                
                # Execute mode exit actions (reverse of entry actions)
                success = await self._execute_actions(service_id, mode_config.get("actions", []), "exit")
            else:
            ...
            if success:
                ...
                logger.info(f"Service {service_id} exited degradation mode {mode_id}: {reason}")
                return True
            else:
                logger.error(f"Failed to exit degradation mode {mode_id} for service {service_id}")
                return False
                
        except Exception as e:
            logger.error(f"Error exiting degradation mode {mode_id} for service {service_id}: {e}")
            return False
    ...
    def get_stats(self, service_id: Optional[str] = None) -> Dict:
        ...
        if service_id:
            return {
                ...
                "total_mode_changes": len([h for h in self._mode_history if h["service_id"] == service_id])
            }
        
        return {
            "total_services": len(self._mode_configs),
            "services_with_active_modes": len([s for s, modes in self._active_modes.items() if modes]),
    ...
    def get_mode_history(self, service_id: Optional[str] = None, limit: int = 100) -> List[Dict]:
    ...
    async def evaluate_triggers(self, service_id: str, signals: Dict[str, float]) -> List[str]:
        """
        ...
        for mode_id, mode_config in self._mode_configs[service_id].items():
            ...
            for trigger in triggers:
                if trigger == "high_latency" and signals.get("latency_p95_ms", 0) > 800:
                    ...
                    should_trigger = True
                elif trigger == "dependency_down" and signals.get("dependency_health", 1.0) < 0.5:
                    should_trigger = True
                elif trigger == "low_budget" and signals.get("error_budget_remaining_pct", 100) < 10:
                    should_trigger = True
    ...
    async def _execute_actions(self, service_id: str, actions: List[str], direction: str) -> bool:
...
async def enter_mode(service_id: str, mode_id: str, reason: str = "api_request") -> None:
