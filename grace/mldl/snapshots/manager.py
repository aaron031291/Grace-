...
class SnapshotManager:
    ...
    async def create_snapshot(self, description: str = None, 
    ...
    async def rollback(self, to_snapshot: str, reason: str = None, 
    ...
    async def get_snapshot(self, snapshot_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Parse JSON fields
            json_fields = ["registry_index", "search_spaces", "calibration_config",
                          "fairness_config", "deployment_policy", "metadata"]
    ...
    async def _collect_deployment_policy(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rollback_triggers": [
                "metric_drop", "drift_spike", "violation", "anomaly"
            ],
            "slo_thresholds": {
                "p95_latency_ms": 500,
                "error_rate": 0.01,
    ...
    def _calculate_state_hash(self, snapshot_payload: Dict[str, Any]) -> str:
        ...
        payload_str = json.dumps(snapshot_payload, sort_keys=True, separators=(',', ':'))
    ...
    async def get_snapshot_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            """)
            type_counts = {row["snapshot_type"]: row["count"] for row in cursor.fetchall()}
            
            # Total storage size
            cursor = self.conn.execute("SELECT SUM(size_bytes) as total_size FROM snapshots")
