...
try:
    ...
    logger.warning(
        "Scikit-learn not available. Evaluation will use mock implementations."
    )
    ...
    try:
        ...
        for group_name, group_mask in groups.items():
            if np.sum(group_mask) > 0:
                ...
                group_metrics[group_name] = evaluate(
                    "classification", group_y_true, group_y_pred, group_proba
                )
            else:
...
def _calculate_fairness_metrics(
    overall_metrics: Dict[str, float], group_metrics: Dict[str, Dict[str, float]]
) -> Dict[str, Any]:
    ...
    for metric in key_metrics:
        if metric in overall_metrics:
            ...
            if len(group_values) > 1:
                ...
                fairness_results[f"{metric}_parity"] = (
                    delta < 0.05
