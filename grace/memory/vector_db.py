...
class VectorDBInterface(ABC):
    ...
    @abstractmethod
    async def insert_vectors(self, collection_name: str, vectors: List[Dict[str, Any]]) -> List[str]:
...
class MilvusVectorDB(VectorDBInterface):
    ...
    async def create_collection(self, collection_name: str, dimension: int) -> bool:
        ...
        try:
            ...
            schema = CollectionSchema(fields, f"Grace memory collection: {collection_name}")
    ...
    async def insert_vectors(self, collection_name: str, vectors: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            ...
            data = [
                ...
                [v.get("timestamp", datetime.now().isoformat()) for v in vectors],  # timestamp
                [v.get("trust_score", 0.5) for v in vectors],  # trust_score
                [v.get("constitutional_score", 0.5) for v in vectors]  # constitutional_score
            ]
        ...
        try:
            ...
            if filters:
                ...
                if "min_constitutional_score" in filters:
                    expr_parts.append(f"constitutional_score >= {filters['min_constitutional_score']}")
                if expr_parts:
            ...
            results = collection.search(
                ...
                limit=top_k,
                expr=expr,
                output_fields=["content", "metadata", "timestamp", "trust_score", "constitutional_score"]
            )
            ...
            if results:
                for hit in results[0]:
                    formatted_results.append({
                        ...
                        "constitutional_score": float(hit.entity.get("constitutional_score", 0.0))
                    })
    ...
    async def search_vectors(self, collection_name: str, query_vector: List[float], 
        ...
        try:
            ...
            if filters:
                ...
                if "min_constitutional_score" in filters:
                    filter_dict["constitutional_score"] = {"$gte": filters["min_constitutional_score"]}
            ...
            for match in search_result.matches:
                ...
                formatted_results.append({
                    ...
                    "metadata": {k: v for k, v in metadata.items() 
                              if k not in ["content", "timestamp", "trust_score", "constitutional_score"]},
                    "timestamp": metadata.get("timestamp"),
                    "trust_score": float(metadata.get("trust_score", 0.0)),
                    "constitutional_score": float(metadata.get("constitutional_score", 0.0))
                })
...
class EmbeddingService:
    ...
    def initialize(self) -> bool:
        ...
        if not SENTENCE_TRANSFORMERS_AVAILABLE:
            logger.error("SentenceTransformers not available. Install sentence-transformers package.")
            return False
...
class VectorMemoryCore:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not await self.vector_db.create_collection(self.default_collection, dimension):
                return False
    ...
    async def store_content(self, 
                          ...
                          constitutional_score: float = 0.5) -> Optional[str]:
    ...
    async def hybrid_search(self, 
        ...
        try:
            ...
            keyword_scores = {r.get("id", ""): r.get("score", 0.0) for r in keyword_results}
