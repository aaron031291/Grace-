...
class VaultComplianceChecker:
    ...
    async def check_request_compliance(
        ...
        """
        logger.info(
            f"Checking vault compliance for request: {request.get('request_id', 'unknown')}"
        )

        # Perform comprehensive validation
        report = await self.vault_engine.validate_comprehensive(
            request, priority_only=False
        )

        return report

    async def check_priority_compliance(
        self, request: Dict[str, Any]
    ) -> ComplianceReport:
        """
        ...
        """
        logger.info(
            f"Checking priority vault compliance for request: {request.get('request_id', 'unknown')}"
        )

        # Perform priority validation only
        report = await self.vault_engine.validate_comprehensive(
            request, priority_only=True
        )
    ...
    async def validate_code_change(self, code_data: Dict[str, Any]) -> ComplianceReport:
        ...
        return await self.check_priority_compliance(
            request
        )  # Focus on priority vaults for code
    ...
    def get_vault_requirements_summary(self) -> str:
        ...
        for vault_id, vault in vaults.items():
            ...
            summary += (
                f"Watermark Required: {'Yes' if vault.watermark_required else 'No'}\n"
            )
