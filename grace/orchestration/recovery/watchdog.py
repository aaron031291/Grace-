...
class HealthMetrics:
    ...
    def is_healthy(self, thresholds: Dict[str, float]) -> bool:
        ...
        return (
            self.cpu_usage <= thresholds.get("cpu_max", 90.0) and
...
class MonitoredTask:
    ...
    def __init__(self, task_id: str, loop_id: str, kernel: str, 
...
class KernelMonitor:
    ...
    def update_health_metrics(self, metrics: HealthMetrics):
        ...
        if metrics.is_healthy(self.health_thresholds):
            ...
            if self.consecutive_failures >= 3:
                ...
                logger.warning(f"Kernel {self.kernel_name} is failing (consecutive failures: {self.consecutive_failures})")
    ...
    def should_isolate(self) -> bool:
        ...
        return (
            self.status == KernelStatus.FAILING and
            self.consecutive_failures >= 5 and
            not self.is_isolated()
        )
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "isolated_since": self.isolated_since.isoformat() if self.isolated_since else None,
            ...
            "latest_metrics": {
                ...
                "last_heartbeat": latest_metrics.last_heartbeat.isoformat() if latest_metrics else None
...
class Watchdog:
    ...
    def _initialize_kernel_monitors(self):
        ...
        known_kernels = [
            "orchestration", "governance", "memory", "learning", "intelligence",
            "interface", "ingress", "mlt", "mtl", "multi_os", "resilience", "event_mesh"
        ]
    ...
    def monitor(self, task_id: str, loop_id: str = None, kernel: str = None,
               callback: Callable = None, timeout_minutes: int = 30):
    ...
    async def isolate(self, kernel: str, strategy: IsolationStrategy = IsolationStrategy.CIRCUIT_BREAKER,
        ...
        if self.event_publisher:
            await self.event_publisher("KERNEL_ISOLATED", {
                "kernel": kernel,
            ...
            while self.running:
                ...
                for kernel_name, monitor in self.kernel_monitors.items():
                    if monitor.is_isolated():
                        ...
                        isolation_duration = (current_time - monitor.isolated_since).total_seconds()
                        
                        if isolation_duration > self.isolation_timeout:
                            # Attempt recovery
                            logger.info(f"Attempting recovery for isolated kernel: {kernel_name}")
    ...
    async def _handle_expired_task(self, task_id: str):
        ...
        # Trigger callbacks
        await task.trigger_callbacks("expired", {
        ...
        # Publish event
        if self.event_publisher:
            await self.event_publisher("TASK_EXPIRED", {
                "task_id": task_id,
    ...
    async def _handle_stale_task(self, task_id: str):
        ...
        # Trigger callbacks
        await task.trigger_callbacks("stale", {
