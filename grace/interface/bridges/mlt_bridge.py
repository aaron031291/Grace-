...
class MLTBridge:
    ...
    async def get_adaptation_plans(self, limit: int = 10, status_filter: Optional[str] = None) -> List[Dict]:
    ...
    async def _structure_plan_summary(self, plan: Any) -> Dict:
        ...
        summary = {
            ...
            "created_at": getattr(plan, 'created_at', now_utc()).isoformat(),
    ...
    def _generate_plan_title(self, plan: Any) -> str:
        ...
        if hasattr(plan, 'actions') and plan.actions:
            action_types = set(getattr(action, 'type', 'unknown') for action in plan.actions)
    ...
    def _generate_plan_description(self, plan: Any) -> str:
        ...
        return '; '.join(description_parts) if description_parts else "Automated system adaptation"
        ...
        try:
            ...
            governance_request = {
                ...
                "context": {
                    ...
                    "impact_level": plan_summary.get("summary", {}).get("impact_assessment"),
    ...
    async def get_experience_data(self, limit: int = 20) -> List[Dict]:
        ...
        try:
            if self.mlt_kernel and hasattr(self.mlt_kernel, 'experience_collector'):
                raw_experiences = self.mlt_kernel.experience_collector.get_recent_experiences(limit)
    ...
    async def get_insights(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            if self.mlt_kernel and hasattr(self.mlt_kernel, 'insight_generator'):
                raw_insights = self.mlt_kernel.insight_generator.get_recent_insights(limit)
