...
async def test_basic_tracing():
    ...
    # Test async span
    async with tracer.async_span("async_test_operation", kind=SpanKind.INTERNAL) as span:
...
async def test_trace_collector():
    ...
    for i in range(10):
        with tracer.span(f"test_operation_{i}", tags={"iteration": i}) as span:
            ...
            # Create child span
            with tracer.span(f"child_operation_{i}", parent_context=span.context) as child_span:
    ...
    if all_spans:
        ...
        print(f"✅ Trace summary: {summary['span_count']} spans, {summary['duration_ms']:.2f}ms")
...
async def test_trace_correlation():
    ...
    return len(correlated_spans) >= 6  # Should have at least 6 spans (parent + 5 children)
