...
class SnapshotManager:
    ...
    def get_snapshot(self, snapshot_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "slo_policies": json.loads(row["slo_policies"])
                if row["slo_policies"]
                else {},
                "resilience_policies": json.loads(row["resilience_policies"])
                if row["resilience_policies"]
                else {},
                "dependency_graphs": json.loads(row["dependency_graphs"])
                if row["dependency_graphs"]
                else {},
                "circuit_states": json.loads(row["circuit_states"])
                if row["circuit_states"]
                else {},
                "degradation_states": json.loads(row["degradation_states"])
                if row["degradation_states"]
                else {},
                "chaos_config": json.loads(row["chaos_config"])
                if row["chaos_config"]
                else {},
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                if record["affected_services"]:
                    record["affected_services"] = json.loads(
    ...
    async def _collect_current_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dependency_graphs": {},
            "circuit_states": {"service_a/dependency_x": "closed"},
            "degradation_states": {"service_a": []},
            "chaos_config": {"enabled": True, "max_blast_radius_pct": 5},
        }
