...
class ValidationResult:
    ...
    def __init__(
        self, passed: bool = True, errors: List[str] = None, warnings: List[str] = None
    ):
        self.passed = passed
...
def _validate_payload_consistency(gme: GraceMessageEnvelope, result: ValidationResult):
    ...
    if has_payload and gme.payload:
        ...
        if payload_size > 262144:  # 256KB
            result.add_warning(
                f"Large payload ({payload_size} bytes) should use payload_ref"
            )
...
def _validate_headers(headers, result: ValidationResult):
    ...
    if headers.hop_count > 32:
        result.add_warning(
            f"High hop count ({headers.hop_count}) may indicate routing loops"
        )


def _validate_governance_requirements(
    gme: GraceMessageEnvelope, result: ValidationResult
):
    ...
    if "deployment" in payload:
        ...
        for field in required_fields:
            if field not in payload.get("deployment", {}):
                result.add_error(
                    f"MLDL deployment payload missing required field: {field}"
                )
