...
class TraceSpan:
    ...
    def set_error(self, error: Exception) -> None:
        ...
        self.log(
            "error", {"error.object": str(error), "error.kind": type(error).__name__}
        )
...
class GTracer:
    ...
    def __init__(
        self, service_name: str = "grace", collector_endpoint: Optional[str] = None
    ):
    ...
    def finish_span(self, span: TraceSpan) -> None:
        ...
        logger.debug(
            f"Finished span {span.operation_name} ({span.context.span_id}) "
    ...
    def get_trace(self, trace_id: str) -> List[TraceSpan]:
        ...
        # Check active spans
        active = [
            s for s in self._active_spans.values() if s.context.trace_id == trace_id
        ]

        # Check finished spans
        finished = [s for s in self._finished_spans if s.context.trace_id == trace_id]
