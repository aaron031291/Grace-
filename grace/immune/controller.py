...
class GraceImmuneController:
    ...
    def _register_event_handlers(self):
        ...
        if hasattr(self.event_bus, 'subscribe'):
            ...
            self.event_bus.subscribe(EventTypes.ADV_TEST_FAILED, self._handle_adversarial_test_failed)
            
            # Subscribe to anomaly detections
            self.event_bus.subscribe(EventTypes.ANOMALY_DETECTED, self._handle_anomaly_detected)
            
            # Subscribe to resilience events for integration
            self.event_bus.subscribe(EventTypes.RESILIENCE_INCIDENT_OPENED, self._handle_resilience_incident)
    ...
    async def _handle_resilience_incident(self, message: GraceMessageEnvelope):
        ...
        try:
            ...
            if component in self.sandboxed_components:
                ...
                await self._publish_immune_event(EventTypes.IMMUNE_SANDBOXED, {
                    "component": component,
            ...
            if component in self.circuit_breakers:
                ...
                if breaker.state == "open":
                    await self._publish_immune_event("CIRCUIT_BREAKER_ACTIVE", {
                        "component": component,
                        "incident_id": incident_id,
    ...
    async def _process_adversarial_signal(self, signal: AdversarialSignal):
        ...
        try:
            ...
            if response_type:
                ...
                await self._publish_immune_event(f"IMMUNE_{response_type.value.upper()}", {
    ...
    def _update_threat_patterns(self, signal: AdversarialSignal):
        ...
        self.threat_patterns[pattern_key] = [
            s for s in self.threat_patterns[pattern_key]
            if s.timestamp > time_cutoff
        ]
    
    async def _determine_immune_response(self, signal: AdversarialSignal) -> Optional[ImmuneResponseType]:
    ...
    async def _execute_immune_response(self, signal: AdversarialSignal, response_type: ImmuneResponseType) -> ImmuneAction:
        ...
        try:
            ...
            logger.info(f"Executed immune response {response_type.value} for component {signal.component}")
            return action
            
        except Exception as e:
            logger.error(f"Failed to execute immune response {response_type.value}: {e}")
            raise
    ...
    async def _apply_circuit_breaker(self, component: str, action: ImmuneAction):
        ...
        if breaker.failure_count >= breaker.failure_threshold:
            ...
            asyncio.create_task(self._schedule_circuit_recovery(component, breaker.recovery_timeout))
    ...
    async def _apply_sandboxing(self, component: str, action: ImmuneAction):
        ...
        asyncio.create_task(self._schedule_sandbox_removal(component, sandbox_config["timeout"]))
    ...
    async def _apply_isolation(self, component: str, action: ImmuneAction):
        ...
        if self.governance_bridge:
            await self.governance_bridge.request_approval(
                ...
                component=component,
                reason=f"Immune response to adversarial signals",
    ...
    async def _request_rollback(self, component: str, action: ImmuneAction):
        ...
        await self._publish_immune_event(EventTypes.ROLLBACK_REQUESTED, {
            "target": component,
    ...
    async def _update_trust_scores(self, signal: AdversarialSignal, action: ImmuneAction):
        ...
        await self._publish_immune_event(EventTypes.TRUST_UPDATED, {
            "component": component,
            "trust_score": new_trust,
    ...
    async def inject_adversarial_signal(self, 
                                      test_type: str,
                                      component: str,
                                      severity: str,
                                      confidence: float,
