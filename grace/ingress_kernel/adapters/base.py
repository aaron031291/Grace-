...
class BaseAdapter(ABC):
    ...
    def _create_raw_event(
        self, payload: Any, headers: Optional[Dict] = None, offset: Optional[str] = None
    ) -> RawEvent:
        ...
        try:
            ...
            for item in mock_items:
                if item["guid"] not in self.seen_guids:
                    ...
                    yield self._create_raw_event(
                        payload=item, offset=f"rss_{item['guid']}"
                    )
...
class GitHubAdapter(BaseAdapter):
    ...
    def _parse_github_uri(self):
        ...
        if "github.com" in self.source_config.uri:
            path_parts = (
                urllib.parse.urlparse(self.source_config.uri).path.strip("/").split("/")
            )
            if len(path_parts) >= 2:
    ...
    async def connect(self) -> bool:
        ...
        try:
            logger.info(
                f"Testing GitHub connection to {self.repo_owner}/{self.repo_name}"
            )
            return True
        except Exception as e:
