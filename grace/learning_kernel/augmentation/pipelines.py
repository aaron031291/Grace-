...
class AugmentationPipelines:
    ...
    def _apply_text_augmentation(
        self, dataset_id: str, version: str, ops: List[Dict]
    ) -> Dict[str, Any]:
    ...
    def _apply_tabular_augmentation(
        self, dataset_id: str, version: str, ops: List[Dict]
    ) -> Dict[str, Any]:
    ...
    def _store_augment_spec(self, spec: Dict[str, Any]):
        ...
        try:
            ...
            """,
                (spec["spec_id"], spec["modality"], json.dumps(spec["ops"])),
            )
            conn.commit()
        finally:
            conn.close()

    def _record_augmentation_application(
        self, dataset_id: str, version: str, spec_id: str, result: Dict[str, Any]
    ):
        """Record augmentation application in database."""
        conn = sqlite3.connect(self.db_path)
        try:
            application_id = (
                f"aug_app_{dataset_id}_{version}_{datetime.now().strftime('%H%M%S')}"
            )
    ...
    def estimate_augmentation_impact(
        self, dataset_id: str, spec: Dict[str, Any]
    ) -> Dict[str, Any]:
