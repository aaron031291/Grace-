...
class ModelSpec(BaseModel):
    ...
    hyperparams: Dict[str, Any] = Field({}, description="Hyperparameters or search space")
...
class MLDLService:
    ...
    def _register_routes(self):
        ...
        async def train_model(request: TrainingJobRequest, background_tasks: BackgroundTasks):
        ...
        @self.app.get("/api/mldl/v1/jobs/{job_id}/status", response_model=JobStatusResponse)
        ...
        async def evaluate_model(request: EvaluationRequest):
            ...
            try:
                ...
                model_bundle = await self.registry.get(request.model_key, request.version)
                if not model_bundle:
    ...
    async def _run_training_job(self, job_id: str, job_spec: Dict[str, Any]):
        ...
        try:
            ...
            if self.event_bus:
                await self.event_bus.publish("MLDL_TRAINING_STARTED", {
                    "job_id": job_id,
                    "job": job_spec
                })
            ...
            if self.event_bus:
                await self.event_bus.publish("MLDL_CANDIDATE_READY", {
                    "job_id": job_id,
                    "bundle": result
                })
    ...
    async def _run_evaluation(self, eval_spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        evaluation_report = {
            ...
            "fairness": {
                "delta": 0.02,
                "groups": eval_spec.get("fairness_groups", [])
            },
            "drift_baseline": {
                "psi_threshold": 0.25,
                "kl_threshold": 0.1
        ...
        if self.event_bus:
            await self.event_bus.publish("MLDL_EVALUATED", {
