...
class PolicyService:
    ...
    def create_policy(self, policy: Dict[str, Any], overwrite: bool = False) -> str:
        ...
        try:
            if overwrite:
                conn.execute(
                    "DELETE FROM label_policies WHERE policy_id = ?", (policy_id,)
                )
    ...
    def get_policy(self, policy_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "pii": {"allow": bool(row["allow_pii"]), "mask": bool(row["mask_pii"])},
    ...
    def validate_label_quality(
        self, label: Dict[str, Any], policy_id: str
    ) -> Dict[str, Any]:
        ...
        if not policy["pii"]["allow"]:
            ...
            for indicator in pii_indicators:
                if (
                    ...
                    or indicator.lower() in evidence.lower()
                ):
                    if policy["pii"]["mask"]:
                        warnings.append(
                            f"Potential PII detected: {indicator} (should be masked)"
                        )
                    else:
    ...
    def calculate_task_quality_metrics(
        ...
        try:
            ...
            agreements = [
                row["agreement"] for row in labels if row["agreement"] is not None
            ]
            confidences = [
                row["confidence"] for row in labels if row["confidence"] is not None
            ]
            ...
            inter_annotator_agreement = self._calculate_inter_annotator_agreement(
                task_id
            )
            ...
            return {
                ...
                "quality_score": round(
                    (avg_agreement + gold_accuracy + inter_annotator_agreement) / 3, 3
                ),
            }
