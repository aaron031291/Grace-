...
from .component_manifest import (
    GraceComponentManifest, ComponentRole, TrustLevel, 
    ActivationState, ComponentDependency
)
...
class GraceOrchestrator:
    ...
    def register_component(self, component: BaseComponent,
                          manifest: GraceComponentManifest) -> bool:
        ...
        try:
            ...
            if component.metadata.component_type != manifest.component_type:
                logger.error(f"Component type mismatch: {component.metadata.component_type} != {manifest.component_type}")
                return False
            ...
            manifest.set_activation_state(ActivationState.REGISTERED, "Registered with orchestrator")
            ...
            if self.event_bus:
                self.event_bus.register_component(
                    component_id, manifest.component_type,
            ...
            logger.info(f"Registered component: {component_id} ({manifest.component_type})")
            return True
    ...
    def unregister_component(self, component_id: str) -> bool:
        ...
        try:
            ...
            manifest.set_activation_state(ActivationState.DECOMMISSIONED, "Unregistered from orchestrator")
    ...
    def _update_dependency_graph(self, component_id: str, 
    ...
    def get_activation_order(self, component_ids: Optional[List[str]] = None) -> List[str]:
    ...
    def _topological_sort(self, component_ids: List[str]) -> List[str]:
        ...
        def visit(component_id: str):
            ...
            for dep_id in self.dependency_graph.get(component_id, set()):
                if dep_id in component_ids:  # Only consider components we're actually sorting
                    visit(dep_id)
    ...
    async def activate_component(self, component_id: str) -> bool:
        ...
        try:
            ...
            # Update manifest state
            manifest.set_activation_state(ActivationState.INITIALIZING, "Starting activation")
            ...
            try:
                ...
                manifest.record_error(f"Activation timeout ({self.activation_timeout_sec}s)")
                return False
            
            if success:
                manifest.set_activation_state(ActivationState.ACTIVE, "Successfully activated")
    ...
    async def deactivate_component(self, component_id: str) -> bool:
        ...
        try:
            ...
            if dependents:
                logger.warning(f"Component {component_id} has active dependents: {dependents}")
            ...
            if success:
                manifest.set_activation_state(ActivationState.REGISTERED, "Successfully deactivated")
                self.orchestration_metrics["components_active"] -= 1
        ...
        for dep in manifest.dependencies:
            ...
            if dep_component_id not in self.registered_components:
                if dep.fallback_strategy:
                    logger.info(f"Using fallback strategy for missing dependency: {dep_component_id}")
                    continue
    ...
    async def stop_orchestration(self) -> bool:
        ...
        try:
            ...
            active_components = [
                comp_id for comp_id, comp in self.registered_components.items()
    ...
    async def _perform_health_checks(self):
        ...
        active_components = [
            (comp_id, comp) for comp_id, comp in self.registered_components.items()
            if comp.get_status() == ComponentStatus.ACTIVE
        ]
