...
class EnvironmentLoader:
    ...
    def load_environment(self) -> Dict[str, Any]:
        ...
        config["environment_config"]["instance_id"] = os.getenv(
            "GRACE_INSTANCE_ID", config["environment_config"]["instance_id"]
        )
        config["environment_config"]["version"] = os.getenv(
            "GRACE_VERSION", config["environment_config"]["version"]
        )
        config["environment_config"]["log_level"] = os.getenv(
            "GRACE_LOG_LEVEL", config["environment_config"]["log_level"]
        )
        config["environment_config"]["debug_mode"] = (
            os.getenv("GRACE_DEBUG", "false").lower() == "true"
        )

        # Load service configuration
        config["environment_config"]["api_host"] = os.getenv(
            "API_HOST", config["environment_config"]["api_host"]
        )
        config["environment_config"]["api_port"] = int(
            os.getenv("API_PORT", str(config["environment_config"]["api_port"]))
        )
        config["environment_config"]["orchestrator_port"] = int(
            os.getenv(
                "ORCHESTRATOR_PORT",
        ...
        config["infrastructure_config"]["enable_telemetry"] = (
            os.getenv("ENABLE_TELEMETRY", "true").lower() == "true"
        )
        config["infrastructure_config"]["enable_health_monitoring"] = (
            os.getenv("ENABLE_HEALTH_MONITORING", "true").lower() == "true"
        )
        config["infrastructure_config"]["auto_rollback_enabled"] = (
            os.getenv("AUTO_ROLLBACK_ENABLED", "true").lower() == "true"
        )
        config["infrastructure_config"]["governance_strict_mode"] = (
            os.getenv("GOVERNANCE_STRICT_MODE", "true").lower() == "true"
        )
        config["infrastructure_config"]["constitutional_enforcement"] = (
            os.getenv("CONSTITUTIONAL_ENFORCEMENT", "true").lower() == "true"
        )

        # Load feature flags
        config["database_config"]["use_postgres"] = (
            os.getenv("USE_POSTGRES", "true").lower() == "true"
        )
        config["database_config"]["use_redis_cache"] = (
            os.getenv("USE_REDIS_CACHE", "true").lower() == "true"
        )
        config["database_config"]["use_chroma_vectors"] = (
            os.getenv("USE_CHROMA_VECTORS", "true").lower() == "true"
        )
