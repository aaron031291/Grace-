...
class SandboxPolicyValidator:
    ...
    def validate_sandbox_config(self, config_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            result["compliance_score"] = found_policies / len(
                self.required_sandbox_policies
            )
            ...
            if not policy_validation.get("required", False):
                result["errors"].append(
                    "Policy validation not required for sandbox branches"
                )

            result["valid"] = (
                len(result["errors"]) == 0 and result["compliance_score"] >= 0.8
            )
    ...
    def create_default_sandbox_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sandbox": {
                ...
                "automated_testing": {
                    "required": True,
                    "test_suites": ["unit_tests", "integration_tests", "policy_tests"],
