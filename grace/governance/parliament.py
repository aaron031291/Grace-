...
class ParliamentMember:
    ...
    def __init__(
        self, member_id: str, name: str, expertise: List[str], weight: float = 1.0
    ):
...
class Parliament:
    ...
    def _initialize_default_members(self) -> Dict[str, ParliamentMember]:
        ...
        default_members = [
            ParliamentMember(
                ...
                ["ethics", "fairness", "harm_prevention"],
                1.2,
            ),
            ParliamentMember(
                "tech_lead", "Technical Lead", ["security", "privacy", "technical"], 1.1
            ),
            ParliamentMember(
                "transparency_officer",
            ...
            ParliamentMember(
                "user_advocate",
                "User Advocate",
                ["user_rights", "accessibility", "usability"],
                1.0,
            ),
            ParliamentMember(
                "legal_counsel",
                "Legal Counsel",
                ["legal", "compliance", "constitutional"],
                1.2,
            ),
            ParliamentMember(
                "domain_expert_1",
                "Domain Expert (AI/ML)",
                ["machine_learning", "ai_safety"],
                1.0,
            ),
            ParliamentMember(
                "domain_expert_2",
    ...
    async def _setup_event_subscriptions(self):
        ...
        await self.event_bus.subscribe(
            "GOVERNANCE_NEEDS_REVIEW", self._handle_review_request
        )

        await self.event_bus.subscribe("PARLIAMENT_VOTE_CAST", self._handle_vote_cast)
    ...
    async def _assign_reviewers(
        self, proposal: ReviewProposal, context: Optional[Dict[str, Any]]
    ) -> List[str]:
        ...
        min_reviewers = {"constitutional": 5, "policy": 4, "operational": 3}
    ...
    async def _notify_reviewers(
        self, proposal: ReviewProposal, assigned_reviewers: List[str]
    ):
    ...
    async def cast_vote(
        self, proposal_id: str, member_id: str, vote: VoteType, rationale: str = ""
    ) -> bool:
        """
        ...
        if approval_ratio >= threshold and participation_ratio >= 0.6:
            ...
            if approval_ratio > rejection_ratio:
                ...
                await self._finalize_proposal(
                    proposal, "approved_by_deadline", approval_ratio
                )
            else:
                proposal.status = ReviewStatus.NEEDS_REVISION
                await self._finalize_proposal(
                    proposal, "needs_revision", rejection_ratio
                )

    async def _finalize_proposal(
        self, proposal: ReviewProposal, outcome: str, final_ratio: float
    ):
        ...
        experience = Experience(
            ...
            success_score=final_ratio
            if outcome.startswith("approved")
            else 1.0 - final_ratio,
        ...
        logger.info(
            f"Finalized proposal {proposal.proposal_id}: {outcome} (ratio: {final_ratio:.3f})"
        )
