...
class DatasetRegistry:
    ...
    def publish_version(
        ...
        try:
            ...
            raise ValueError(
                f"Version {version} already exists for dataset {dataset_id}"
            )
        finally:
            conn.close()
    ...
    def get_version(self, dataset_id: str, version: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "size": {"rows": row["row_count"] or 0, "bytes": row["byte_size"] or 0},
    ...
    def update_version_stats(
        self, dataset_id: str, version: str, stats: Dict[str, Any]
    ):
