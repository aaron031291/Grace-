...
def evaluate_sli(samples: dict, slo: dict) -> dict:
    ...
    try:
        ...
        return {"sli": slo.get("sli", "unknown"), "status": "error", "message": str(e)}
...
def _evaluate_drift_psi(samples: dict, objective: float, window: str) -> dict:
    ...
    for expected, actual in zip(expected_dist, actual_dist):
        if expected > 0 and actual > 0:
            psi += (actual - expected) * (
                log(actual / expected) if actual > 0 and expected > 0 else 0
            )
...
class SLIMonitor:
    ...
    def add_samples(self, service_id: str, samples: Dict[str, Any]):
        ...
        sample_entry = {"timestamp": timestamp.isoformat(), "samples": samples}
    ...
    def get_breach_summary(
        self, service_id: Optional[str] = None, hours: int = 24
    ) -> Dict:
        ...
        if service_id:
            recent_evaluations = [
                e for e in recent_evaluations if e.get("service_id") == service_id
            ]
    ...
    def _get_windowed_samples(self, service_id: str, window: str) -> Dict:
        ...
        recent_samples = [
            entry
            for entry in self.sample_buffers[service_id]
