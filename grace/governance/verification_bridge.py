...
class VerificationBridge:
    ...
    def __init__(self):
        ...
        self._blocked_sources = {"anonymous", "unknown", "guest", "temp", "test"}
    ...
    def verify(self, request) -> Dict[str, Any]:
        ...
        details = {check_name: passed for check_name, passed in checks}
    ...
    def verify_signature(self, request, signature: str) -> bool:
        ...
        try:
            ...
            if not request_content:
                logger.warning(
                    "Unable to extract content from request for signature verification"
                )
                return False
    ...
    def verify_source(self, requester: str, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            is_trusted = any(
                trusted in requester.lower() for trusted in self._trusted_domains
            )
    ...
    def _validate_context(self, requester: str, context: Dict[str, Any]) -> bool:
        ...
        if "source_ip" in context:
            ...
            if (
                ...
                or not ip.replace(".", "").replace(":", "").isdigit()
            ):
                return False

        return True

    def _perform_additional_source_checks(
        self, requester: str, context: Dict[str, Any]
    ) -> bool:
        ...
        if not context.get("session_valid") and not context.get("token_valid"):
            ...
            if (
                "system" not in requester.lower()
                and "internal" not in requester.lower()
            ):
                return False
