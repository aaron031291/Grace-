...
class TriggerLedger:
    ...
    def get_events_for_target(self, target_id: str) -> List[TriggerEvent]:
        ...
        return [
            event for event in self.store.trigger_events if event.target_id == target_id
        ]
    ...
    def get_events_by_type(self, event_type: str) -> List[TriggerEvent]:
        ...
        return [
            event
            for event in self.store.trigger_events
    ...
    def get_recent_events(self, limit: int = 10) -> List[TriggerEvent]:
        ...
        return sorted(
            self.store.trigger_events, key=lambda x: x.created_at, reverse=True
        )[:limit]
