...
class MLTKernelML:
    ...
    async def _process_learning_cycle(self):
        ...
        try:
            ...
            if len(experiences) < 5:  # Need sufficient data
                logger.debug(
                    f"Insufficient experiences ({len(experiences)}) for learning cycle"
                )
                return
            ...
            adaptation_plan = await self.adaptation_planner.create_adaptation_plan(
                insights
            )
            ...
            policy_recommendations = (
                await self.policy_tuner.generate_policy_recommendations(
                    insights, current_policies
                )
            )

            # 5. Submit plan to governance for approval
            proposal_id = await self.gov_bridge.submit_plan_for_approval(
                adaptation_plan
            )

            logger.info(
                f"Completed learning cycle: {len(experiences)} exp -> {len(insights)} insights -> 1 plan -> proposal {proposal_id}"
            )
            ...
            if self.event_bus:
                for insight in insights:
                    await self.event_bus.publish(
                        "MLT_INSIGHT_READY", {"payload": insight.to_dict()}
                    )
    ...
    async def _subscribe_to_events(self):
        ...
        await self.event_bus.subscribe(
            "GOVERNANCE_APPROVED", self._handle_governance_approved
        )
        await self.event_bus.subscribe(
            "GOVERNANCE_REJECTED", self._handle_governance_rejected
        )

        # Subscribe to rollback requests
        await self.event_bus.subscribe(
            "ROLLBACK_REQUESTED", self._handle_rollback_request
        )
        ...
        try:
            ...
            if latest_snapshot:
                ...
                if success and self.event_bus:
                    ...
                    logger.info(
                        f"Rollback completed to snapshot {latest_snapshot.snapshot_id}"
                    )
            ...
            for name, component in components:
                try:
                    ...
                    health["components"][name] = (
                        "healthy" if component_healthy else "degraded"
                    )
