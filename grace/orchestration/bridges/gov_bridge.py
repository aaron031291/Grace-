...
class GovernanceBridge:
    ...
    async def validate_orchestration_decision(
    ...
    async def _perform_governance_validation(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Perform actual governance validation."""
        if self.governance_kernel and hasattr(
            self.governance_kernel, "validate_decision"
        ):
    ...
    async def _basic_governance_validation(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_policy_violations(
        ...
        if critical_violations:
            ...
            logger.error(
                f"Blocking operation due to critical violations: {critical_violations}"
            )
            ...
            self.enforcement_actions += 1
            logger.warning("Multiple violations detected, taking enforcement action")
    ...
    async def validate_loop_execution(
        self, loop_id: str, loop_config: Dict[str, Any]
    ) -> bool:
    ...
    async def validate_task_dispatch(
        self, task_id: str, loop_id: str, inputs: Dict[str, Any]
    ) -> bool:
    ...
    async def validate_scaling_action(
        self, action: str, loop_id: str, current_instances: int, target_instances: int
    ) -> bool:
    ...
    async def validate_rollback_request(
        self, snapshot_id: str, reason: str, authorized_by: str = None
    ) -> bool:
    ...
    async def report_governance_violation(self, violation: Dict[str, Any]):
        ...
        try:
            if self.governance_kernel and hasattr(
                self.governance_kernel, "record_violation"
            ):
    ...
    async def get_governance_policies(
        ...
        try:
            if self.governance_kernel and hasattr(
                self.governance_kernel, "get_policies"
            ):
                return await self.governance_kernel.get_policies(
                    scope="orchestration", policy_type=policy_type
                )
            else:
