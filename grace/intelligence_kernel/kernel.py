...
class IntelligenceKernel:
    ...
    def consensus(self, feed_ids: List[str], context: Optional[Dict] = None) -> QuorumResult:
        """
        ...
        specialists = [
            s for s in self.registry.get_applicable_specialists(context)
    ...
    def _convert_feed_ids_to_items(self, feed_ids: List[str]) -> List[QuorumFeedItem]:
        ...
        if not self.mtl_kernel:
            ...
            for i, memory_id in enumerate(feed_ids[:5]):  # Limit to 5 items
                feed_items.append(QuorumFeedItem(
                    memory_id=memory_id,
        ...
        for memory_id in feed_ids[:10]:  # Limit to 10 items for performance
            try:
                ...
                if memory_entry:
                    # Get trust score
                    trust_score = self.mtl_kernel.trust_service.get_trust_score(memory_id)
    ...
    def get_specialist_analysis(self, specialist_name: str, feed_ids: List[str], context: Optional[Dict] = None) -> Optional[Dict]:
