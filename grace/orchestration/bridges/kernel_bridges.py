...
class KernelBridges:
    ...
    def __init__(self, event_publisher=None):
        ...
        self.known_kernels = {
            "governance": {
                ...
                "endpoints": {
                    "validate": "/api/gov/v1/validate",
                    "policies": "/api/gov/v1/policies",
                },
            },
            "memory": {
                "capabilities": [
                ...
                "endpoints": {
                    "store": "/api/memory/v1/store",
                    "query": "/api/memory/v1/query",
                },
            },
            "learning": {
                "capabilities": ["pattern_recognition", "adaptation", "meta_learning"],
                "endpoints": {
                    "learn": "/api/learn/v1/learn",
                    "adapt": "/api/learn/v1/adapt",
                },
            },
            "intelligence": {
                "capabilities": ["decision_making", "problem_solving", "reasoning"],
                "endpoints": {
                    "analyze": "/api/intel/v1/analyze",
                    "decide": "/api/intel/v1/decide",
                },
            },
            "interface": {
            ...
            "ingress": {
                ...
                "endpoints": {
                    "ingest": "/api/ingress/v1/ingest",
                    "sources": "/api/ingress/v1/sources",
                },
            },
            "mlt": {
                "capabilities": [
                ...
                "endpoints": {
                    "trust": "/api/mlt/v1/trust",
                    "models": "/api/mlt/v1/models",
                },
            },
            "multi_os": {
                "capabilities": [
                ...
                "endpoints": {
                    "health": "/api/os/v1/health",
                    "resources": "/api/os/v1/resources",
                },
            },
            "resilience": {
                "capabilities": [
                ...
                "endpoints": {
                    "health": "/api/resilience/v1/health",
                    "recover": "/api/resilience/v1/recover",
                },
            },
            "event_mesh": {
                "capabilities": ["event_routing", "message_queuing", "pub_sub"],
                "endpoints": {
                    "publish": "/api/mesh/v1/publish",
                    "subscribe": "/api/mesh/v1/subscribe",
                },
            },
        }
    ...
    async def register_kernel(
        ...
        try:
            ...
            if capabilities:
                ...
                kernel_info.capabilities.update(
                    self.known_kernels[kernel_name]["capabilities"]
                )

            # Set default endpoints
            if not endpoints and kernel_name in self.known_kernels:
                kernel_info.endpoints.update(
                    self.known_kernels[kernel_name]["endpoints"]
                )
            ...
            logger.info(
                f"Successfully registered kernel {kernel_name} with {len(kernel_info.capabilities)} capabilities"
            )
    ...
    async def send_message(
        self, kernel_name: str, method: str, params: Dict[str, Any] = None
    ) -> Optional[Any]:
    ...
    async def broadcast_message(
        self, method: str, params: Dict[str, Any] = None, capability_filter: str = None
    ) -> Dict[str, Any]:
        ...
        for kernel_name, kernel_info in self.kernels.items():
            ...
            if heartbeat_age > self.heartbeat_timeout_seconds:
                if kernel_info.status != KernelStatus.OFFLINE:
                    logger.warning(
                        f"Kernel {kernel_name} heartbeat timeout ({heartbeat_age:.1f}s)"
                    )
    ...
    def get_registered_kernels(
        self, status_filter: KernelStatus = None, capability_filter: str = None
    ) -> List[Dict[str, Any]]:
    ...
    def get_kernel_by_capability(self, capability: str) -> List[str]:
        ...
        for kernel_name, kernel_info in self.kernels.items():
            if (
                capability in kernel_info.capabilities
                and kernel_info.status == KernelStatus.HEALTHY
            ):
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "kernels": {name: info.to_dict() for name, info in self.kernels.items()},
        }
