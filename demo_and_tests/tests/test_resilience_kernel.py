...
async def test_circuit_breaker():
    ...
    try:
        ...
        print(
            f"   ✓ Circuit breaker stats: {stats['state']}, {stats['failure_count']} failures"
        )
...
async def test_degradation_manager():
    ...
    try:
        ...
        print(
            f"   ✓ Degradation stats: {len(stats['configured_modes'])} modes configured"
        )
...
async def test_sli_evaluation():
    ...
    try:
        ...
        print(
            f"   ✓ P95 latency: {result['current_value']:.1f}ms (status: {result['status']})"
        )
        ...
        print(
            f"   ✓ Availability: {result['current_value']:.2f}% (status: {result['status']})"
        )
        ...
        monitor.register_slo(
            "test_service",
            {
                "slos": [{"sli": "latency_p95_ms", "objective": 800, "window": "30d"}],
                "error_budget_days": 0.5,
            },
        )
...
async def test_error_budget():
    ...
    try:
        ...
        tracker.record_breach(
            "test_service", "availability_pct", 0.1, 300
        )  # 5 min breach
        ...
        print(
            f"   ✓ Deployment decision: {'BLOCK' if deployment_decision['should_block'] else 'ALLOW'}"
        )
...
async def test_snapshot_manager():
    ...
    try:
        ...
        snapshot = await manager.create_snapshot(
            description="Test snapshot", snapshot_type="manual"
        )
        ...
        rollback_result = await manager.rollback(
            to_snapshot=snapshot_id, reason="test_rollback"
        )
