...
class PlanBuilder:
    ...
    def build_plan(
        self, task_req: Dict[str, Any], route: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.config.get("enable_preflight_checks", True):
                ...
                if not preflight_result["passed"]:
                    raise ValueError(
                        f"Pre-flight check failed: {preflight_result['reason']}"
                    )
    ...
    def _optimize_route(
        self, route: Dict[str, Any], task_req: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if latency_budget:
            optimized_route = self._optimize_for_latency(
                optimized_route, latency_budget
            )
    ...
    def _optimize_for_latency(
        self, route: Dict[str, Any], latency_budget: int
    ) -> Dict[str, Any]:
    ...
    def _optimize_for_cost(
        self, route: Dict[str, Any], cost_budget: float
    ) -> Dict[str, Any]:
    ...
    def _optimize_ensemble(
        self, route: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _preflight_checks(
        ...
        checks = {"passed": True, "reason": None, "details": []}
        ...
        try:
            ...
            if incompatible_models:
                checks["passed"] = False
                checks["reason"] = (
                    f"Model incompatibilities: {'; '.join(incompatible_models)}"
                )
                return checks
            ...
            if ensemble_method == "stack" and len(models) > 5:
                checks["details"].append(
                    "WARNING: Large ensemble may be resource intensive"
                )
    ...
    def update_plan_status(
        self, plan_id: str, status: str, details: Optional[Dict] = None
    ):
    ...
    def cleanup_completed_plans(self, max_age_hours: int = 24):
        ...
        for plan_id, plan in self.active_plans.items():
            ...
            if age_hours > max_age_hours and plan.get("status") in [
                "completed",
                "failed",
                "cancelled",
            ]:
