...
from .envelope import GraceMessageEnvelope, MessageKind, Priority, QoSClass
...
class MessageRouter:
    ...
    async def route_message(self, envelope: GraceMessageEnvelope) -> bool:
        ...
        try:
            ...
            for route in matching_routes:
                if self.enable_circuit_breaker and self.circuit_breakers.get(route.route_id, False):
                    continue
                ...
                try:
                    ...
                    if route.failure_count > 10:
                        ...
                        logger.warning(f"Circuit breaker activated for route {route.route_id}")
    ...
    async def _deliver_message(self, route: Route, envelope: GraceMessageEnvelope) -> bool:
    ...
    def get_route_health(self, route_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_success': route.last_success.isoformat() if route.last_success else None,
            'last_failure': route.last_failure.isoformat() if route.last_failure else None,
    ...
    def get_router_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'healthy_routes': sum(1 for r in self.routes.values() if r.health == RouteHealth.HEALTHY),
            'degraded_routes': sum(1 for r in self.routes.values() if r.health == RouteHealth.DEGRADED),
            'unhealthy_routes': sum(1 for r in self.routes.values() if r.health == RouteHealth.UNHEALTHY),
            'active_circuit_breakers': len([cb for cb in self.circuit_breakers.values() if cb]),
    ...
    async def health_check(self):
        ...
        for route_id, route in self.routes.items():
            ...
            if (self.circuit_breakers.get(route_id, False) and 
                route.last_failure and
                current_time - route.last_failure > timedelta(minutes=5)):
