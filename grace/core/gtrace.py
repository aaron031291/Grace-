...
from .contracts import EventType, generate_correlation_id, generate_decision_id, Experience
...
class TraceChain:
    ...
    def _validate_constitutional_compliance(self) -> None:
        ...
        if not self.metadata.constitutional_compliance:
            ...
            logger.warning(f"Trace {self.chain_id} flagged for governance review: {compliance_checks}")
    
    def _check_transparency(self) -> bool:
        """Check transparency compliance."""
        return len(self.events) > 0 and all(e.decision_narrative for e in self.events if e.event_type == "decision")
    ...
    def _check_consistency(self) -> bool:
        ...
        return len(set(e.component_id for e in self.events)) <= 3  # Reasonable component scope
    
    def _check_harm_prevention(self) -> bool:
        """Check harm prevention compliance."""
        return not any(e.error_data and "security" in str(e.error_data) for e in self.events)
...
class GraceTracer:
    ...
    def __init__(self, 
        ...
        self.trace_index: Dict[str, List[str]] = defaultdict(list)  # component_id -> trace_ids
    ...
    async def start_trace(self, 
        ...
        if governance_required and self.event_bus:
            governance_decision_id = await self._request_governance_validation(trace_chain)
    ...
    async def add_trace_event(self,
                             trace_id: str,
                             event_type: str,
                             operation: str,
                             input_data: Optional[Dict[str, Any]] = None,
        ...
        if contradictions:
            ...
            logger.warning(f"Contradictions detected in trace {trace_id}: {contradictions}")
        ...
        if event_type in ["decision", "policy_change", "security_action"]:
            event.governance_validated = await self._validate_with_governance(event, trace_chain)
    ...
    async def complete_trace(self, trace_id: str, 
                           success: bool = True,
        ...
        if self.kpi_monitor:
            await self.kpi_monitor.update_trust_score(
                trace_chain.metadata.component_id,
                trace_chain.final_trust_score
            )
        ...
        if self.memory_core:
            experience = Experience(
                ...
                context={
                    "trace_id": trace_id,
                    "operation": trace_chain.events[0].operation if trace_chain.events else "unknown",
        ...
        logger.info(f"Trace {trace_id} completed with status: {trace_chain.status.value}")
    ...
    async def _initialize_vault_compliance(self, metadata: TraceMetadata) -> None:
        ...
        metadata.vault_compliance[VaultCompliance.VAULT_2_CORRELATION.value] = bool(metadata.correlation_id)
        
        # Vault 8: Trust management
        metadata.vault_compliance[VaultCompliance.VAULT_8_TRUST.value] = metadata.trust_score >= 0.0
    ...
    async def _requires_sandbox(self, component_id: str, operation: str) -> bool:
        ...
        return (operation in high_risk_operations or 
                any(comp in component_id.lower() for comp in untrusted_components))
    
    async def _setup_sandbox_isolation(self, trace_chain: TraceChain) -> None:
        """Setup sandbox isolation for trace (Vault 15)."""
        trace_chain.metadata.vault_compliance[VaultCompliance.VAULT_15_SANDBOX.value] = True
        logger.info(f"Sandbox isolation enabled for trace {trace_chain.chain_id}")
    
    async def _detect_contradictions(self, event: TraceEvent, trace_chain: TraceChain) -> List[str]:
        ...
        for prev_event in trace_chain.events:
            if (prev_event.output_data and event.input_data and
                self._has_data_conflict(prev_event.output_data, event.input_data)):
                contradictions.append(f"Data conflict between {prev_event.event_id} and {event.event_id}")
    ...
    def _has_data_conflict(self, output_data: Dict[str, Any], input_data: Dict[str, Any]) -> bool:
    ...
    async def _validate_event_data(self, event: TraceEvent) -> bool:
        ...
        try:
            ...
            if event.input_data is not None and not isinstance(event.input_data, dict):
                return False
            if event.output_data is not None and not isinstance(event.output_data, dict):
                return False
    ...
    async def _validate_with_governance(self, event: TraceEvent, trace_chain: TraceChain) -> bool:
    ...
    async def _update_trust_score(self, trace_chain: TraceChain, delta: float) -> None:
        ...
        trace_chain.metadata.trust_score = max(0.0, min(1.0, trace_chain.metadata.trust_score + delta))
        trace_chain.metadata.vault_compliance[VaultCompliance.VAULT_8_TRUST.value] = True
    ...
    async def _final_constitutional_validation(self, trace_chain: TraceChain) -> bool:
        ...
        try:
            ...
            if trace_chain.events:
                ...
                critical_events = [e for e in trace_chain.events if e.event_type in ["decision", "policy_change"]]
                if critical_events and not all(e.governance_validated for e in critical_events):
                    validation_result = False
            
            trace_chain.metadata.vault_compliance[VaultCompliance.VAULT_14_CONSTITUTIONAL.value] = validation_result
    ...
    async def _ensure_audit_compliance(self, trace_chain: TraceChain) -> None:
        ...
        audit_compliant = (
            len(trace_chain.events) > 0 and
            trace_chain.metadata.correlation_id and
            all(e.timestamp for e in trace_chain.events)
        )
        
        trace_chain.metadata.vault_compliance[VaultCompliance.VAULT_10_AUDIT.value] = audit_compliant
    ...
    async def _request_governance_validation(self, trace_chain: TraceChain) -> Optional[str]:
    ...
    async def get_trace_status(self, trace_id: str) -> Optional[Dict[str, Any]]:
        ...
        trace_chain = self.active_traces.get(trace_id) or self.completed_traces.get(trace_id)
        if not trace_chain:
            return None
    ...
    async def get_component_traces(self, component_id: str, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        total_vaults_checked = len(VaultCompliance) * (len(self.active_traces) + len(self.completed_traces))
        
        if total_vaults_checked > 0:
            self.metrics["vault_compliance_rate"] = total_vault_compliance / total_vaults_checked
...
async def create_grace_tracer(event_bus: Optional[EventBus] = None,
                            memory_core: Optional[MemoryCore] = None,
                            immutable_logs: Optional[ImmutableLogs] = None,
                            kpi_monitor: Optional[KPITrustMonitor] = None) -> GraceTracer:
...
def trace_operation(tracer: GraceTracer, 
                   component_id: str, 
                   operation: str,
                   governance_required: bool = False):
...
__all__ = [
    'GraceTracer', 'TraceChain', 'TraceEvent', 'TraceMetadata',
    'TraceLevel', 'VaultCompliance', 'TraceStatus',
