...
async def test_orchestration_service():
    ...
    try:
        ...
        task_id = await service.scheduler.dispatch_task(
            loop_id="test_ooda", inputs={"test_data": "sample"}, priority=5
        )
...
async def test_component_integration():
    ...
    try:
        ...
        # Test monitoring
        watchdog.monitor(
            "test_task_123", "integration_test", "scheduler", timeout_minutes=1
        )
...
async def test_contracts_and_schemas():
    ...
    try:
        ...
        contracts_dir = (
            Path(__file__).parent.parent / "grace" / "orchestration" / "contracts"
        )
