...
class EventRoute:
    ...
    transform_payload: Optional[Dict[str, str]] = (
        None  # Field mappings for payload transformation
    )
...
class EnhancedEventBus:
    ...
    def load_routing_config(self, config_path: str):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.routing_rules)} routing rules and "
    ...
    def create_default_routing_config(self, config_path: str):
        ...
        try:
            ...
            self.routing_rules = [
                EventRoute(**rule) for rule in default_config["routing_rules"]
            ]
    ...
    async def subscribe(
        ...
        for subscription in self.subscribers[event_pattern]:
            if subscription.component_id == component_id:
                logger.warning(
                    f"Component {component_id} already subscribed to {event_pattern}"
                )
                return
    ...
    async def unsubscribe(self, component_id: str, event_pattern: str):
        ...
        if event_pattern in self.subscribers:
            self.subscribers[event_pattern] = [
                sub
                for sub in self.subscribers[event_pattern]
    ...
    def _transform_payload(
        self, payload: Dict[str, Any], transform_rules: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        for target_field, source_field in transform_rules.items():
            if source_field in payload:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to transform field {source_field} to {target_field}"
                    )
    ...
    async def publish(
        ...
        for rule in routing_rules:
            ...
            transformed_payload = self._transform_payload(
                payload, rule.transform_payload
            )
                ...
                for pattern, subscriptions in self.subscribers.items():
                    if self._matches_pattern(event_type, pattern):
                        for subscription in subscriptions:
                            if (
                                subscription.component_id == component_id
        ...
        for pattern, subscriptions in self.subscribers.items():
            if self._matches_pattern(event_type, pattern):
                for subscription in subscriptions:
                    if (
                        subscription.component_id not in delivered_to
                        and subscription.is_active
                    ):
                        task = self._safe_deliver(
                            subscription.handler, event, True, 3, 5000
                        )
                        delivery_tasks.append(task)
        ...
        logger.info(
            f"Published {event_type} event to {len(delivery_tasks)} handlers "
    ...
    async def _safe_deliver(
        ...
        for attempt in range(retry_attempts):
            try:
                ...
                logger.warning(
                    f"Handler timeout (attempt {attempt + 1}/{retry_attempts})"
                )
                if attempt == retry_attempts - 1:
                    logger.error(
                        f"Handler delivery failed after {retry_attempts} attempts (timeout)"
                    )
            except Exception as e:
                logger.warning(
                    f"Handler error (attempt {attempt + 1}/{retry_attempts}): {e}"
                )
                if attempt == retry_attempts - 1:
                    logger.error(
                        f"Handler delivery failed after {retry_attempts} attempts: {e}"
                    )
