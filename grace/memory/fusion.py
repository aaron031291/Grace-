...
class FusionMemory:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_entries_archived ON entries(archived)")
        ...
        try:
            with self._lock:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    """, (
                        entry.entry_id, entry.key, entry.content_type,
                        entry.size_bytes, entry.content_hash, entry.created_at.isoformat(),
                        False, json.dumps(entry.tags), json.dumps(entry.metadata),
                        entry.value_json
                    ))
    ...
    def search(self, 
              ...
              limit: int = 100) -> List[FusionEntry]:
        ...
        try:
            with self._lock:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    logger.info(f"Archived {archived_count} entries to {archive_filename}")
