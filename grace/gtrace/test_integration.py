...
async def test_orb_interface_tracing():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as tmp_file:
        tmp_file.write(
            "This is a test document for Grace tracing integration. It contains some sample content to test the memory ingestion pipeline with comprehensive tracing capabilities."
        )
...
""")
        ...
        try:
            ...
            search_results = await pipeline.search_memory(
                query="Grace Trace integration", user_id="test_user", limit=5
            )
...
async def test_trace_correlation_across_systems():
    ...
    async with tracer.async_span(
        "grace.user_workflow",
        tags={"workflow.type": "document_analysis", "user.id": "test_user"},
    ) as workflow_span:
        # Simulate Orb interface operations
        orb = GraceUnifiedOrbInterface()

        async with tracer.async_span(
            "grace.session_management", parent_context=workflow_span.context
        ) as session_span:
            session_id = await orb.create_session(
                "test_user", {"workflow": "document_analysis"}
            )
            session_span.set_tag("session.id", session_id)

        # Simulate document upload
        async with tracer.async_span(
            "grace.document_upload", parent_context=workflow_span.context
        ) as upload_span:
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=".txt", delete=False
            ) as tmp_file:
        ...
        async with tracer.async_span(
            "grace.memory_query", parent_context=workflow_span.context
        ) as query_span:
    ...
    for span in trace_spans:
        system = (
            span.operation_name.split(".")[0]
            if "." in span.operation_name
            else "unknown"
        )
...
async def test_trace_performance_monitoring():
    ...
    print("✅ Collector performance stats:")
    print(f"   - Total spans: {stats['current_stats']['total_spans']}")
    print(
        f"   - Storage utilization: {stats['current_stats']['storage_utilization']:.1%}"
    )
