...
async def test_memory_operations():
    ...
    async with auditor.audit_session("test_session_123") as session:
        ...
        read_id = await session.log_memory_read(
            {"query": "test query", "filters": {}, "results_count": 5},
            user_id="test_user",
        )
        ...
        response_id = await session.log_api_response(
            {"endpoint": "/api/test", "status": "success", "response_size": 1024},
            user_id="test_user",
        )

    print(
        f"✅ Session audit complete: read={read_id}, write={write_id}, response={response_id}"
    )
    return True
