...
class ErrorBudgetTracker:
    ...
    def record_breach(self, service_id: str, sli_type: str, breach_amount: float, duration_s: int = 60):
        """
        ...
        error_budget_days = policy.get("error_budget_days", 0.1)  # Default 2.4 hours per month
        ...
        budget_consumed = (duration_s / 86400) * consumption_rate  # Convert to fraction of daily budget
        ...
        logger.info(f"Recorded error budget consumption for {service_id}: {budget_consumed:.4f} days")
    ...
    def get_remaining_budget(self, service_id: str, window_days: int = 30) -> Dict:
        ...
        consumed_budget = sum(record["budget_consumed"] for record in recent_consumption)
        remaining_budget = max(0, total_budget - consumed_budget)
        remaining_pct = (remaining_budget / total_budget * 100) if total_budget > 0 else 100.0
    ...
    def should_block_deployment(self, service_id: str, risk_threshold_pct: float = 10.0) -> Dict:
        ...
        return {
            ...
            "recommendation": self._get_deployment_recommendation(remaining_pct, risk_threshold_pct)
        }
    ...
    def _get_deployment_recommendation(self, remaining_pct: float, threshold_pct: float) -> str:
