...
class ConsentService:
    ...
    def revoke_consent(self, consent_id: str) -> bool:
        ...
        logger.info(
            f"Revoked {consent.scope} consent {consent_id} for user {consent.user_id}"
        )
        return True

    def deny_consent(
        self, user_id: str, scope: str, evidence_uri: Optional[str] = None
    ) -> str:
    ...
    def list_pending_consents(self) -> List[ConsentRecord]:
        ...
        return [c for c in self.consents.values() if c.status == "pending"]
    ...
    def get_expiring_consents(self, days_ahead: int = 30) -> List[ConsentRecord]:
        ...
        return [
            c
            for c in self.consents.values()
            if c.status == "granted" and c.expires_at and c.expires_at <= cutoff_date
        ]
        ...
        for scope in ["autonomy", "pii_use", "external_share", "canary_participation"]:
            ...
            if scope_consents:
                ...
                summary[scope] = {
                    ...
                    "granted_at": latest.created_at.isoformat()
