...
class IngressMLTBridge:
    ...
    async def emit_experience(
        ...
        try:
            experience = IngressExperience(
                source_id=source_id, stage=stage, metrics=metrics, samples=samples
            )
    ...
    async def consume_adaptation_plan(self, plan: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.adaptation_plans[plan_id]["status"] = (
    ...
    async def get_parser_accuracy(
        self, parser_type: str, source_id: Optional[str] = None
    ) -> float:
    ...
    async def _emit_stage_experience(
        self, source_id: str, stage: str, stage_metrics: Dict[str, Any]
    ):
        ...
        try:
            ...
            metrics = {
                ...
                "avg_latency_ms": stage_metrics.get("latency_sum", 0)
                / max(events_count, 1),
            ...
            if stage == "parse":
                samples["parser_accuracy"] = await self.get_parser_accuracy(
                    self.ingress_kernel.sources[source_id].parser, source_id
                )
            elif stage == "enrich":
