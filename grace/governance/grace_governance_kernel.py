...
# Core infrastructure
from ..core import EventBus, MemoryCore, create_grace_tracer
...
class GraceGovernanceKernel:
    ...
    def __init__(
        ...
        if (
            isinstance(mtl_kernel, dict)
            and intelligence_kernel is None
            and config is None
        ):
    ...
    async def _initialize_governance_components(self):
        ...
        self.components["verification_engine"] = VerificationEngine(
            event_bus, memory_core, self.vault_engine
        )
    ...
    async def _setup_component_integration(self):
        ...
        trigger_mesh.register_component(
            "verification_engine", "governance", ["CLAIM_VERIFICATION"]
        )

        trigger_mesh.register_component(
            "unified_logic", "governance", ["CONSENSUS_REQUEST"]
        )
        ...
        trigger_mesh.register_component(
            "avn_core", "health", ["ANOMALY_DETECTED", "COMPONENT_FAILOVER"]
        )
        ...
        gtrace.register_governance_hook(
            "critical_decision", self._handle_critical_trace
        )
        ...
        try:
            ...
            try:
                ...
                request_dict = (
                    ...
                    else self._convert_request_to_dict(request)
                )
                ...
                try:
                    ...
                    vault_compliance_report = loop.run_until_complete(
                        self.vault_compliance_checker.check_priority_compliance(
                            request_dict
                        )
                    )
                except RuntimeError:
                ...
                if not vault_compliance_report.overall_compliance:
                    return {
                        ...
                        "vault_violations": [
                            v.description
                            for v in vault_compliance_report.critical_violations
                        ],
                        "vault_watermarks": [
                            r.watermark
                            for r in vault_compliance_report.vault_results.values()
                        ],
                ...
                logger.warning(
                    f"Vault compliance check failed: {e}, continuing with standard validation"
                )
            ...
            if requires_quorum and self.mtl_kernel:
                ...
                quorum_consensus = self.quorum_bridge.consensus(
                    feed_ids, context={"request": request}
                )
    ...
    def _check_requires_quorum(
        self, request, policy_results: List[Dict[str, Any]]
    ) -> bool:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        if "avn_core" in self.components:
            status["system_health"] = self.components[
                "avn_core"
    ...
    async def _handle_critical_trace(
    ...
    async def _detect_policy_contradictions(
        self, event_data: Dict[str, Any], trace_context: Dict[str, Any]
    ) -> List[str]:
                ...
                if hasattr(policy, "conflicts_with"):
                    if policy.conflicts_with(event_operation, component_id):
                        contradictions.append(
                            f"Policy {policy_id} conflicts with operation {event_operation}"
                        )
        ...
        if "trust_core" in self.components:
            ...
            if component_id and hasattr(trust_core, "get_trust_score"):
                ...
                if (
                    trust_score
                    and trust_score < 0.5
                    and "critical" in event_operation.lower()
                ):
                    contradictions.append(
                        f"Low trust component {component_id} performing critical operation"
                    )
                ...
                "claims": [
                    {
                        ...
                        "sources": [
                            {
                                "uri": "https://security-report.example.com",
                                "credibility": 0.8,
                            }
                        ],
                        "evidence": [
                            {"type": "doc", "pointer": "security_audit_2024.pdf"}
                        ],
                        "confidence": 0.85,
                        "logical_chain": [
                            {"step": "Security audit identified vulnerabilities"}
                        ],
                    }
                ],
