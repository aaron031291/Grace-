...
def _uncertainty_sampling(
    uncertainty: List[float], batch_size: int, min_confidence: float = 0.0
) -> List[str]:
    """Select items with highest prediction uncertainty."""
    # Filter by min confidence and sort by uncertainty (descending)
    candidates = [
        (i, u) for i, u in enumerate(uncertainty) if (1.0 - u) >= min_confidence
    ]
...
def _margin_sampling(
    ...
    candidates = [
        ...
        if (1.0 - uncertainty[i]) >= min_confidence
    ]
...
def _entropy_sampling(
    ...
    entropies = [
        -u * math.log2(u + 1e-10) - (1 - u) * math.log2(1 - u + 1e-10)
...
class ActiveLearningStrategies:
    ...
    def select_batch(self, cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        selected_indices = select_batch(
            uncertainty_scores, diversity_scores, coreset_indices, cfg
        )
        selected_items = [
            unlabeled_items[int(idx)]
            for idx in selected_indices
            if int(idx) < len(unlabeled_items)
        ]
    ...
    def _store_query(
        self, query_id: str, cfg: Dict[str, Any], selected_items: List[str]
    ):
