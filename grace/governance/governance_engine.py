...
class GovernanceEngine:
    ...
    async def _setup_event_subscriptions(self):
        ...
        await self.event_bus.subscribe(
            EventType.GOVERNANCE_VALIDATION.value, self._handle_governance_validation
        )

        # Subscribe to learning experiences for threshold tuning
        await self.event_bus.subscribe(
            "LEARNING_EXPERIENCE", self._handle_learning_feedback
        )
    ...
    async def handle_validation(
        ...
        try:
            logger.info(
                f"Processing governance validation {correlation_id} for decision {unified_decision.decision_id}"
            )
    ...
    async def _apply_constitutional_policies(
        self, decision: UnifiedDecision, claims: List[Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _check_transparency(
        self, decision: UnifiedDecision, context: Dict[str, Any]
    ) -> float:
    ...
    async def _check_fairness(
        self, decision: UnifiedDecision, claims: List[Any]
    ) -> float:
        ...
        fairness_signals = [
            signal
            for signal in decision.inputs.get("component_signals", [])
            if isinstance(signal, dict) and "fairness" in signal.get("component", "")
        ]

        if fairness_signals:
            avg_fairness = sum(
                signal.get("weight", 0) for signal in fairness_signals
            ) / len(fairness_signals)
    ...
    async def _check_harm_prevention(
        self, decision: UnifiedDecision, context: Dict[str, Any]
    ) -> float:
    ...
    async def _validate_thresholds(self, decision: UnifiedDecision) -> Dict[str, bool]:
        ...
        return {
            "confidence_sufficient": decision.confidence
            >= self.governance_thresholds["min_confidence"],
            "trust_sufficient": decision.trust_score
            >= self.governance_thresholds["min_trust"],
            "meets_all_thresholds": (
                decision.confidence >= self.governance_thresholds["min_confidence"]
                and decision.trust_score >= self.governance_thresholds["min_trust"]
            ),
        }
    ...
    async def _generate_verdict(
        ...
        return {
            ...
            "requires_manual_review": escalation_needed
            or outcome == "GOVERNANCE_NEEDS_REVIEW",
        }

    async def _audit_and_publish(
        self, verdict: Dict[str, Any], processing_start: datetime
    ):
            ...
            type(
                ...
                (),
                {
                    ...
                    "timestamp": datetime.fromisoformat(
                        verdict["timestamp"].replace("Z", "+00:00")
                    ),
                },
            )(),
        ...
        logger.info(
            f"Governance decision {decision_id} processed in {processing_time:.3f}s: {verdict['outcome']}"
        )

    async def _record_governance_experience(
        self, decision: UnifiedDecision, verdict: Dict[str, Any], processing_time: float
    ):
    ...
    async def _handle_learning_feedback(self, event: Dict[str, Any]):
        ...
        if experience_type == "CONSTITUTIONAL_COMPLIANCE":
            ...
            if compliance_score < 0.6:
                current_min = self.governance_thresholds[
    ...
    async def create_snapshot(self) -> GovernanceSnapshot:
        ...
        await self.event_bus.publish(
            EventType.GOVERNANCE_SNAPSHOT_CREATED.value, snapshot.to_dict()
        )
