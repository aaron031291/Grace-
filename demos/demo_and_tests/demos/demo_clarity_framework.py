...
class DemoGovernanceComponent(BaseComponent):
    ...
    async def process_governance_decision(self, payload: dict) -> GraceLoopOutput:
        ...
        reasoning_chain = ReasoningChain(
            chain_id=f"chain_{loop_id}", loop_type="governance"
        )
        ...
        loop_output.add_reasoning_step(
            ...
            output_data={
                "decision": "APPROVED",
                "rationale": "Complies with constitution",
            },
            confidence=0.9,
        )
...
class DemoLearningComponent(BaseComponent):
    ...
    async def health_check(self) -> dict:
        return {"status": "healthy", "patterns_learned": self.patterns_learned}

    async def get_metrics(self) -> dict:
        return {"patterns_learned": self.patterns_learned, "learning_rate": 0.02}


async def event_handler(event: dict):
    """Demo event handler."""
    print(
        f"📥 Received event: {event['type']} (correlation: {event['correlation_id']})"
    )
...
async def demo_clarity_framework():
    ...
    governance_result = await gov_component.process_governance_decision(
        {"claim": "This is a test governance decision", "context": "demo_run"}
    )
