...
class StateManager:
    ...
    def _define_valid_transitions(
        self,
    ) -> Dict[OrchestrationState, Set[OrchestrationState]]:
    ...
    async def _load_persisted_state(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                logger.info(
                    f"Loaded state: {self.current_state.value}, policies: {len(self.policies)}"
                )
    ...
    async def transition_state(
        self, to_state: OrchestrationState, trigger: str, context: Dict[str, Any] = None
    ) -> bool:
        """Transition to a new orchestration state."""
        if not self._is_valid_transition(self.current_state, to_state):
            logger.warning(
                f"Invalid state transition: {self.current_state.value} -> {to_state.value}"
            )
            return False
        ...
        logger.info(
            f"State transition: {old_state.value} -> {to_state.value} (trigger: {trigger})"
        )
        return True

    def _is_valid_transition(
        self, from_state: OrchestrationState, to_state: OrchestrationState
    ) -> bool:
    ...
    def _clear_policy_cache(
        self, policy_type: PolicyType = None, scope: PolicyScope = None
    ):
    ...
    async def evaluate_policy_compliance(
        self, context: Dict[str, Any], policy_type: PolicyType = None
    ) -> Dict[str, Any]:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "policies": {
                ...
                "by_scope": {
                    scope.value: sum(
                        1 for p in self.policies.values() if p.scope == scope
                    )
